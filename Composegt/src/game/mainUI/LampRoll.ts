	...
	{	
		...
		if (this._rollInfo) {
			if (this._rollInfo.dtype == 1) {
				...
				if(this._rollInfo.sexflag&&this._rollInfo.sexflag==1&&LanguageManager.checkHasKey("wifeName_"+this._rollInfo.need+"_male"))
				{
					rollingString = LanguageManager.getlocal("lampInfoType1_male",[this._rollInfo.name, LanguageManager.getlocal("wifeName_"+this._rollInfo.need+"_male")]);
				}
			}
			else if (this._rollInfo.dtype == 2) {
				let __quality: string = "";
				let __servant: Config.ServantItemCfg = Config.ServantCfg.getServantItemById(this._rollInfo.need)
				if (__servant.quality > 1) {
					__quality = LanguageManager.getlocal(`servantQuality_name${__servant.quality}`);
				}
				rollingString = LanguageManager.getlocal("lampInfoType2",[this._rollInfo.name, __quality, LanguageManager.getlocal("servant_name"+this._rollInfo.need)]);
			}
			...
			}else if (this._rollInfo.dtype == 6) {
				let godName = this._rollInfo.info[0];
				rollingString = LanguageManager.getlocal("lampInfoType"+this._rollInfo.dtype, [godName.substring(5)] );
			}else{
