...
class ChatVo extends BaseVo
{
	...
	//不带分享的
	public worldVoObj2:any[] = [];
	//帮会聊天
	public allianceVoObj:any[] = [];
	public allianceVoObj2:any[] = [];
	/*
	* uid 作为唯一标识属性 {
		'10002147' : {
			chattext : {//消息记录
	...
	public initData(data:any):void
	{
		if(data)
		{
			...
			if(data.channel < 10000){
				...
				//不带分享的
				if (!data.content.stype)
				{
					this.worldVoObj2.push(data);
					if(this.worldVoObj2.length > 30)
					{
						this.worldVoObj2.shift();
					}
				}
				if(data.ts >= Api.chatVoApi.lastPublicMsgst){
					Api.chatVoApi.lastPublicMsgst = data.ts;
				}
			}else
			{	
				...
				if (!data.content.stype)
				{
					this.allianceVoObj2.push(data);
					if(this.allianceVoObj2.length > 30)
					{
						this.allianceVoObj2.shift();
					}
					if(data.sender != Api.playerVoApi.getPlayerID()){
						Api.chatVoApi.isNewAlliMsg = true;
					}
				}
				if(data.ts >= Api.chatVoApi.lastAlliMsgst){
					Api.chatVoApi.lastAlliMsgst = data.ts;
				}
			}
