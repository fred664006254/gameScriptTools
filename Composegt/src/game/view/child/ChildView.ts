...
class ChildView extends CommonView {
	...
	//册封加成
	private _childAddText:BaseTextField;
	//亲密度描述
	...
	public initView(): void {
		...
		//孩子背景
		let childBg: BaseLoadBitmap = BaseLoadBitmap.create("childview_newbg");
		childBg.width =640;
		childBg.height =440;
		...
		this.addChildToContainer(titleBg);
			...
			//进度条背景
		let barBg = BaseBitmap.create("public_balckbg");
		barBg.width = 640;
		// barBg.height = 
		barBg.x = 0;
		...
		if (Api.childVoApi.getChildNum() > 0) {
			...
			//等级蓝色背景图
			let servant_levebg = BaseBitmap.create("childview_lvbg")
			servant_levebg.x = 25;
			servant_levebg.y = barBg.y-3;
			this._childContainer.addChild(servant_levebg);

			//红色属性背景条  
			let servant_attributemap = BaseBitmap.create("playerview_canvas01")
			...
			let level = ComponentManager.getTextField(LanguageManager.getlocal("servant_infoLv"), 28, TextFieldConst.COLOR_WHITE);
			...
			let mSize = TextFieldConst.FONTSIZE_CONTENT_SMALL;
		...
		let bottomFoot = BaseBitmap.create("childview_bottombg");
	...
	private updChildAttData(childInfoVo: ChildInfoVo,isProAni?:boolean) {
		...
		{
			let artistryValue = LanguageManager.getlocal("wifeArtistry") + " " + wifeInfoVo.artistry;
	...
	protected showUpgradeEffect(isPro?:boolean)
	{
		...
		let upgradeClip = ComponentManager.getCustomMovieClip("servant_upgrade_frame",8,100);
