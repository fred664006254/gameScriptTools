...
class CreateUserView extends CommonView
{
	private _maleBody:BaseLoadBitmap | BaseLoadDragonBones;
	private _maleBodyTmp:BaseLoadDragonBones;


	private _maleHead:BaseLoadBitmap;
	private _maleHair:BaseLoadBitmap;
	private _maleContainer:BaseDisplayObjectContainer;

	private _femaleBody:BaseLoadBitmap | BaseLoadDragonBones;
	private _femaleBodyTmp:BaseLoadDragonBones;

	private _femaleHead:BaseLoadBitmap;
	private _femaleHair:BaseLoadBitmap;
	private _femaleContainer:BaseDisplayObjectContainer;
	...
	private _maleLoadNum:number = 0;
	private _femaleLoadNum:number = 0;
	...
	private _circle1:BaseBitmap = null;
	private _circle2:BaseBitmap = null;
	private _sexIcon:BaseBitmap = null;

	private _headList:any[] = [];

	private _curManId:number = 1;
	private _curWomanId:number = 6;

	private _descIcon1:BaseBitmap= null;
	private _descIcon2:BaseBitmap = null;

	private _descNode1:BaseDisplayObjectContainer = null;
	private _descNode2:BaseDisplayObjectContainer = null;

	private _descMask1:BaseBitmap = null;
	private _descMask2:BaseBitmap = null;

	private _descMask21:BaseBitmap = null;
	private _descMask22:BaseBitmap = null;

	private _sexClip:CustomMovieClip = null;
	private _isAnimPlaying:boolean = false;

	private _glowDragon:BaseLoadDragonBones = null;
	private _manHeadList:any[] = [];
	private _womanHeadList:any[] = [];

	//头像角度间隔
	private _headAngle = 24;
	//头像半径
	private _headRadius = 336;

	private _headX = 385;
	private _headY = 360;
	private _headAnimTime = 100;

	private _headAllTime = 300;
	private _flower:BaseBitmap = null;
	private _inputTF = null;

	private _maxLoadHeadNum = 10;
	private _curLoadHeadNum = 0;
	public constructor() 
	{
		super();
	}
	...
	protected getResourceList():string[]
	{
		let rewardPic:string[] = [
            "createuser_bg",// "guide_createuserbg",  createuser_title
            "createuser_bottom",
            "createuser_btn",
            "createuser_btn1",
            "createuser_btn2",
            ...
            "createuser_headbg2",
            "createuser_mask",
            "createuser_namebg",// "dinner_jz_01","guide_namebg"
	...
	protected initView():void
	{
        ...
        let bottom = BaseBitmap.create("createuser_bottom");
        bottom.x = 0;
        bottom.y = GameConfig.stageHeigth - bottom.height;
        this.addChild(bottom);


		let inputY = bottom.y - 70 - 46 + (1136 - GameConfig.stageHeigth) / (1136 - 960) * 35;

		if (!this.param ){
        	//输入框
			let inputBg:BaseBitmap = BaseBitmap.create("createuser_namebg");
			inputBg.x = this.viewBg.x + this.viewBg.width/2 - inputBg.width/2;

            inputBg.y =  inputY;
			this.addChild(inputBg);
        
			let inputTF = ComponentManager.getInputTextField(TextFieldConst.COLOR_LIGHT_YELLOW,TextFieldConst.FONTSIZE_TITLE_SMALL,200,46,null);
			inputTF.x = 220;
			...
			let randomBtn = ComponentManager.getButton("createuser_randombtn","",this.clickRanomHandler,this);
			randomBtn.x = 420;
		...
		let mask = BaseBitmap.create("createuser_mask");
		...
		let maleResPath = "chuangjue_male";
		let femaleResPath = "chuangjue_female";

		let maleImgResPath = "chuangjue_male";
		let femaleImgResPath = "chuangjue_female";
		...
		// if(1==1)
		{
				...
				let maleLoadComplete=function(container:BaseDisplayObjectContainer):void
				{
					this._maleLoadNum ++;
				...
				let maleLoadCompleteD=function(container:BaseDisplayObjectContainer):void
				{
					if(this._maleBodyTmp)
					{
						let temp = this._maleBody;
				...
				let femaleLoadComplete=function(container:BaseDisplayObjectContainer):void
				{
					this._femaleLoadNum ++;
				...
				let femaleLoadCompleteD=function(container:BaseDisplayObjectContainer):void
				{

					
					if(this._femaleBodyTmp)
					{
						let temp = this._femaleBody;
				...
				let rect12:egret.Rectangle= new egret.Rectangle();//egret.Rectangle.create();
				...
				let rect:egret.Rectangle= new egret.Rectangle();//egret.Rectangle.create();
			...
			this._maleBody = <BaseLoadBitmap>this._maleContainer.getChildByName("myBody");
			this._maleHead = <BaseLoadBitmap>this._maleContainer.getChildByName("myHead");
			this._maleHair = <BaseLoadBitmap>this._maleContainer.getChildByName("myHair");

			this._femaleBody = <BaseLoadBitmap>this._femaleContainer.getChildByName("myBody");
			this._femaleHead = <BaseLoadBitmap>this._femaleContainer.getChildByName("myHead");
			this._femaleHair = <BaseLoadBitmap>this._femaleContainer.getChildByName("myHair");
		...
		this.createHeadList();
		let iniSexIcon = "createuser_btn1";
		// if(picid <=5){
		...
		if(this.param){
			...
			let tempPic = Api.playerVoApi.getPlayePicId();
			if(tempPic > 10){
				tempPic = 1;
			}
		...
		let createBtn = ComponentManager.getButton("createuser_btn",btnStr,this.clickCreateHandler,this,null,null,26);
		...
		let createBtnAnim = ComponentManager.getCustomMovieClip("createuser_anim", 10, 100);
	...
	private flowerAnim():void
	{
		...
		let x = this._flower.x;
		let y = this._flower.y;
		egret.Tween.get(this._flower,{loop:true})
	...
	private createHeadList():void
	{
		...
		let headMask1 = BaseBitmap.create("createuser_headmask");
		...
		let headMask2 = BaseBitmap.create("createuser_headmask");
		...
		for (var index = 0; index < 10; index++) {
			...
			let headNode = this.createHead(index + 1);
			if(index <=4){
	...
	private createHead(headId:number):BaseDisplayObjectContainer
	{
	
		// let radius = 336;

		let headNode = new BaseDisplayObjectContainer();
		headNode.width = 100;
		headNode.height = 100;
		...
		let head = new BaseDisplayObjectContainer();
		head.width = 100;
		head.height = 100;
		...
		let headBg1 = BaseBitmap.create("createuser_headbg1");
		...
		let headBg2 = BaseBitmap.create("createuser_headbg2");
		...
		let selclip = ComponentManager.getCustomMovieClip("createuser_headeff",7,150);
		...
		let headPic = "user_head" + headId;
		let userHead:BaseLoadBitmap = BaseLoadBitmap.create(headPic,null,{callback:this.headLoadComplete,callbackThisObj:this});
		userHead.width = 136;
		userHead.height = 143;
	 ...
	 */
	 ...
	 private showLoop()
	 {
		...
		let words = ComponentManager.getTextField(wordStr ,20);

		let wordNode = new BaseDisplayObjectContainer();
		let wordBg = BaseBitmap.create("public_9_viewmask");
		wordBg.alpha = 0.5;
		...
		.to({y:85},2800).call(
			function(wordNode:BaseDisplayObjectContainer){
	...
	private sexIconClick()
	{
		...
		let centerX = this.viewBg.x + this.viewBg.width/2 - 382/2 + 5 + 52;
		let centerY = 160 + 125;
    ...
    private initHead()
	{
			...
			for(let i = 0; i < this._headList.length/2; i ++){
				let head = this._headList[i];
			...
			for(let j = 5; j < this._headList.length; j ++){
				let head = this._headList[j];
