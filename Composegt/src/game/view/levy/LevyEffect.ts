class LevyEffect extends BaseClass
{
	private _bitmapList:BaseBitmap[]=[];
	private _maxNum:number=3;
	...
	public start(resKey:string,startPoint:egret.Point,endPoint:egret.Point,displayContainer:BaseDisplayObjectContainer,endCallback?:Function,endCallbackThisObj?:any,endCallbackParams?:any[]):void
	{
		...
		for(var i:number=0;i<this._maxNum;i++)
		{
			...
			bmp.alpha=0;
			let randX:number=Math.max(0,startPoint.x-30) +Math.random()*60;
