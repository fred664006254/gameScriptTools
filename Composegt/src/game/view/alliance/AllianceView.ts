...
class AllianceView extends CommonView
{
	...
	public initView():void
	{
		...
		let bottomBg = BaseLoadBitmap.create("alliance_bg1");
		bottomBg.x = 0;
		bottomBg.y = -15;
		...
		let idTF =  ComponentManager.getTextField(idStr, 20, TextFieldConst.COLOR_WHITE);
		...
		//	公告
		let noticeBg = BaseBitmap.create("public_9v_bg10");
		noticeBg.width = 200
		noticeBg.height = 300;
		noticeBg.x = 400;
		noticeBg.y = 200;
		...
		let noticeTF = ComponentManager.getTextField(LanguageManager.getlocal("allianceNotice"), TextFieldConst.FONTSIZE_TITLE_SMALL, TextFieldConst.COLOR_WHITE);
		// this._levelTF.setScale(0.8); 
	...
	private initIcons()
	{
		let acIconList=Api.acVoApi.getAllianceActivityIcons();
	...
	private initButtom():void
	{
		...
		this.addChild(this._bottomContiner);
				...
				//聊天按钮相关
		let chatbg = BaseBitmap.create(ResourceManager.getRes("public_lt_bg01"));
		...
		let bottomBg:BaseBitmap = BaseBitmap.create("alliance_bg");
		...
		// }
		let isManage = myAllianceVo.po == 1 || myAllianceVo.po == 2;
	...
	private showList():void
	{
			...
			{
				let atkracedes3 = ComponentManager.getTextField(LanguageManager.getlocal("atkracedes3"), 20,TextFieldConst.COLOR_LIGHT_YELLOW);
				atkracedes3.x =250;
				atkracedes3.y =300;
	...
	private showText():void
	{
		if(this._atkraceInfoVoList&&this._atkraceInfoVoList.length>0)
		{
			...
			let dtype = this._atkraceInfoVoList[0][0];
	...
	public tick()
	{
		...
		if (this._bottomContiner) {
			...
			if (Api.switchVoApi.checkOpenAllianceWar()&&(allianceVo.level >= Config.AlliancewarCfg.allianceLevelNeed)) {
				...
				let warBtn = <BaseButton>this._bottomContiner.getChildByName("war");
