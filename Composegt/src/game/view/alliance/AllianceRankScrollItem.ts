...
class AllianceRankScrollItem extends ScrollListItem
{
	...
	public initItem(index:number,rankData:any):void
	{
		...
		let lvTF = ComponentManager.getTextField(lvStr,TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN);
		...
		let attrStr = LanguageManager.getlocal("allianceRankTotalPower",[App.StringUtil.changeIntToText(rankData.affect)]) ;
		...
		let memberStr = LanguageManager.getlocal("allianceMemberTitle2",[rankData.mn + "/" + rankData.maxmn]);
		let memberTF = ComponentManager.getTextField(memberStr,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
