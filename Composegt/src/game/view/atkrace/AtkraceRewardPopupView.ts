...
class AtkraceRewardPopupView extends PopupView
{
	...
	{	
		...
		let itemBg:BaseBitmap = BaseBitmap.create("public_9v_bg12");
		itemBg.width = 530;
		itemBg.height = 340;
	...
	{
		let awardBg:BaseBitmap = BaseBitmap.create("public_9v_bg15");
		awardBg.width = 540;
		awardBg.setPosition(GameConfig.stageWidth/2 - awardBg.width/2, this.viewBg.y + this.viewBg.height + 20);
		this.addChild(awardBg);

		let awardText:BaseTextField = ComponentManager.getTextField(this._rewardsStr,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WARN_YELLOW_NEW);
