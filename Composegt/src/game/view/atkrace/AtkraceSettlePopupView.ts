 ...
 * 也可以直接把接口返回的data直接传入
 */
class AtkraceSettlePopupView extends PopupView {
    public constructor() {
        super();
    }

    private data: AtkraceSettleInfo;

    protected initView() {
        Api.rookieVoApi.checkWaitingGuideByKey("atkrace_4");
        // this.data = this.param.data;
        this.initData();

        let _bg = BaseBitmap.create(this.data.type == 1 ? "atkrace_bg5":"atkrace_bg4");
        this.addChildToContainer(_bg);
        let _title = BaseBitmap.create(this.data.type == 1 ? "atkrace_label2":"atkrace_label1");
        this.addChildToContainer(_title);
        _title.setPosition((this.getShowWidth()-_title.width)/2, 34);

        const _comB: string = this.data.type == 1 ? "atkrace_btn2":"atkrace_btn1";
        let _confirmBtn = ComponentManager.getButton(_comB, "", this.onComfirmTap, this, null, 0);
        this.addChildToContainer(_confirmBtn);
        _confirmBtn.setPosition((this.getShowWidth()-_confirmBtn.width)/2, 688);

        let _tipKey: string = this.data.type == 1 ? "atkrace_addtext13":"atkrace_addtext12";
        if (this.data.fightnum == 0) {
            _tipKey = "atkrace_addtext20";
        }
        let _tip = ComponentManager.getTextField(
            LanguageManager.getlocal(_tipKey, [this.data.fightnum + ""]),
            30, this.data.type == 1 ? 0xacacac : 0xfff0b5
        )
        ...
        // 蓝方--我
        let _pbg_B = BaseBitmap.create("atkrace_bg7");
        this.addChildToContainer(_pbg_B);
        _pbg_B.setPosition(0, 376);
        let _head_B = Api.playerVoApi.getPlayerCircleHead(this.data.pic);
        this.addChildToContainer(_head_B)
        _head_B.setPosition(
            this.getShowWidth()/2 - (_head_B.width + 3),
            _pbg_B.y-12
        )

        let _name_B = ComponentManager.getTextField(this.data.name, 20, 0xfff1b3);
        _name_B.width = 200;
        _name_B.textAlign = TextFieldConst.ALIGH_RIGHT;
        this.addChildToContainer(_name_B);
        _name_B.setPosition(0, _pbg_B.y + 17);
        let _score_B = ComponentManager.getTextField(
            LanguageManager.getlocal("atkrace_addtext14", [this.data.score]),
            20, 0x3ae269
        );
        _score_B.width = 200;
        ...
        // 红方--敌
        let _pbg_R = BaseBitmap.create("atkrace_bg6");
        this.addChildToContainer(_pbg_R);
        _pbg_R.setPosition(this.getShowWidth() - _pbg_R.width, 376);
        let _head_R = Api.playerVoApi.getUserCircleHead(this.data.pic2);
        this.addChildToContainer(_head_R)
        _head_R.setPosition(
            this.getShowWidth()/2 + 3,
            376-12
        )

        let _name_R = ComponentManager.getTextField(this.data.uname2, 20, 0xfff1b3);
        _name_R.width = 200;
        ...
        let _score_R = ComponentManager.getTextField(
            LanguageManager.getlocal("atkrace_addtext14", [this.data.score2]),
            20, 0xfddad6
        );
        _score_R.width = 200;
        ...
        if (this.data.rewards && this.data.rewards.length > 0) {
            let _rewTip = ComponentManager.getTextField(LanguageManager.getlocal("atkrace_addtext15"), 24, 0xfef0b3);
            ...
            let _icons = GameData.getRewardItemIcons(this.data.rewards, true);
            const _startX = (this.getShowWidth() - _icons.length * (94 + 16) + 16) / 2;
     ...
     */
    private initData() {
        let _param: {addPoint: number, rewards?: string, win: number} = this.param.data;
