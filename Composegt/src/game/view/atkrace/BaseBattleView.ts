...
class BaseBattleView extends CommonView
{
	...
	private _upduizhanpifu:BaseLoadDragonBones;
	private _downduizhanpifu:BaseLoadDragonBones;
	public constructor() {
		super();
	}
	...
	public showSkinEffectAniHandle(area:number,moveTime1:number,critTime:number,tan:number)
	{
		if(area == 1 && this._downduizhanpifu){
			this._downduizhanpifu.rotation =  tan;
			egret.Tween.get(this._downduizhanpifu).wait(critTime+300).call(()=>{
				this._downduizhanpifu.playDragonMovie("idle",1);
			},this).wait(moveTime1+200).call(()=>{
				this._downduizhanpifu.stop();
				this._downduizhanpifu.rotation = 0;
			},this);
		}

		if(area == 2 && this._upduizhanpifu){
			this._upduizhanpifu.rotation =  tan;
			egret.Tween.get(this._upduizhanpifu).wait(critTime+300).call(()=>{
				this._upduizhanpifu.playDragonMovie("idle",1);
			},this).wait(moveTime1+200).call(()=>{
				this._upduizhanpifu.stop();
				this._upduizhanpifu.rotation = 0;
			},this);
		}
	}
	/**
	...
	protected attackHandle(area:number,damage:number,isCrit?:boolean):void
	{
		...
		//皮肤特效的动画
		let tan = 0;//Math.abs( Math.tan(moveY/offsetX) *180/Math.PI );
		if (area == 1) {
	...
	private showCritAnim(area:number):void
	{
		...
		let speed_endY = baseobject.y - (226 * 1.6 - baseobject.serImgHeight) / 2;
		...
		let endX = area == 1 ? (80) : (baseobject.x + baseobject.serImgWith);
		let endY = baseobject.y + baseobject.serImgHeight / 2 - 75 * 0.7;
