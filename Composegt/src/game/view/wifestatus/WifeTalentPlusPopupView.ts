 ...
 * author jiangliuyang
 */
class WifeTalentPlusPopupView extends PopupView
{
	// 滑动列表
	...
	private modelRefresh()
	{
		...
		if(this._scrollList2){
			let infoList2 = Api.wifebattleVoApi.wifebattleVo.info.tmpattr.wifeadd_arr;
	...
	{		
		...
		let plusDesc:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("wifeTalentPlusPopupViewCurPlus1",[String(statusadd)]),20,TextFieldConst.COLOR_BROWN_NEW);
		...
		if(this.checkHaveBuff()){
			let rcfg:Config.AcCfg.WifeBattleRankCfg =  <Config.AcCfg.WifeBattleRankCfg> (Api.acVoApi.getActivityVoByAidAndCode("wifeBattleRank").config);
			...
			maxV +=1;
			//才情当前等级txt1
			let lvtxt1 = ComponentManager.getTextField(lvStr,18,TextFieldConst.COLOR_BROWN_NEW);
			lvtxt1.setPosition(80 , 20);
			this.addChildToContainer(lvtxt1);

			let lvtxt2 = ComponentManager.getTextField(lvStr2 ,18,TextFieldConst.COLOR_BROWN_NEW);
		...
		let descText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("wifeTalentPlusPopupViewDesc"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
		...
		if(this.checkHaveBuff()){
			...
			let descText2:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("wifeTalentPlusPopupViewDesc2"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
