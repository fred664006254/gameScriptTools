...
class AffairView  extends CommonView
{
	...
	{
		let bigBg:BaseLoadBitmap = BaseLoadBitmap.create("affairview_bg");
		bigBg.y = -15;
		...
		let topbg = new MainUINewTop({showName:false});
		...
		let timesBg = BaseBitmap.create("prisonview_1");
		timesBg.x =280;
		...
		let bottomBg:BaseBitmap = BaseBitmap.create("itemview_daoju_bg01");
		bottomBg.x = 0; 
		...
		//桌子
		let affairview_table = BaseBitmap.create("public_bottombg1");
	...
	protected checkAffairNum()
	{
		...
		if (Api.manageVoApi.getCurAffairNums() > 0)
		{
			...
			if(Api.switchVoApi.checkOpenOfficialbusiness())
			{
				let needVip = Api.vipVoApi.getNeedVip("affairUnlock");
	...
	private initButtomContainer():void
	{
		...
		let selectedDescTextF = ComponentManager.getTextField("",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
