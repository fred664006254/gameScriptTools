...
class AllianceWarView extends CommonView {
	...
	private _rewardBtntxt:BaseBitmap;
	private _sendBtntxt:BaseBitmap;
	...
	public initView(): void {
		...
		//顶部的背景
		this._topBg = BaseBitmap.create("tigertrappass_topbg");
		...
		this._topTip.setPosition(this._topBg.x + this._topBg.width / 2 - this._topTip.width / 2, this._topBg.y + 15);
		...
		let scoreBtntxt = BaseBitmap.create("awscorerank_txt")
		scoreBtntxt.setPosition(scoreBtn.x + scoreBtn.width/2 -scoreBtntxt.width/2 , scoreBtn.y + scoreBtn.height -scoreBtntxt.height);
		this.addChildToContainer(scoreBtntxt);

		//底部的背景
		this._buttomBg = BaseBitmap.create("arena_bottom");
		this._buttomBg.height = 110;
		// this._buttomBg.setPosition(bg.x, bg.y + bg.height - this._buttomBg.height);
		this._buttomBg.setPosition(bg.x,GameConfig.stageHeigth - this.container.y - this._buttomBg.height);
		...
		let tsize1 =  TextFieldConst.FONTSIZE_CONTENT_COMMON;

		if(PlatformManager.checkIsViSp() || PlatformManager.checkIsKRSp() || PlatformManager.checkIsJPSp()||PlatformManager.checkIsKRNewSp()){
			tsize1 = 18;
		}
		//总战斗力
		this._allFightTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarViewAllFight", [""]), tsize1, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		//参战门客
		this._servantNameTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarViewServantName", [""]), tsize1, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		//我的战斗力
		this._myFightTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarViewMyFight", [""]), tsize1, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		this._buttomTimeBg.setPosition(this._infoBg.x + this._infoBg.width - this._buttomTimeBg.width - 30, this._infoBg.y - this._buttomTimeBg.height - 3);
		...
		let rewardBtntxt = BaseBitmap.create("awawardbtn_txt")
		rewardBtntxt.setPosition(this._reviceRewardBtn.x + this._reviceRewardBtn.width/2 -rewardBtntxt.width/2 , this._reviceRewardBtn.y + this._reviceRewardBtn.height -rewardBtntxt.height);
		this.addChildToContainer(rewardBtntxt);
		this._rewardBtntxt = rewardBtntxt;
		...
		App.DisplayUtil.setLayoutPosition(LayoutConst.horizontalCenterbottom, btn, this._reviceRewardBtn, [0,this._reviceRewardBtn.height+20]);
		this._sendBtn = btn;

		let sendBtntxt = BaseBitmap.create("awsend_txt")
		sendBtntxt.setPosition(this._sendBtn.x + this._sendBtn.width/2 -sendBtntxt.width/2 , this._sendBtn.y + this._sendBtn.height -sendBtntxt.height);
		this.addChildToContainer(sendBtntxt);
		this._sendBtntxt = sendBtntxt;

		//第一个log 第一段话
		this._firstLog1Txt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarViewFirestLog1", ["", ""]), TextFieldConst.FONTSIZE_BUTTON_COMMON, TextFieldConst.COLOR_WARN_GREEN2);
		this._firstLog1Txt.setPosition(5, this._buttomBg.y + 16);
		...
		if(PlatformManager.checkIsViSp() || PlatformManager.checkIsKRSp() || PlatformManager.checkIsJPSp()||PlatformManager.checkIsKRNewSp()){
			this._firstLog2Txt.size = 18;
			this._firstLog2Txt.width = 480;
			this._firstLog2Txt.multiline = true;
			this._firstLog2Txt.lineSpacing = 4;
		}
		...
		this._npcTalkBg = BaseBitmap.create("public_9v_bg11");
		this._npcTalkBg.width = 300;
		// this._npcTalkBg.anchorOffsetX = this._npcTalkBg.width;
		this._npcTalkTxt.width = this._npcTalkBg.width - 30;
		this._npcTalkBg.height = this._npcTalkTxt.height + 50;
		this._npcTalkBg.scaleX = -1;

		this._npcTalkBg.setPosition(this._talkArcher.x + this._npcTalkBg.width - 25, this._talkArcher.y - this._npcTalkBg.height + 3);
		this._npcTalkTxt.setPosition(this._npcTalkBg.x - this._npcTalkBg.width / 2 - this._npcTalkTxt.width / 2, this._npcTalkBg.y + this._npcTalkBg.height / 2 - this._npcTalkTxt.height / 2-10)
	...
	private npcManagerClick() {
		...
		let scene = '';
		let param: any = {};
		this._sendBtntxt.visible = this._sendBtn.visible = period == 1;
	...
	protected tick() {
		...
		this._sendBtntxt.visible = this._sendBtn.visible = periodType == 1;
		switch (periodType) {
			case 1:
				if (isChange) {
					...
					if (Api.allianceWarVoApi.isFight()) {
						...
						this._rewardBtntxt.visible = this._reviceRewardBtn.visible = true;
					}
					else {
						this._buttomTimeBg.setVisible(false);
						this._buttomTime.setVisible(false);
						this._rewardBtntxt.visible = this._reviceRewardBtn.visible = false;
					}
				}
	...
	protected getResourceList(): string[] {
		return super.getResourceList().concat([
			"awscorerank", "awscorerank_down", "awawardbtn", "awawardbtn_down", "arena_bottom","awscorerank_txt","awawardbtn_txt",
			...
			"tigertrappass_topbg","awsend_txt",
		]);
	}
	/**
	 * model 刷新监听事件
	...
	private allianceWarModelHandle() {
		...
		this._npcTalkTxt.width = this._npcTalkBg.width - 30;
		this._npcTalkBg.height = this._npcTalkTxt.height + 50;
		// this._npcTalkBg.setPosition(this._talkArcher.x - 25, this._talkArcher.y - this._npcTalkBg.height + 3);
		this._npcTalkTxt.setPosition(this._npcTalkBg.x - this._npcTalkBg.width / 2 - this._npcTalkTxt.width / 2, this._npcTalkBg.y + this._npcTalkBg.height / 2 - this._npcTalkTxt.height / 2-10)
	...
	public dispose(): void {
		...
		this._sendBtn = null;
		this._rewardBtntxt = null;
		this._sendBtntxt = null;
		super.dispose();
	}
}