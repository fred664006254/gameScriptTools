...
class WifeskinView extends CommonView {
	...
	private _wifeSkinVo: WifeskinInfoVo;
	...
	private _lineCon:BaseDisplayObjectContainer;

	private _detailBtn:BaseButton = null;

	private _curSkinId:string = null;
	...
	public initView(): void {
		...
		let officeTxt = ComponentManager.getTextField(LanguageManager.getlocal("wifeskin_valuetitle"),24,TextFieldConst.COLOR_BROWN_NEW);
		...
		let startY = innerbg.y + 17;
        for (var index = 0; index < 10; index++) {
			let addX = 0;
			if(index%2>0){
				addX = 270;
			}
            let attrNameTxt =  ComponentManager.getTextField("0",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN_NEW);
		...
		let tipTxt = ComponentManager.getTextField(LanguageManager.getlocal("wifeskinDesc"),TextFieldConst.FONTSIZE_CONTENT_SMALL);
	...
	private initState()
	{
		...
		if(this._wifeSkinVo)
		{
			let skinCfg = Config.WifeskinCfg.getWifeCfgById(this._wifeSkinVo.equip);
			if(skinCfg)
			{
				let id = skinCfg.id;
				this.setData(id)
	...
	{	
		...
		if(this._childScrollItem)
		{
			if(this._childScrollItem.getChildByName("bg2"))
			{
				...
				bg.texture = ResourceManager.getRes("wifeview_hongyantyouxiang1")
	...
	protected dealAttrChangeInfo(skinId:string)
    {
		...
		let atkLvUpAdd = skinCfg.atkLvUpAdd;
		let inteLvUpAdd = skinCfg.inteLvUpAdd;
		let politicsLvUpAdd = skinCfg.politicsLvUpAdd;
		let charmLvUpAdd = skinCfg.charmLvUpAdd;

        let wifeIntimacy = skinCfg.wifeIntimacy;
        let wifeGlamour = skinCfg.wifeGlamour;

		let wifeLvUpIntimacy = skinCfg.wifeLvUpIntimacy;
        let wifeLvUpGlamour = skinCfg.wifeLvUpGlamour;
		...
		{
			let baseV = atkAdd && atkAdd[0] == 1 ? atkAdd[1]:0;
			let baseP = atkLvUpAdd && atkLvUpAdd[0] == 1 ? atkLvUpAdd[1]:0;
		...
		{
			let baseV = inteAdd && inteAdd[0] == 1 ? inteAdd[1]:0;
			let baseP = inteLvUpAdd && inteLvUpAdd[0] == 1 ? inteLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = politicsAdd && politicsAdd[0] == 1 ? politicsAdd[1]:0;
			let baseP = politicsLvUpAdd && politicsLvUpAdd[0] == 1 ? politicsLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = charmAdd && charmAdd[0] == 1 ? charmAdd[1]:0;
			let baseP = charmLvUpAdd && charmLvUpAdd[0] == 1 ? charmLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = atkAdd && atkAdd[0] == 2 ? atkAdd[1]:0;
			let baseP = atkLvUpAdd && atkLvUpAdd[0] == 2 ? atkLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = inteAdd && inteAdd[0] == 2 ? inteAdd[1]:0;
			let baseP = inteLvUpAdd && inteLvUpAdd[0] == 2 ? inteLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = politicsAdd && politicsAdd[0] == 2 ? politicsAdd[1]:0;
			let baseP = politicsLvUpAdd && politicsLvUpAdd[0] == 2 ? politicsLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		{
			let baseV = charmAdd && charmAdd[0] == 2 ? charmAdd[1]:0;
			let baseP = charmLvUpAdd && charmLvUpAdd[0] == 2 ? charmLvUpAdd[1]:0;
			if(baseV + (lv-1)*baseP > 0){
		...
		if((wifeIntimacy && wifeIntimacy > 0) || (wifeLvUpIntimacy && wifeLvUpIntimacy > 0)){
			...
			let baseV = wifeIntimacy ? wifeIntimacy : 0;
			let baseP = wifeLvUpIntimacy ? wifeLvUpIntimacy : 0;
			if(baseV + (lv-1)*baseP > 0){
		...
		if((wifeGlamour && wifeGlamour > 0)||(wifeLvUpGlamour && wifeLvUpGlamour > 0)) {
			let baseV = wifeGlamour ? wifeGlamour : 0;
			let baseP = wifeLvUpGlamour ? wifeLvUpGlamour : 0;
			if(baseV + (lv-1)*baseP > 0){
