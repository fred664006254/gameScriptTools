class AtkracecrossServer2Item extends ScrollListItem
{
	public constructor() {
		super();
	}
	...
	protected initItem(index:number,data:any)
    {
		...
		this.height = 40;
		this._data = data;
		let rankTxt = ComponentManager.getTextField(String(index+1),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
        this.addChild(rankTxt);
	   
		let namestr = Api.mergeServerVoApi.getAfterMergeSeverName(null,true,data.zid);
        let nameTxt = ComponentManager.getTextField(namestr,TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
        this.addChild(nameTxt);

		let scoreTxt = ComponentManager.getTextField(String(data.point),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
