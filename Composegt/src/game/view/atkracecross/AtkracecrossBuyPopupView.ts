class AtkracecrossBuyPopupView  extends PopupView
{
	...
	protected initView():void
	{
		...
		// let gemsBg:BaseBitmap = BaseBitmap.create("public_tc_bg02");
		// gemsBg.setPosition(55,12);
		...
		this._moraleText = ComponentManager.getTextField( Api.atkracecrossVoApi.getMyInfo().morale.toString(),20,TextFieldConst.COLOR_WARN_YELLOW_NEW);
	...
	private getBuyTypeContainer(info:any, idx:number , type:number):BaseDisplayObjectContainer
	{
		...
		if (isBuy == "0" || !isBuy) {
			...
			buyBtn.setPosition(bgContainer.width - buyBtn.width - 10, bgContainer.height/2-buyBtn.height/2);
