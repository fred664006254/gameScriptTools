class PrisonView  extends CommonView
{
    ...
    public initView():void
	{
		...
		{	
			let progressNum:number= 410;
		  	progressNum =494; 
			let progressBar:ProgressBar=ComponentManager.getProgressBar("progress_type1_red","progress_type1_bg2",progressNum);
		...
		let nameBg:BaseBitmap=BaseBitmap.create("prisonview_1");
		if(this._newPrisonBoo)
		{
	...
	private showDialogue():void
	{
		...
		var _dialogueBgStr:string = "public_9v_bg11"; 
		//求饶文字底图
	...
	{	
	 	...
	 	let tokenClip = ComponentManager.getCustomMovieClip("tokenAnimation",4,100); 
		tokenClip.width=229;
	...
	public useCallback(event: egret.Event): void {		
		...
		{	
			...
			if(this.prisonId!=10)
			{
				let progressBar:ProgressBar=ComponentManager.getProgressBar("progress_type1_red","progress_type1_bg2",410);
