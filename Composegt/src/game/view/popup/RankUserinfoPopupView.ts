...
class RankUserinfoPopupView  extends PopupView
{
	...
	{	
		...
		let nameTF:BaseTextField = ComponentManager.getTextField(UIData.name,TextFieldConst.FONTSIZE_TITLE_SMALL,0xeeeccd);
		// nameTF.border = true;
		nameTF.stroke = 1;
		...
		let uidTF:BaseTextField = ComponentManager.getTextField(uidStr,textSize,TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let allianceTxt:BaseTextField = ComponentManager.getTextField(allName,20,0xffdb59);
		...
		let alliancePoTxt:BaseTextField = ComponentManager.getTextField(poStr,20,0xffffff);
		...
		userContainer.y = 40;
		//衣服出左边框裁掉
		let maskX = bg0.x - userContainer.x;
		...
		let officebg =  BaseBitmap.create("prisonview_1");//  prisonview_1
		// officebg.width = 260;
		// officebg.height = 40;
		...
		let officeTF:BaseBitmapText|BaseTextField = ComponentManager.getBitmapText(Api.playerVoApi.getPlayerOfficeByLevel(UIData.level), "office_fnt");
		...
		let playerview_probg = BaseBitmap.create("wifeview_balckbg");
		...
		let maskBg = BaseBitmap.create("public_tc_bg06");
		...
		//称号背景
		let probg1 = BaseBitmap.create("public_tc_bg01");
		probg1.width = 537;
		probg1.height = 261;
		probg1.x = tarWidth/2 - probg1.width/2;
		probg1.y = 98 + 9;

		let probg2 = BaseBitmap.create("public_tc_bg03");
		...
		let playerRank_titleTxt = ComponentManager.getTextField(LanguageManager.getlocal("playerRank_title"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN);
		...
		titleNode.height += 40;
		let rect = new egret.Rectangle(0,0,this.viewBg.width,probg1.height- 80);
