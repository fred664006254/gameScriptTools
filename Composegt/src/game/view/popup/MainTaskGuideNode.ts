...
class MainTaskGuideNode  extends BaseLoadDisplayObjectContiner
{
	...
	protected init():void
	{
		...
		let rbg:BaseBitmap = BaseBitmap.create("maintask_guidebg");
		rbg.setPosition(0,-rbg.height/2);
		this.addChild(rbg);
		rbg.addTouchTap(this.bgHandler,this);
		this._rbg = rbg;

		let icon = BaseBitmap.create("maintask_guideicon");
		icon.x = rbg.x  ;
		...
		let closeBtn = ComponentManager.getButton("maintask_guideclose","",this.hide,this);
	...
	protected refreshUIInfo()
	{
		...
		let cfg = MainTaskGuidePosCfg.poscfg[taskCfg.openType];
		if(this._showUIName && this._showUIName != ""){
			cfg = MainTaskGuidePosCfg.poscfg[this._showUIName];
		}
		if (!this._taskId || !taskCfg||!cfg)
		{
			return ;
		}
		this.x = cfg.x;
	...
	public resetUIPos(uiName?:string)
	{
		...
		let cfg = MainTaskGuidePosCfg.poscfg[uiName];
		if (!this._taskId || !taskCfg||!cfg)
		{
			return ;
		}
	...
	private bgHandler()
	{
		if (Api.mainTaskVoApi.isCurTaskReach()){
			...
			for (var index = mindex-1; index >=0; index--) {
				if(pchildren[index]){
					let child = <BaseLoadDisplayObjectContiner>pchildren[index];
					child.hide();
				}
			}
		}
