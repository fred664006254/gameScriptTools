...
class WelfareViewRebateScrollItem extends ScrollListItem
{
	...
	protected initItem(index:number,data:any)
    {
		...
		if(index!=0)
		{
			let titleText:BaseTextField =  ComponentManager.getTextField(LanguageManager.getlocal("rebateTitle"+index),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BLACK);
			titleText.setPosition(temW/2-titleText.width/2*titleText.scaleX,line1.y);
			this.addChild(titleText); 
		}
		 
		let  desText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("rebateDes"+index),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BLACK);
