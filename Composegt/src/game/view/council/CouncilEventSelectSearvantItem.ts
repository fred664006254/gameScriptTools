...
class CouncilEventSelectSearvantItem extends ScrollListItem
{
	...
	protected initItem(index:number,info:any)
    {
        ...
        leftpic.width = 120;
        leftpic.height = 110;
        view.setLayoutPosition(LayoutConst.leftverticalCenter, leftpic, bg, [5,0]);
        view.addChild(leftpic);
        
        let temW:number = 94;
		let iconBgBt:BaseLoadBitmap = BaseLoadBitmap.create(data.qualityBoxImgPath,null, {callback : ()=>{
            view.setLayoutPosition(LayoutConst.leftverticalCenter, iconBgBt, bg, [20,0]);
            ...
            namebg.width = 170;
            // namebg.height = 155;
            view.setLayoutPosition(LayoutConst.lefttop, namebg, leftpic, [125,10]);
            view.addChild(namebg);

            let nameTxt = ComponentManager.getTextField(data.servantName, 22, TextFieldConst.COLOR_BROWN);
            view.setLayoutPosition(LayoutConst.horizontalCenterverticalCenter, nameTxt, namebg);
            view.addChild(nameTxt);

            let levelTxt = ComponentManager.getTextField(LanguageManager.getlocal('discussServantLevel',[data.level.toString()]), 20, TextFieldConst.COLOR_BROWN);
            view.setLayoutPosition(LayoutConst.lefttop, levelTxt, namebg, [10,namebg.height + 15]);
            ...
            let mainatrText = ComponentManager.getTextField(LanguageManager.getlocal(attr, [mainAtr]), 20, TextFieldConst.COLOR_BROWN);
            view.setLayoutPosition(LayoutConst.lefttop, mainatrText, levelTxt, [0,levelTxt.textHeight + 7]);
        ...
        let selectBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, 'allianceTaskSendBtnTxt', view.selectClick, view);
