...
class Soldier extends BaseDisplayObjectContainer 
{
	...
	public init(namePer:string):void
	{
		...
		this._deathFrames= [this._namePre+"_death1"];
		...
		{
			this._runFrames.push(this._namePre+"_walk"+i);
		}
		...
		{
			this._attackFrames.push(this._namePre+"_attack"+i);
		}
	}
	...
	{	
		...
		this._slowDownValue = this.area == SoldierArea.LEFT ? 1.5 : (this.endlong / 2) * 3;
