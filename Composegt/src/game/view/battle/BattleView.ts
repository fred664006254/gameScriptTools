...
class BattleView extends CommonView
{
	...
	//固定数据
	private _preLineNum:number = 6;
	private _maxLineNum:number = 5;
	private _unitNunberTab:number[] = [100000,500000,250000];
	...
	{
		let lv = Api.composemapVoApi.getMaxLv();
		return super.getResourceList().concat([
			`composebiglv_anim_${lv}`,"composebiglv_anim_npc"
		]);
	}
	...
	{	
		var nameTab = `composebiglv${Api.composemapVoApi.getMaxLv()}`;//this.getSoldierKinds();
		let totalLine:number = Math.floor((this._showCount[0]-1)/this._preLineNum);
		let tmpX = 190 - (this._maxLineNum - totalLine) * 20;
		...
		{	
			...
			//横着跑
			 ...
			 oneMan.endlong = v+1;
			 oneMan.lineNumber = h;
			 oneMan.x = tmpX - v * 50 - (h-1) * 10;
			 oneMan.y = GameConfig.stageHeigth/2 - (10 + Math.max(0,(GameConfig.stageHeigth - 1080))) + (h-1) * 60;
			 oneMan.standBy();
		}
	...
	protected clearRightSoldiers(){
		for (let index = 0; index < this._rightSoldiers.length; index++) {
			let rightSoldier = this._rightSoldiers[index];
			rightSoldier.dispose();
			rightSoldier = null;
		}
		this._rightSoldiers = [];
	}
	protected clearLeftSoldiers(){
		for (let index = 0; index < this._leftSoldiers.length; index++) {
			let leftSoldier = this._leftSoldiers[index];
			leftSoldier.dispose();
			leftSoldier = null;
		}
		this._leftSoldiers = [];
	...
	protected initRightSoldiers():void 
	{
		...
		let tmpX = 310 + (this._maxLineNum - totalLine) * 20;
		...
		{	
			...
			if (v == totalLine) { //最后一列
			 ...
			 oneMan.x = tmpX + v * 50 + (h-1) * 10;
			 oneMan.y = GameConfig.stageHeigth/2 - (20 + Math.max(0,(GameConfig.stageHeigth - 1080))) + (h-1) * 60;
			 oneMan.standBy();
			 //横跑
	...
	protected calculateSoldierNumber():void
	{
		...
		for (var i:number = 0; i<=1; i++) 
		{
			...
			this._showCount[i] = this._showCount[i] > maxNumber ? maxNumber : this._showCount[i];//this._showCount[i] = 0 + Math.floor(Math.random() * 30);
	...
	private getSoldierKinds():string[]
	{
		...
		for (var i:number = 1; i<= this._showCount[0]; i++) {
			...
			//test code
			level=1;
			nameTab.push( "composebiglv"+level );
		} 
		return nameTab;
	}
