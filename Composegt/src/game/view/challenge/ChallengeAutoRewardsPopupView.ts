class ChallengeAutoRewardsPopupView  extends PopupView
{
	...
	protected receiveData(data:{ret:boolean,data:any}):void
	{
		...
		{	
			...
			fightNum++;
			let smallId:number = ((cid-Api.challengeVoApi.smallNum)%41)%8;
			//结算
			if (smallId==0 || (i+1) == allKey.length)
			{
				// let bcid:number = Math.floor(cid / 41) + 1;
				// let mcid:number = Math.floor((cid-1) % 41 / 8 ) + 1;
				let bcid = Api.challengeVoApi.getBigChannelIdByCid2(cid);
				let mcid = Api.challengeVoApi.getMiddleChannelById(cid);
	...
	{	
		...
		let finalText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal(finialKey),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_WARN_YELLOW_NEW);
	...
	private getRewardInfoContainer(info:any):BaseDisplayObjectContainer
	{
		...
		let rewardBg:BaseBitmap = BaseBitmap.create("public_listbg3");
		rewardBg.width = 520;
		rewardBg.height = 190;
		...
		let targetText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("challengeRewardTitle",[String(info.bigId),targetStr,String(info.midId)]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
		...
		let nn = App.StringUtil.changeIntToText(info.curSd)

		let fightText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("challengeRewardFight",[String(info.fightCont),String(nn)]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
		...
		let awardText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("challengeReward"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN_NEW);
