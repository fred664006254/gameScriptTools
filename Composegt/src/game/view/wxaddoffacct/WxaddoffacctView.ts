class WxaddoffacctView extends PopupView {
    ...
    protected initView(): void {
        ...
        this.adImgs = [];
        for (let i = 0; i < 4; i++) {
            this.adImgs[i] = BaseLoadBitmap.create("wxaddoffacct_" + (i+1));
            this.adNode.addChild(this.adImgs[i]);
        }

        this.rewardNode = new BaseDisplayObjectContainer();
        this.addChild(this.rewardNode);
        let rewards = Config.CustomgiftCfg["2"].content;
    ...
    {        
        ...
        this.okButton.setText(
            ...
            + ((WxaddoffacctView.readOverTime === 0 || WxaddoffacctView.readOverTime + this.READTIME < GameData.serverTime) ? "" : ("(" + Math.floor(WxaddoffacctView.readOverTime + this.READTIME - GameData.serverTime) + "s)"))
        , false);
