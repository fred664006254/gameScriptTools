 ...
 */
class AcLotteryBoxRewardPopupView  extends PopupView
{
	...
	{	
        ...
        let cfg = Config.AcCfg.LotteryCfg = Config.AcCfg.getCfgByActivityIdAndCode(aid, code);
        ...
        let nameStr = GameData.formatRewardItem(cfg.getReward)[0].name;
        let tipTxt1 = ComponentManager.getTextField("",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_LIGHT_YELLOW);
        tipTxt1.text = LanguageManager.getlocal("acLotteryBoxDesc",[String(need),nameStr]);
        ...
        let baseX = this.viewBg.width / 2 - (108 * 4 + 15 * 3)/2;
