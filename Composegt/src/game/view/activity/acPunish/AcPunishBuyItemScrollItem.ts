...
class AcPunishBuyItemScrollItem extends ScrollListItem
{
	...
	public initItem(index:number,data:any):void
	{
		...
		if(Api.playerVoApi.getPlayerVipLevel() >0 && data.buyLimit && key == "2")
		{
			...
			let ssTF = ComponentManager.getTextField("(",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
		...
		if(data.holdLimit)
		{
			...
			if(typeof(data.buyLimit) == "number"){
				maxNum = data.holdLimit
			}else{
				if(data.holdLimit){
					// if(Api.switchVoApi.checkPunishVip()){
						maxNum = data.holdLimit[Api.playerVoApi.getPlayerVipLevel()];
						if(!maxNum)
						{
							maxNum = data.holdLimit[data.holdLimit.length-1];
						}
					// }else{
					// 	maxNum = data.buyLimit[0];
					// }
			...
			let holdLimit:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("rescueHoldLimit",[maxNum]),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
	...
	private chooseBtnClick(){
		...
		if(this._itemIndex != 2 && this._itemIndex != 3){
			...
			if(this._itemData.holdLimit)
			{
				...
				if(typeof(this._itemData.buyLimit) == "number"){
					maxNum = this._itemData.holdLimit
				}else{
					if(this._itemData.holdLimit){
						// if(Api.switchVoApi.checkPunishVip()){
							maxNum = this._itemData.holdLimit[Api.playerVoApi.getPlayerVipLevel()];
							if(!maxNum)
							{
								maxNum = this._itemData.holdLimit[this._itemData.holdLimit.length-1];
							}
					}
				
				}

				let itemInfo = Api.itemVoApi.getItemInfoVoById(Number(this._itemData.item));
				let itemNum = 0;
				if(itemInfo){
