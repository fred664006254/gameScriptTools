 ...
 * author yanyuling
 */
class AcMonopolyTaskScrollItem  extends ScrollListItem
{
    ...
    protected initItem(index:number,data:any)
    {
        ...
        let rewardstr = "201_1_"+this._uiData.dice;
        let rewardArr =  GameData.formatRewardItem(rewardstr);
		let scroStartY = namebg.y + namebg.height + 15;
        let tmpX = 10;
        for (var index = 0; index < rewardArr.length; index++) {
			let iconItem = GameData.getItemIcon(rewardArr[index],false,true);
    ...
    {         
        ...
        if(event){
			if(event.data&&event.data.ret){
                ...
                let adddicenum = data.adddicenum;
                let rewards = "201_1_"+ adddicenum;
