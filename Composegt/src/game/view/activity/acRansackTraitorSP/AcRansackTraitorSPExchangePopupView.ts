 ...
 * 奸臣皮肤兑换
 */
class AcRansackTraitorSPExchangePopupView extends PopupView {
	...
	protected initView(): void {
		...
		this._acvo = <AcRansackTraitorSPVo> Api.acVoApi.getActivityVoByAidAndCode(aid,code);
		// let cfg = <Config.AcCfg.RansackTraitorSPCfg>this._acvo.config;
		...
		let exchangeBtn = ComponentManager.getButton(ButtonConst.BTN_NORMAL_YELLOW,"ransackTraitorSP_btn3",this.exchangeBtnHandler,this,null,null,36);
		...
		let isexcge =  this._acvo.checkCanExchangeByIndex(this._skinIdIndex);//(this._acvo as AcRansackTraitorSPVo ).isExchangeEnable();
		if(isexcge){
		...
		for (let index = 0; index < auraIdList.length; index++) {
			...
			let bnode = new AcRansackTraitorSPScrollItem();
			bnode.init(element);
			bnode.x = 5 ;
			bnode.y = startY ;
 		...
 		for (let index2 = 0; index2 < addAbility.length; index2++) {
			let bnode = new AcRansackTraitorSPBookScrollItem();
