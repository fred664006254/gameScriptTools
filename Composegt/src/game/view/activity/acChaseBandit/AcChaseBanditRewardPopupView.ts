 ...
 */
class AcChaseBanditRewardPopupView extends PopupView {
	public constructor() {
		super();
	}
	private aid: any;
	...
	protected initView(): void {
		...
		let acCfg = <Config.AcCfg.ChaseBanditCfg>Config.AcCfg.getCfgByActivityIdAndCode(this.aid, this.code);
		let wifeSkinCfg = Config.WifeskinCfg.getWifeCfgById(acCfg.superp);
		if(wifeSkinCfg){
			this.showWifeSkinReward(wifeSkinCfg);
		}else{
			let titlecfg = Config.TitleCfg.getTitleCfgById(acCfg.superp);
			if(titlecfg){
	...
	private showTitleReward(titlecfg:Config.TitleItemCfg)
	{
		let bg = BaseLoadBitmap.create("acthrowarrowview_servantskinbg");
		bg.width = 544;
		bg.height = 618;
		bg.setPosition(this.viewBg.x + this.viewBg.width / 2 - bg.width / 2, 30);
		this.addChildToContainer(bg);
		
		let userContainer = Api.playerVoApi.getPlayerPortrait(Number(titlecfg.id),Api.playerVoApi.getPlayePicId());

		userContainer.x = bg.x + bg.width/2 - userContainer.width/2;
		userContainer.y =  bg.y + 5;
		//衣服出左边框裁掉
		let maskX = bg.x  - userContainer.x;
		let maskWidth = userContainer.width - maskX;
		...
		let topDesc = ComponentManager.getTextField(LanguageManager.getlocal("acChaseBanditRewardPopupViewTopDesc-" + this.code), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BROWN);
		topDesc.setPosition(bg.x + bg.width / 2 - topDesc.width / 2, bg.y - topDesc.height   - 3);
		this.addChildToContainer(topDesc);

		let skinnamebg = BaseBitmap.create("public_infobg2");
		skinnamebg.rotation = -90;
		skinnamebg.setPosition(bg.x+10, bg.y + 100);
		this.addChildToContainer(skinnamebg);

		let skinNameTxt = ComponentManager.getTextField(titlecfg.name, TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_WHITE);
		skinNameTxt.setPosition(bg.x+30, bg.y + 55);
		this.addChildToContainer(skinNameTxt);

	}

	private showWifeSkinReward(wifeSkinCfg:Config.WifeSkinItemCfg)
	{
