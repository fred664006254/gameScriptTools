/**
  ...
  */
class AcRechargeBoxSPView extends AcCommonView {
	...
	public initView()
	{
		...
		let vo = <AcRechargeBoxSPVo>Api.acVoApi.getActivityVoByAidAndCode(this.aid, this.code);
		...
		{
			let titletxt = BaseBitmap.create("acrechargeboxspview_title_txt"+this.code);
		...
		let actimeTF = ComponentManager.getTextField(LanguageManager.getlocal("acRechargeBoxSPTime",[vo.acTimeAndHour]),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_LIGHT_YELLOW);
		actimeTF.width = 580;
		...
		if(this.code == "8"){
			...
			for(let i = 0;i < boxList.length;i++)
			{
				...
				let gemNum =  ComponentManager.getTextField( rechargecfg ? "" + rechargecfg.gemCost : ""+boxCfg.thirdPayGem ,TextFieldConst.FONTSIZE_BUTTON_COMMON,TextFieldConst.COLOR_WARN_YELLOW);
				...
				let receiveNum = ComponentManager.getTextField(LanguageManager.getlocal("acRechargeBoxSPReceiveNum",[String(rechargeNum),String(boxCfg.limit)]),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_QUALITY_WHITE);
				
				let rechargebgStr = "acrechargeboxspview_textbg_1";
	...
	private initcode8box()
	{
		...
		let wifeid = "2211";
        let dragonScale = 1;
        let dragonY =  GameConfig.stageHeigth - 10;
        let imgScale = 0.8;
        let imgY =  GameConfig.stageHeigth - 660;
        let fY =  1136 - 267 + 80;
        ...
        {
            let droWifeIcon=App.DragonBonesUtil.getLoadDragonBones("wife_full3_"+wifeid);
        ...
        else
        {
            ...
            let wifeBM =  BaseLoadBitmap.create("wife_skin_"+wifeid);
            wifeBM.width = 640;
            wifeBM.height = 840;
		...
		// 
		let code8DescSp = BaseLoadBitmap.create("acrechargeboxspview_dectext_8" );
		...
		let boxList:Config.AcCfg.RechargeBoxSPItemCfg[] = cfg.getBoxListBaseData();
		boxList.sort((a:Config.AcCfg.RechargeBoxSPItemCfg,b:Config.AcCfg.RechargeBoxSPItemCfg)=>{
				return b.id - a.id;
			});
		
		let bg1 = BaseLoadBitmap.create("acrechargeboxspview_desk_1_8" );
		bg1.setPosition(292,GameConfig.stageHeigth- 518);
		this.addChild(bg1);

		let bg2 = BaseLoadBitmap.create("acrechargeboxspview_desk_2_8" );
		bg2.setPosition(216,GameConfig.stageHeigth- 321);
		this.addChild(bg2);

		let bg3 = BaseLoadBitmap.create("acrechargeboxspview_desk_3_8" );
		...
		for(let i = 0;i < boxList.length;i++)
		{
			let BoxStr = "acrechargeboxspview_box";
			let boxCfg = boxList[i];
			let offsetX = 0;
			let offsetY = 0;
			let rechargecfg =  Config.RechargeCfg.getRechargeItemCfgByKey(boxCfg.needGem);
			...
			let boxBg = BaseBitmap.create("acrechargeboxspview_whitebg_8");
			if(i == 0){
				boxBg.x = 325;
				boxBg.y = bg1.y;
			}else if(i == 1){
			...
			let box = BaseLoadBitmap.create(BoxStr);
			...
			let gemNum =  ComponentManager.getTextField( rechargecfg ? "" + rechargecfg.gemCost : ""+boxCfg.thirdPayGem ,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WARN_YELLOW);
			gemNum.setColor(TextFieldConst.COLOR_QUALITY_WHITE);
			gemNum.setPosition(gemBM.x + gemBM.width + 5,gemBM.y + gemBM.height / 2  - gemNum.height / 2);
			this.addChild(gemNum);
			...
			let rechargebg = BaseBitmap.create("acrechargeboxspview_textbg_1");
			...
			this.addChild(rechargebg);
			this.addChild(receiveNum);
			let boxInfo1:{ "boxBg":BaseBitmap;"needGem": string;"lightBM": BaseBitmap;"boxBM": BaseLoadBitmap;"rechargeTF": BaseTextField;"rechargebg":BaseBitmap;} = {"boxBg":boxBg,"needGem": boxCfg.needGem,"lightBM":light,"boxBM": box,"rechargeTF": receiveNum,"rechargebg":rechargebg};
	...
	private refreshView()
	{
		...
		{
			let baseBoxStr = "acrechargeboxspview_basebox";
			let BoxStr = "acrechargeboxspview_box";
			if(this.code == "8"){
				if(i == 0){
					BoxStr = BoxStr + "3_";
			...
			let boxCfg:Config.AcCfg.RechargeBoxSPItemCfg =  cfg.getBoxData(boxInfo.needGem);
	...
	private boxClick(event: egret.TouchEvent,needGem:any)
	{
		...
		if(this.code == "7"){
			...
			let cfg = <Config.AcCfg.RechargeBoxSPCfg>Config.AcCfg.getCfgByActivityIdAndCode(this.aid, this.code);
			let boxCfg = cfg.getBoxData(String(needGem));
	...
	protected getResourceList():string[]
	{
        ...
        return super.getResourceList().concat(resByCode).concat([
	...
	{
		return  this.code == "6" || this.code == "7" || this.code == "8" ? "" : super.getTitleStr();
	}

	protected getBgName():string
	{
		return  this.code == "8" ? "acrechargeboxspbg_8" : super.getTitleStr();
	}
