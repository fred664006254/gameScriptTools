/**
  ...
  */
class AcRyeHarvestActivityRewardShopScrollItem extends ScrollListItem {
	private _itemData: Config.AcCfg.RyeHarvestShopItemCfg = null;
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		if (this._itemData.discount) {
			...
			let tagTxt = ComponentManager.getTextField(LanguageManager.getlocal('acRyeHarvestActivityRewardShopScrollItemDiscount-' + this._aidAndCode.code, [(this._itemData.discount * 10).toString()]), 18, TextFieldConst.COLOR_WARN_YELLOW);
		...
		let limitTF = ComponentManager.getTextField(LanguageManager.getlocal("acRyeHarvestActivityRewardShopScrollItemLimit-" + this._aidAndCode.code, [String(limitNum)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WARN_GREEN2);
		...
		let shopBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "acRyeHarvestActivityRewardShopScrollItemshop-" + this._aidAndCode.code, () => {
			if ((!vo.isStart)) {
			...
			if (needPartVo.type == 1) {
				...
				if (Api.itemVoApi.getItemNumInfoVoById(needPartVo.id) < needPartVo.num) {
					App.CommonUtil.showTip(LanguageManager.getlocal("acRyeHarvestActivityRewardShopScrollItemNotEnoughItem-" + this._aidAndCode.code));
					return;
				}
			}
