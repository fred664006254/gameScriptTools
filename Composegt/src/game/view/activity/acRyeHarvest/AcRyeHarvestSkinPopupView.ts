 ...
 * author 钱竣
 */
class AcRyeHarvestSkinPopupView extends PopupView {
	public constructor() {
		super();
	}
	...
	protected initView(): void {
		...
		let isWifeskin = obj[view.code] == `wife`;
		if(isWifeskin){
			let skinCfg = Config.WifeskinCfg.getWifeCfgById(view.cfg.getSkin(view.code));
			...
			let topDesc = ComponentManager.getTextField(LanguageManager.getlocal(`acRyeHarvestSKinTip1-${view.getUiCode()}`,[view.cfg.bigPrize.needValue.toString(), Config.WifeskinCfg.getWifeCfgById(view.cfg.getSkin(view.code)).name]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			...
			let addValues = Api.wifeSkinVoApi.getWifeSkinProAdd(view.cfg.getSkin(view.code), true);
			//getWifeSkinProAdd
			let txt = [
				{
					txtKey:"skinLvuptxt2",
			...
			let view = this;
			let acCfg = this.cfg;
			let skinid = acCfg.getSkin(view.code);
			...
			let bg = BaseLoadBitmap.create(`luckdrawshowbg-${view.getUiCode()}`);
			bg.width = 544;
			bg.height = 400;
			bg.setPosition(this.viewBg.x + this.viewBg.width / 2 - bg.width / 2, 0);
			this.addChildToContainer(bg);
			...
			let topDesc = ComponentManager.getTextField(LanguageManager.getlocal(`acRyeHarvestSKinTip1-` + this.getUiCode(), [view.cfg.bigPrize.needValue.toString(), view.cfg.getSkinName(view.code)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			topDesc.setPosition(topbg.x + topbg.width / 2 - topDesc.width / 2, topbg.y + topbg.height / 2 - topDesc.height / 2);
			this.addChildToContainer(topDesc);
