...
class AcRedLotusWarriorReportView extends BaseView
{
	...
	protected initView():void
	{
		...
		//名字
		let nameTf:BaseTextField = ComponentManager.getTextField( LanguageManager.getlocal("acRedLotusWarriorReportView-"+ this.param.data.code),TextFieldConst.FONTSIZE_TITLE_SMALL);
		nameTf.textColor = TextFieldConst.COLOR_LIGHT_YELLOW;
		nameTf.setPosition(lookBg.width/2 - nameTf.width/2, lookBg.y + 40);
		this.addChild(nameTf);

		let descTf:BaseTextField = ComponentManager.getTextField( LanguageManager.getlocal("acRedLotusWarriorReportDesc-"+ this.param.data.code),TextFieldConst.FONTSIZE_CONTENT_SMALL);
