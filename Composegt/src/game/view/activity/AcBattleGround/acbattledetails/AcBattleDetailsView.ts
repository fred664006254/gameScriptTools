...
class AcBattleDetailsView extends CommonView {
	...
	private freshText():void{
        ...
        let str = this.getDefaultCn("acBattleRoundCDTxt"+view._period);//`acBattleRoundCDTxt${view._period}-${code}`;
	...
	{ 	 
		...
		// 存活人数
		let gangsRankTxt = ComponentManager.getTextField(LanguageManager.getlocal("acsurvival",[this.alivemn+"",this.mn+""]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BLACK);
		...
		// 帮会排名
		let acsurvivaTxt = ComponentManager.getTextField(LanguageManager.getlocal("acgangsRank",[this.getallindex()+"",this.getMaxLength()+""]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BLACK);
		...
		this.freshText(); 
		let logbtn = ComponentManager.getButton(this.getDefaultCn("battlegroundlog"),'', ()=>{
            //打开详情界面
		...
		let tmpRect =  new egret.Rectangle(0,0,626,bg.height-100);
		...
		if(Api.switchVoApi.checkOpenAtkracegChangegpoint()){
			let scoreTip = ComponentManager.getTextField(LanguageManager.getlocal(this.getDefaultCn("acBattleGroundTip12")), 20, TextFieldConst.COLOR_LIGHT_YELLOW);
