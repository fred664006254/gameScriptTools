...
class AcBattleGroundBuyPopupView extends PopupView
{
	...
	{	
		let tmpattr:any = this.vo.getMyFightInfo().tmpattr;
	...
	protected initView():void
	{
		...
		let infoText3:BaseTextField = ComponentManager.getTextField(skillAdd.toString() + "%",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WARN_GREEN);
		...
		let progressBar:ProgressBar=ComponentManager.getProgressBar("progress_type1_red","progress_type1_bg",410);
		...
		if (myAtkInfo.fightnum == 0) {
			...
			let tmpattr:any = this.vo.getMyFightInfo().tmpattr.list;
	...
	private getBuyTypeContainer(info:any, idx:number , type:number):BaseDisplayObjectContainer
	{
		...
		let descText:BaseTextField = ComponentManager.getTextField(desc,TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
		descText.setPosition(itemName.x, itemName.y + itemName.height + 12);
		bgContainer.addChild(descText);

		let expendText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("adultChooseCost")+":",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
		...
		let expend:BaseTextField = ComponentManager.getTextField(cost.toString(),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
	...
	private buyClick(idx:number):void
	{
		...
		if (attrInfo.costGem) {
			...
			let hasNum:number =this.vo.getMyInfo().morale;
			if (needItem > hasNum) {
