...
class AcWipeBossAllianceInfoScrollItem extends ScrollListItem
{
    ...
    {	
		...
		let name = data.type == 1 ? LanguageManager.getlocal('acwipeBossAllNpcInfo', [cfg.npcName, (data.curBlood/bpsshp*100).toFixed(2)]) : cfg.npcName;
		...
		//发现者
		let findTxt:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal('acwipeBossAllFindPlayer', [data.findname]),18,TextFieldConst.COLOR_BROWN);
		...
		if(data.type == 1){
			...
			let killTxt:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal(cfg.type == 1 ? 'acwipeBossAllKillPlayer' : 'acwipeBossAllOpenPlayer', [data.killername]),18,TextFieldConst.COLOR_BROWN);
