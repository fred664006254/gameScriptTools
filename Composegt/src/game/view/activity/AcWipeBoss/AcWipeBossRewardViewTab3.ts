...
class AcWipeBossRewardViewTab3 extends CommonViewTab
{
	...
	protected initView():void
	{
		...
		for (let index = 0; index < rankList.length; index++) {
			...
			let rank = rItem.rank
			let txt = ComponentManager.getTextField(rItem.type == 1 ? rItem.npcName : `acwipeBossKillBox${key - 7}`,22,TextFieldConst.COLOR_BROWN);
			txt.x =  GameConfig.stageWidth/2 - txt.width/2;
			txt.y = winbg.y + winbg.height/2 - txt.height/2;
			this._nodeContainer.addChild(txt);

			let alltxt = ComponentManager.getTextField(LanguageManager.getlocal('acwipeBossKillBasicReward'),22,TextFieldConst.COLOR_BROWN);
			...
			let scoreImg = BaseBitmap.create("acwipeboss_scoreicon2");
			...
			let scoreImg2 = BaseBitmap.create("acwipeboss_scoreicon1");
			...
			let ordtxt = ComponentManager.getTextField(LanguageManager.getlocal('acwipeBossKillPoolReward'),22,TextFieldConst.COLOR_BROWN);
