 ...
 * desc:围剿鳌拜探索结果界面
*/
class AcWipeBossSearchResultView extends CommonView
{
	...
	public initView():void{	
		...
		//按钮
		let cancelBtn = ComponentManager.getButton(ButtonConst.BTN_NORMAL_BLUE, 'acwipeBossSearchCancel', view.hide, view);
		App.DisplayUtil.setLayoutPosition(LayoutConst.leftbottom, cancelBtn, descbg, [100,55]);
		infoGroup.addChild(cancelBtn);

		let confirmBtn = ComponentManager.getButton(ButtonConst.BTN_NORMAL_YELLOW, resultInfo.type == 1 ? 'sysConfirm' : 'acwipeBossSearchOpen', view.confirmClick, view);
		...
		if(resultInfo.type == 2){
			...
			let keyTxt = ComponentManager.getTextField(`${LanguageManager.getlocal(`acwipeBossKillKey${boxId}`)}${keynum}/1`, 22, TextFieldConst.COLOR_LIGHT_YELLOW);
