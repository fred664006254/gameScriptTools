class AcLimitedRewardDetailScrollItem extends ScrollListItem
{
	...
	protected initItem(index:number,data:any)
    {
		...
		let bgtitle:BaseBitmap = BaseBitmap.create("public_up3");
		bgtitle.width = 510;
		bgtitle.height=40;
		bgtitle.x =5;
		bgtitle.y =5;
		...
		let rewardGradeTF:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("rewardGradeNum",[this._limitedRewardInfoVo.id.toString()]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_BROWN);
		rewardGradeTF.x = 20;
		rewardGradeTF.y = 15;
		...
		let conditonTF:BaseTextField = ComponentManager.getTextField("",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
		...
		{	
			...
			for(let i = 0;i<rewardList.length;i++)
			{
				...
				icon.y = rewardGradeTF.y + rewardGradeTF.height +5 + 13;
