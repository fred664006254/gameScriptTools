//
class AcCrossServerWipeBossRewardViewTab1 extends CommonViewTab
{
	...
	protected initView():void
	{
		...
		for (let index = 0; index < rankList.length; index++) {
			...
			if(key == 1){
				...
				let firstData = this.api.getRankFirstPlayer();

				if(firstData){
					let playerHead:BaseDisplayObjectContainer = Api.playerVoApi.getPlayerCircleHead(firstData.pic,firstData.ptitle);
					playerHead.x = 50;
					...
					let playerScore = ComponentManager.getTextField(LanguageManager.getlocal("acBattleGroundDetailsViewScore",[String(firstData.v)]),20,TextFieldConst.COLOR_BROWN);
