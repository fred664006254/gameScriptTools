 ...
 * author qianjun
 */
class AcCrossServerWipeBossRankView extends PopupView
{
	// 滑动列表
	...
	{		
		...
		let name = "";
		let rankV = view.api.getMyRank();
		let score = view.api.getMyScore();
		...
		let myNameTxt = ComponentManager.getTextField(LanguageManager.getlocal('accrossserverwipeBossPlayerName', [name]), 22, TextFieldConst.COLOR_WARN_GREEN); 
		...
		let myRankStr =  ComponentManager.getTextField(LanguageManager.getlocal("accrossserverwipeBossPlayerRank",[str.toString()]), 22, TextFieldConst.COLOR_WARN_GREEN); 
		...
		let scoreStr =  ComponentManager.getTextField(LanguageManager.getlocal("accrossserverwipeBossPlayerScore",[score.toString()]), 22, TextFieldConst.COLOR_WARN_GREEN); 
	...
	protected clickTabbarHandler(params:any){
		...
		let rankV = 0;
		let score = 0;
		let scorename = 'accrossserverwipeBossPlayerScore';
		let name = "";
		switch(params.index){
			case 0:
				name = this.api.getMyRankName();
