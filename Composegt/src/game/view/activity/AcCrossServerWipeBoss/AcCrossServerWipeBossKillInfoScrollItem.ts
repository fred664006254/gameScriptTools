 ...
 */
class AcCrossServerWipeBossKillInfoScrollItem  extends ScrollListItem
{
    public constructor()
    {
        super();
    ...
    protected initItem(index:number,data:any,itemparam)
    {
        ...
        let str = cfg.type == 1 ? LanguageManager.getlocal('accrossserverwipeBossAllKillInfoDesc', [servantcfg.name,cfg.npcName]) : (LanguageManager.getlocal('accrossserverwipeBossAllOpenInfoDesc', [cfg.npcName]));
        ...
        let rewardstr = cfg.type == 1 ? (LanguageManager.getlocal('accrossserverwipeBossAllKillReward', [cfg.killScore.toString(),reward_str])) : (LanguageManager.getlocal('accrossserverwipeBossAllOpenReward',[reward_str]));
