...
class AcDragonBoatDayRankView extends PopupView
{
	...
	private _tcbg2 : BaseBitmap =null;
	private _rank_biao:BaseBitmap = null;
	...
	{		
        ...
        tabbarGroup.x = 35;
        tabbarGroup.y = 15; 
		tabbarGroup.setSpace(10);
		this.addChildToContainer(tabbarGroup);
		
		let contentBg = BaseBitmap.create("public_tc_bg01");
		contentBg.width = 540;
        contentBg.height =660;
        contentBg.x = 35;
        contentBg.y = tabbarGroup.y+tabbarGroup.height;
		this.addChildToContainer(contentBg);

		let bottomBg = BaseBitmap.create("public_tc_bg03");//public_tc_bg01
		bottomBg.width = 510; 
		bottomBg.height = 100; 
		bottomBg.x =50;// this.viewBg.x + this.viewBg.width/2 - bottomBg.width/2;
		bottomBg.y = 610 ;//+ contentBg.height + 5;
		...
		let myRankStr = LanguageManager.getlocal("acRank_myrank1",[rankV]);
		...
		this._acCDTxt.x = bottomBg.x + 250;
		...
		this._sendDesc = ComponentManager.getTextField(LanguageManager.getlocal("acRanktip"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BROWN);
		...
		rect.setTo(0,0,508,contentBg.height - 130);
		this._scrollList = ComponentManager.getScrollList(AcMayDayRankRewardScrollItem,dataList,rect);
		this.addChildToContainer(this._scrollList);
		this._scrollList.setPosition(contentBg.x + 15 ,contentBg.y + 10);
		this._scrollList.setEmptyTip(LanguageManager.getlocal("acPunishNoData") );
		
		bottomBg.height = this._sendDesc.textHeight + this._sendDesc.y + 28 - bottomBg.y;
		//个人榜单	
        ...
        tcbg2.width = bg2.width-30;
        tcbg2.height = 530;
        tcbg2.x = bg2.x+15;
        tcbg2.y = contentBg.y+10;
		this._tcbg2 =tcbg2;
		this._tcbg2.visible = false;
        this.addChildToContainer(tcbg2);

		let rank_biao= BaseBitmap.create("rank_biao");
        rank_biao.width = 480; 
        rank_biao.x = 55;
        rank_biao.y = 83;
		this._rank_biao = rank_biao;
		this._rank_biao.visible =false;
        this.addChildToContainer(rank_biao);
        ...
        this._titleTxt1.x = 100;//contentBg.x + 40;
        this._titleTxt1.y = contentBg.y + 11+15;//8
        ...
        this._nickNameTF.x = bottomBg.x +30; //66;
        ...
        this._sendTip = ComponentManager.getTextField(LanguageManager.getlocal("DragonBoatDayTip_"+this.param.data.code,[this.cfg.rankNeed.toString()]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BROWN);
		this._sendTip.x = this._nickNameTF.x + 214-20;
		...
		rect2.setTo(0,0,508,contentBg.height - 60-110);
		...
		this._scrollList2.setPosition(contentBg.x + 15 ,contentBg.y + 50);
    ...
    protected refreshRankList()
    {
        this._acCDTxt.visible = this._sendDesc.visible = this._scrollList.visible = this._curTabIdx == 0;
     	this._rank_biao.visible = this._tcbg2.visible =  this._topBg.visible = this._titleTxt3.visible = this._titleTxt1.visible = this._titleTF.visible = this._nickNameTF.visible = this._sendTip.visible = this._scoreTF.visible = this._scrollList2.visible = !this._scrollList.visible;
