 ...
 * 奸臣皮肤兑换
 */
class AcStargazerView extends AcCommonView {
...
// --------------
	...
	private get cfg() : Config.AcCfg.StargazerCfg{
		if(!this._cfg){
	...
	protected initView(): void {
		...
		this._acvo = <AcStargazerVo> Api.acVoApi.getActivityVoByAidAndCode(this.aid,this.code);

		let titleFont = BaseBitmap.create(this.getDefaultRes("acstargazer_titletxt"));
	...
	private initCircle():void{
		...
		for(let i = 0;i < 5; i ++){
			if(i == 4){
				...
				let shopItem = this.acVo.config.exchangeShop[0];
				...
				let allCount = shopItem["proofNum"];
				let itemCount = this._acvo.getItemNumByIndex(String(shopItem.itemID));
	...
	private refreshUIInfos()
	{
		...
		let idfall = (this.acVo as AcStargazerVo ).isExchangeEnable();
	...
	private searchHandler(param:any)
	{
		...
		if(this.acVo["cannum"] < param){
			let rewardStr = LanguageManager.getlocal("acStargazer_notEnough",[this.acVo["cannum"]]);
