 ...
 * 奸臣皮肤兑换
 */
class AcStargazerExchangePopupView extends PopupView {
	...
	protected initView(): void {
		...
		let isexcge =  this._acvo.checkCanExchangeByIndex(this._skinIdIndex);
		if(isexcge){
			App.CommonUtil.addIconToBDOC(this._exchangeBtn);
		}else{
			App.CommonUtil.removeIconFromBDOC(this._exchangeBtn);
		}
	 	let skinTipTxt = ComponentManager.getTextField(LanguageManager.getlocal("servantSkinEffect"+exchangeShopItem.skinID),18,TextFieldConst.COLOR_BROWN);
	...
	{ 
		...
		let sid = Config.ServantskinCfg.getServantSkinItemById(exchangeShopItem.skinID).servantId;
