...
class DinnerOpenedView extends CommonView 
{
	...
	protected initView():void
	{
		...
		openWord.setPosition(GameConfig.stageWidth/2  - openWord.width/2, 245.5);
		...
		let openSuccess:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("openSuccessDesc",[dinnerType]),TextFieldConst.FONTSIZE_TITLE_SMALL,TextFieldConst.COLOR_WHITE);
		...
		let playerId:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("playerId",[Api.playerVoApi.getPlayerID().toString()]),TextFieldConst.FONTSIZE_TITLE_SMALL,TextFieldConst.COLOR_WARN_GREEN2);
		playerId.setPosition(GameConfig.stageWidth/2 - playerId.width/2 , descBg.y-52+descBg.height);
		this.addChild(playerId);

		let shareDinner:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("shareDinnerDesc"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_QUALITY_ORANGE);
