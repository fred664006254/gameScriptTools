...
class AdultPlayerInfoScrollItem extends ScrollListItem
{
	...
	public initItem(index:number,bData:any):void
	{
		...
		if(bData.type == 'choosevisit' && bData.laifang){
			...
			let havevisitTxt = ComponentManager.getTextField(LanguageManager.getlocal('adulthavevisited'),18, TextFieldConst.COLOR_BROWN);
		...
		let leadStr = LanguageManager.getlocal("mainui_shili") +  bData.power;
		...
		let attrStr = LanguageManager.getlocal("mainui_officer") + Api.playerVoApi.getPlayerOfficeByLevel(bData.level);
		...
		let gstr = bData.mygname == "" ? LanguageManager.getlocal('nothing') : bData.mygname ;
		let conStr = LanguageManager.getlocal("acRank_myAlliancenick",[gstr])  ;
		...
		if(bData.type == 'choosevisit'){
			let adultbfzhong = BaseLoadBitmap.create('adultbfzhong');
			...
			let friendTxt = ComponentManager.getTextField(LanguageManager.getlocal('adultfriendnum', [bData.friend]),18, TextFieldConst.COLOR_BROWN);
