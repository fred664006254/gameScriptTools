...
class CustomMovieClip extends BaseBitmap
{
    ...
    protected imageNamePre:string=""; //资源前缀
    ...
    public set frameImages(textureNames:string[]){
        ...
        this._groupName = ResourceManager.loadResources(textureNames,[],()=>{
            if(!this.isPlaying)
            {
                let texture=ResourceManager.getRes(this._frameImages[this._playingIndex]);
                // console.log(egret.getQualifiedClassName(texture),this._playingIndex,this._frameImages[this._playingIndex]);
    ...
    public setPosition(pointOrX:number|{x:number,y:number},yOrNull?:number):void 
    {
    ...
    public setFramesByNamePre(imageNamePre:string,frameCount?:number):void
    {   
        if(!imageNamePre)
        {
            return;
        ...
        let spaceStr=ResourceManager.hasRes(imageNamePre+"_1")?"_":"";
        if(!frameCount)
        {
            while(ResourceManager.hasRes(imageNamePre+spaceStr+(frameCount+1)))
            {
                frameCount++;
                resArr.push(imageNamePre+spaceStr+frameCount);
            }
        }
        else
        {
            ...
            {
                resArr.push(imageNamePre+spaceStr+i);
            }
        }
        if(ResourceManager.getAtlasByResName(imageNamePre+spaceStr+"1")==ResourceManager.getAtlasByResName(this.imageNamePre+spaceStr+"1"))
        {
