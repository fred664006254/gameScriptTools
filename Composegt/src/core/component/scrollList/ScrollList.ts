...
class ScrollList extends ScrollView
{
	...
	private changeHandler(e:egret.Event):void
	{
		...
		if(this.checkWailToButtom()&&this._curPageIndex<=this._maxPage)
		{
			this.initNextPage();
		}
	}
	...
	public getScrollListHeightArr(index:number):egret.Rectangle{
	 ...
	 * 检测距离底部一个item的高度
	 */
	public checkWailToButtom():boolean
	{
		if(this._scrollListHeightArr.length>0)
		{
			let lastIndex:number=this._scrollListHeightArr.length-1;	
			let maxY:number = this._scrollListHeightArr[lastIndex].y-this._scrollRect.height;
	...
	/** 播放飞入动画 */
	public playEnterFlyAni() {
		if (!Api.switchVoApi.checkOpenListFly() || Api.rookieVoApi.isInGuiding) {
			return;
		}
		let lastTw : egret.Tween= null;
		let count = 0;
		for (var index = 0; index < this._scrollListItemArr.length; index++) {
			var scrollListItem = this._scrollListItemArr[index];
			let top = this.scrollTop;
			if (scrollListItem.y + scrollListItem.height > top && scrollListItem.y < top + this.height) {
				let toX = scrollListItem.x;				
				scrollListItem.x = scrollListItem.x-50;
				// 大至列数
				let colCount = Math.max(1, Math.floor(this.width / scrollListItem.width));
				let tw = egret.Tween.get(scrollListItem);
