...
hexieplat='wd,37wd,h5ly,jxh5,wdly,wanba,wx,wxapp,'
...
if [[ $childFile == '' ]];then
    ...
    #  if [ "$localpath" = "local" ] || [ "$platname" = "h5ly" ] || [ "$platname" = "testh5ly" ];then
    #     echo '切换资源库ing...'
...
replaceResource()
{
    ...
    if [[ $childFile == '' ]];then
        ...
        if [ -f "$filepath2/$zhengshiname/gt_$platname/index.php" ]; then
            ...
            if [ ! -d /Library/WebServer/Documents/publish/hs ]
            then
                sudo mkdir -p -m 777 /Library/WebServer/Documents/publish/hs
                sudo cp -rf $filepath/template/index.php /Library/WebServer/Documents/publish/hs
            else
                cp -rf $filepath/template/index.php /Library/WebServer/Documents/publish/hs
            fi

            if [ "$localpath" = "local" ]
            then
                curl -d "publishplat=$platname&local=$localpath" 127.0.0.1/publish/hs/index.php > $filepath2/$zhengshiname/gt_$platname/index.html
            else
                curl -d "publishplat=$platname" 127.0.0.1/publish/hs/index.php > $filepath2/$zhengshiname/gt_$platname/index.html
            fi
        ...
        then
            ...
            echo "multifileEnd" $multifileEnd
        fi
...
# 处理完毕
    ...
    then
        ...
        if [[ ! -d $projectpath/client/rsdk-framework-js/ ]]; then
            git clone http://ke.chen%40rjoy.com:chenke123@g.rjoy.com/plattech-rsdkh5/rsdk-framework-js/ $projectpath/client/rsdk-framework-js/
        else
...
echo '继续检测' $zhengshiname 
if [[ $zhengshiname == "th" ]];then
    echo '发布en'
    oldplatname=$platname
    platname=${platname//th/en}
    zhengshiname=${zhengshiname//th/en}
    mv $filepath/bin-release/web/gt_$oldplatname $filepath/bin-release/web/gt_$platname
    replaceResource $platname
fi
