...
(function (LoginManager) {
    ...
    function requestUseLogin() {
        ...
        if (GameConfig.isLoaded) {
            ...
            var port = ServerCfg.selectServer.port_server;
    ...
    function userLoginRequestHandler(event, isReLogin) {
        //致劲老用户导入
    ...
    function randName() {
        ...
        if (userNameCount >= 5) {
            ...
            var pic = App.MathUtil.getRandom(1, 6);
    ...
    function checkName(e) {
        var rData = e.data.data.data;
        var ret = e.data.data.ret;
        if (rData.nameflag == 0) {
            console.log("randName success and creater");
            App.MessageHelper.removeNetMessage(NetRequestConst.REQUEST_USER_CHECKNAME, checkName, LoginManager);
            App.MessageHelper.addNetMessage(NetRequestConst.REQUEST_USER_CREATEUSER, startGuide, LoginManager);
            var pic = App.MathUtil.getRandom(1, 7);
    ...
    LoginManager.isGuideNamed = false;
    function startGuide(e) {
        if (e) {
            LoginManager.isGuideNamed = true;
        }
        if (LoginManager.isGuideNamed == true && App.LoginResLoader.isLoginResLoaded) {
    ...
    LoginManager.startGuide = startGuide;
    ...
    function checkAndCreateScene() {
        console.log("checkAndCreateScene" + App.LoginResLoader.isLoginResLoaded + LoginManager.isLoginSuccess + isNewGuideComplete + LoginManager.isCreateScene);
