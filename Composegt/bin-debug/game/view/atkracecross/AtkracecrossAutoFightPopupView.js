...
var AtkracecrossAutoFightPopupView = (function (_super) {
    __extends(AtkracecrossAutoFightPopupView, _super);
    function AtkracecrossAutoFightPopupView() {
    ...
    /**生成新标头 */
    AtkracecrossAutoFightPopupView.prototype.isHaveTitle = function () {
        return true;
    };
    AtkracecrossAutoFightPopupView.prototype.initView = function () {
        ...
        var moraleText = ComponentManager.getTextField(Api.atkracecrossVoApi.getMyInfo().morale.toString(), 20, TextFieldConst.COLOR_WARN_YELLOW_NEW);
        ...
        var infoText2 = ComponentManager.getTextField(atkAdd.toString() + "%", TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BROWN_NEW);
        infoText2.setPosition(infoDesc2.x + infoDesc2.width, infoDesc2.y);
        this.addChildToContainer(infoText2);
        var infoText3 = ComponentManager.getTextField(skillAdd.toString() + "%", TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BROWN_NEW);
    ...
    AtkracecrossAutoFightPopupView.prototype.attAllClickHandler = function () {
        this.request(NetRequestConst.REQUEST_ATKRACECROSS_BATCHFIGHT, { pos: this._dotIdx, activeId: "crossServerAtkRace-" + Api.acVoApi.getActivityVoByAidAndCode("crossServerAtkRace").code });
    };
    //请求回调
    AtkracecrossAutoFightPopupView.prototype.receiveData = function (data) {
        var rData = data.data;
        if (data.ret == false) {
            App.MessageHelper.dispatchEvent(MessageConst.MESSAGE_ATKRACE_BATCH_FIGHT_RET_REFRESH);
            return;
        }
        if (rData.cmd == NetRequestConst.REQUEST_ATKRACECROSS_BATCHFIGHT) {
    ...
    AtkracecrossAutoFightPopupView.prototype.dotHandle = function (obj, idx) {
        if (idx != this._dotIdx) {
            if (this._dotIdx) {
    ...
    AtkracecrossAutoFightPopupView.prototype.getResourceList = function () {
        return _super.prototype.getResourceList.call(this).concat([
            "public_select", "public_select_down",
        ]);
    };
    // 计算背景高度时使用，在container高度的基础上添加该高度
    AtkracecrossAutoFightPopupView.prototype.getBgExtraHeight = function () {
        return 25;
    };
    AtkracecrossAutoFightPopupView.prototype.getTitleStr = function () {
        return "atkraceAutoFightPopupViewTitle";
    };
    AtkracecrossAutoFightPopupView.prototype.dispose = function () {
    ...
    return AtkracecrossAutoFightPopupView;
}(PopupView));
__reflect(AtkracecrossAutoFightPopupView.prototype, "AtkracecrossAutoFightPopupView");
