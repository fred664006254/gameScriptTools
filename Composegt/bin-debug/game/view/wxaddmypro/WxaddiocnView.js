...
var WxaddiconView = (function (_super) {
    __extends(WxaddiconView, _super);
    function WxaddiconView() {
    ...
    WxaddiconView.prototype.initView = function () {
        ...
        for (var i_1 = 0; i_1 < 4; i_1++) {
            this.adImgs[i_1] = BaseLoadBitmap.create("wxaddicon_" + (i_1 + 1));
    ...
    WxaddiconView.prototype.leftButtonClick = function () {
        this.curIndex = Math.max(0, this.curIndex - 1);
        this.autoNextPoint = -1;
        this.refreshIndex();
    };
    WxaddiconView.prototype.rightButtonClick = function () {
        this.curIndex = Math.min(3, this.curIndex + 1);
        this.autoNextPoint = -1;
        this.refreshIndex();
    };
    WxaddiconView.prototype.okButtonClick = function () {
        // console.log("okButtonClick");
        // if (WxaddiconView.readOverTime + this.READTIME > GameData.serverTime) {
        ...
        //     App.CommonUtil.showTip(LanguageManager.getlocal("wxaddicon_please_open_from_icon"));
        //     return;
        // } else if (Api.otherInfoVoApi.wxaddmyproStatus() == 1) {
        //     this.request(NetRequestConst.REQUEST_OTHERINFO_GETWXADDFLOAT,{});
        // }
        // if(!PlatformManager.isFromWxIcon)
        // {
        // 	App.CommonUtil.showTip(LanguageManager.getlocal("wxaddicon_please_open_from_icon"));
        ...
        else if (Api.otherInfoVoApi.wxiconStatus() == 1) {
            this.request(NetRequestConst.REQUEST_OTHERINFO_GETWXADDFLOAT, {});
            this.okButton.setEnable(false);
        }
        // this.request(NetRequestConst.REQUEST_OTHERINFO_GETWXADDFLOAT,{});
    };
    WxaddiconView.prototype.receiveData = function (data) {
        if (data.ret) {
    ...
    WxaddiconView.prototype.refreshIndex = function () {
        console.log("refreshIndex", this.curIndex);
        if (this.curIndex >= 3 && WxaddiconView.readOverTime === 0) {
            // 标记一下读完的时间
            WxaddiconView.readOverTime = GameData.serverTime;
        }
        for (var i = 0; i < 4; i++) {
    ...
    WxaddiconView.prototype.tick = function () {
        // if (WxaddiconView.readOverTime > 0 && WxaddiconView.readOverTime + this.READTIME < GameData.serverTime) {
        ...
        //     + ((WxaddiconView.readOverTime === 0 || WxaddiconView.readOverTime + this.READTIME < GameData.serverTime) ? "" : ("(" + Math.floor(WxaddiconView.readOverTime + this.READTIME - GameData.serverTime) + "s)"))
        // , false);
    ...
    WxaddiconView.prototype.showBg = function () {
    ...
    WxaddiconView.prototype.resetBgSize = function () {
    ...
    WxaddiconView.prototype.getShowHeight = function () {
        return 775;
    };
    WxaddiconView.prototype.getCloseBtnName = function () {
        return "load_closebtn";
    };
    WxaddiconView.prototype.getBgName = function () {
        return "load_bg";
    };
    WxaddiconView.prototype.getSheepType = function () {
        return 1;
    };
    WxaddiconView.prototype.dispose = function () {
        this.okButton = null;
        this.adNode = null;
        this.adImgs = null;
        this.bg = null;
    ...
    WxaddiconView.readOverTime = 0;
    return WxaddiconView;
}(PopupView));
__reflect(WxaddiconView.prototype, "WxaddiconView");
