...
var BaseBattleView = (function (_super) {
    ...
    BaseBattleView.prototype.getResourceList = function () {
        return _super.prototype.getResourceList.call(this).concat([
            ...
            "enermycrit_light", "critenermyflash1", "critenermyflash2", "critenermyflash3",
            "critenermyspeed1", "critenermyspeed2", "critenermyspeed3", "critenermyspeed4",
            "critenermyspeed5", "mycrit_circle", "mycrit_light", "critmyflash1", "critmyflash2",
            "critmyflash3", "critmyspeed1", "critmyspeed2", "critmyspeed3", "critmyspeed4", "critmyspeed5",
            "progress_type1_red", "progress_type1_bg2",
    ...
    BaseBattleView.prototype.setTopProgress = function (value, maxValue, progressWidth, type) {
        ...
        if (this._topProgress == null) {
            this._topProgress = ComponentManager.getProgressBar("progress_blood", "progress_bloodbg", progressWidth ? progressWidth : GameConfig.stageWidth);
    ...
    BaseBattleView.prototype.setBottomProgress = function (value, maxValue) {
        ...
        if (this._bottomProgress == null) {
            this._bottomProgress = ComponentManager.getProgressBar("progress_blood", "progress_bloodbg", GameConfig.stageWidth);
    ...
    BaseBattleView.prototype.setUpHero = function (picName, info, type) {
        ...
        this._upHero.setPosition(GameConfig.stageWidth / 2 - this._upHero.serImgWith / 2, 60);
    ...
    BaseBattleView.prototype.setDownHero = function (picName, info, type) {
        ...
        this._downHero.setPosition(GameConfig.stageWidth / 2 - this._upHero.serImgWith / 2, GameConfig.stageHeigth - this._downHero.serImgHeight - 60 - this.getTitleButtomY());
    ...
    // public checkBattleHerosSkin()
    // {
    // 	this.checkUpHeroSkin();
    // 	this.checkDownHeroSkin();
    // }
    BaseBattleView.prototype.checkUpHeroSkin = function () {
        //显示皮肤攻击特效
        if (this._upHero.isSkinPic()) {
            if (!this._upduizhanpifu) {
                this._upduizhanpifu = App.DragonBonesUtil.getLoadDragonBones("duizhanpifu");
                this._upduizhanpifu.stop();
                this._upduizhanpifu.x = this._upHero.serImgWith / 2;
                this._upduizhanpifu.y = this._upHero.height;
                // let idx = this.container.getChildIndex(this._upduizhanpifu);
                // this._upduizhanpifu.addChildAt(this._upduizhanpifu,0);
                this._upHero.addChildAt(this._upduizhanpifu, 0);
            }
        }
        else {
            if (this._upduizhanpifu) {
                this._upduizhanpifu.stop();
                this._upduizhanpifu.dispose();
                this._upduizhanpifu = null;
            }
        }
    };
    BaseBattleView.prototype.checkDownHeroSkin = function () {
        //显示皮肤攻击特效
        if (this._downHero.isSkinPic()) {
            if (!this._downduizhanpifu) {
                this._downduizhanpifu = App.DragonBonesUtil.getLoadDragonBones("duizhanpifu");
                this._downduizhanpifu.stop();
                this._downduizhanpifu.x = this._downHero.serImgWith / 2;
                this._downduizhanpifu.y = this._downHero.serImgHeight / 2;
                this._downHero.addChildAt(this._downduizhanpifu, 0);
            }
        }
        else {
            if (this._downduizhanpifu) {
                this._downduizhanpifu.stop();
                this._downduizhanpifu.dispose();
                this._downduizhanpifu = null;
            }
        }
    };
    BaseBattleView.prototype.showSkinEffectAniHandle = function (area, moveTime1, critTime, tan) {
        var _this = this;
        if (area == 1 && this._downduizhanpifu) {
            this._downduizhanpifu.rotation = tan;
            egret.Tween.get(this._downduizhanpifu).wait(critTime + 300).call(function () {
                _this._downduizhanpifu.playDragonMovie("idle", 1);
            }, this).wait(moveTime1 + 200).call(function () {
                _this._downduizhanpifu.stop();
                _this._downduizhanpifu.rotation = 0;
            }, this);
        }
        if (area == 2 && this._upduizhanpifu) {
            this._upduizhanpifu.rotation = tan;
            egret.Tween.get(this._upduizhanpifu).wait(critTime + 300).call(function () {
                _this._upduizhanpifu.playDragonMovie("idle", 1);
            }, this).wait(moveTime1 + 200).call(function () {
                _this._upduizhanpifu.stop();
                _this._upduizhanpifu.rotation = 0;
            }, this);
        }
    };
    /**
    ...
    BaseBattleView.prototype.attackHandle = function (area, damage, isCrit) {
        ...
        //皮肤特效的动画
        var tan = 0; //Math.abs( Math.tan(moveY/offsetX) *180/Math.PI );
        if (area == 1) {
            this.showSkinEffectAniHandle(area, moveTime1, critTime, tan + 150);
        }
        else {
            this.showSkinEffectAniHandle(area, moveTime1, critTime, tan - 30);
        }
        //hero
    ...
    BaseBattleView.prototype.showBeAttackAnim = function () {
        ...
        this._beAttackClip.setPosition(this._heroArray[1].x + this._heroArray[1].serImgWith / 2 - tempBitmap.width / 2, beAttackHero.y + beAttackHero.serImgHeight / 2 - tempBitmap.height / 2);
        ...
        this._damageText.setPosition(this._heroArray[1].x + this._heroArray[1].serImgWith / 2 - this._damageText.width / 2, beAttackHero.y + beAttackHero.serImgHeight / 2 - 50);
    ...
    BaseBattleView.prototype.showCritAnim = function (area) {
        ...
        egret.Tween.get(crit_bg).to({ alpha: 0.7 }, 130).call(function () {
            ...
            flash_eff.x = baseobject.x + baseobject.serImgWith / 2 - 160 * 2;
            flash_eff.y = baseobject.y + baseobject.serImgHeight / 2 - 90 * 2;
        ...
        var speed_endY = baseobject.y - (226 * 1.6 - baseobject.serImgHeight) / 2;
        ...
        egret.Tween.get(speed_eff).wait(200).to({ scaleY: 1.6, alpha: 1 }, 130).call(function () {
            //三个光圈
            var circle_arr = [{
                    x: area == 1 ? (baseobject.x + baseobject.serImgWith / 2) : (baseobject.x - 30),
                    y: speed_eff.y + 50,
                    moveX: 10,
                    moveY: 15,
                    scale: 0.5,
                }, {
                    x: area == 1 ? (baseobject.x + baseobject.serImgWith / 2 + 40) : (baseobject.x - 60),
                    y: speed_eff.y + 53,
                    moveX: -15,
                    moveY: -20,
                    scale: 0.7,
                }, {
                    x: area == 1 ? (baseobject.x - 234) : (baseobject.x + baseobject.serImgWith + 80),
                    y: speed_eff.y + 100,
                    moveX: -30,
                    moveY: 0,
                    scale: 1,
        ...
        var endX = area == 1 ? (80) : (baseobject.x + baseobject.serImgWith);
        var endY = baseobject.y + baseobject.serImgHeight / 2 - 75 * 0.7;
