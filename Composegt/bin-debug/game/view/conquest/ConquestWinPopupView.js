...
var ConquestWinPopupView = (function (_super) {
    ...
    ConquestWinPopupView.prototype.initView = function () {
        ...
        var fontSize = TextFieldConst.FONTSIZE_TITLE_SMALL;
        if (PlatformManager.checkIsViSp()) {
            fontSize = TextFieldConst.FONTSIZE_CONTENT_SMALL;
        }
        var titleTxt = ComponentManager.getTextField(titleStr, fontSize, TextFieldConst.COLOR_WHITE);
        ...
        for (var i = 0; i < stringTab.length; i++) {
            var str = stringTab[i];
            var descTxt = ComponentManager.getTextField(str, fontSize, TextFieldConst.COLOR_WARN_GREEN2);
    ...
    ConquestWinPopupView.prototype.showWordAnim = function () {
        ...
        var scale1 = 2.5;
        var scale2 = 0.9;
        var tempsPos1 = egret.Point.create(GameConfig.stageWidth / 2 - winText.width / 2 * scale1, bgHeight - winText.height / 2 * scale1 + 30 - 70);
        var tempsPos2 = egret.Point.create(GameConfig.stageWidth / 2 - winText.width / 2 * scale2, bgHeight - winText.height / 2 * scale2 - 70);
        var realPos = egret.Point.create(GameConfig.stageWidth / 2 - winText.width / 2, bgHeight - winText.height / 2 - 70);
