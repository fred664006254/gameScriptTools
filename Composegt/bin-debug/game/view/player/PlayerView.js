...
var PlayerView = (function (_super) {
    ...
    PlayerView.prototype.initView = function () {
        ...
        var temY = fanImg.y;
        var nameTF = ComponentManager.getTextField(Api.playerVoApi.getPlayerName(), TextFieldConst.FONTSIZE_TITLE_SMALL, TextFieldConst.COLOR_WARN_YELLOW_NEW);
        // nameTF.border = true;
        nameTF.stroke = 1;
        ...
        var uidStr = Api.playerVoApi.getPlayerID().toString();
        var textSize = 20;
        ...
        if (Api.switchVoApi.checkOpenPrestigeShow() && MainUIUnLockCfg.getIsUnLockByKey("mainuiCity")) {
            var prestigeIcon = BaseBitmap.create("playerview_pro17");
            prestigeIcon.x = poIcon.x + 4;
            var prestigeBtn = ComponentManager.getButton("mainui_funicon", null, this.prestignBtnListener, this);
            ...
            var pValue = Api.prestigeVoApi.getPem();
            var prestigeTF = ComponentManager.getTextField(pValue + "", textSize, TextFieldConst.COLOR_WARN_YELLOW_NEW);
            // uidTF.text = "爽肤水地方是";
        ...
        */
        ...
        var titleText1 = ComponentManager.getTextField(LanguageManager.getlocal("playerview_quanshi", [App.StringUtil.changeIntToText(Api.playerVoApi.getPlayerPower())]), 24, TextFieldConst.COLOR_WARN_YELLOW_NEW);
        ;
        // 	if(PlatformManager.checkIsViSp()){
        ...
        //我的官位
        var officeImg = ComponentManager.getTextField(LanguageManager.getlocal("playerview_guanwei", [Api.playerVoApi.getPlayerMinLevelStr()]), 24, TextFieldConst.COLOR_BROWN_NEW); // BaseBitmap.create("playerview_guanwei_img");
        ...
        bottomInfoY += 10;
        var innerbg2 = BaseBitmap.create("public_9v_bg12");
        innerbg2.width = 590;
        innerbg2.height = 120;
        ...
        playerview_pro7.y = playerview_newpro2.y + playerview_newpro2.height + 10;
        ; //GameConfig.stageHeigth - 60//this._progressBar.y + this._progressBar.height/2 - playerview_pro7.height/2;
        ...
        //赚速
        var upgradeOfficeBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "promotionViewUpd", this.clickUpgradeHandler, this);
    ...
    PlayerView.prototype.prestignBtnListener = function () {
    ...
    PlayerView.prototype.refreshLvCfg = function (bottomInfoY) {
        ...
        var nextLvCfg = Config.MinlevelCfg.getCfgByMinLevelId(curMinLvId + 1);
        var priTxtStr = [
        ...
        if (nextLvCfg && nextLvCfg.batchCompose) {
            priTxtStr = priTxtStr.concat([
                [LanguageManager.getlocal("promotion_compose_desc5"), ""],
            ]);
        }
        ...
        for (var index = 0; index < priTxtStr.length; index++) {
            ...
            if (!nowVTxt1) {
                if (procfg[1] == "personLv") {
                    var str_1 = Api.composemapVoApi.getCanHaveMaxPersonLv();
                    nowVTxt1 = ComponentManager.getTextField(str_1 + '', 18, TextFieldConst.COLOR_BROWN_NEW);
                    this._changeVTxtList.push(nowVTxt1);
                    this.previewTxtCon.addChild(nowVTxt1);
                }
                else {
                    var str_2 = curLvCfg[procfg[1]];
                    nowVTxt1 = ComponentManager.getTextField(curLvCfg[procfg[1]], 18, TextFieldConst.COLOR_BROWN_NEW);
                    this._changeVTxtList.push(nowVTxt1);
                    this.previewTxtCon.addChild(nowVTxt1);
                }
            }
            var str = curLvCfg[procfg[1]];
            var showArrow = false;
            if (procfg[1] == "personLv") {
            ...
            if (nextLvCfg) {
                ...
                else if (index == 3 && nextLvCfg.batchCompose) {
    ...
    PlayerView.prototype.refreshProgressAndTxt = function () {
        ...
        var nextLvCfg = Config.MinlevelCfg.getCfgByMinLevelId(curMinLv + 1);
        if (nextLvCfg) {
            var str1 = LanguageManager.getlocal("playerview_exp") + Api.playerVoApi.getPlayerExp() + " / " + nextLvCfg.exp;
            this._progressBar.setPercentage(Api.playerVoApi.getPlayerExp() / nextLvCfg.exp, str1);
            // this._expTipTxt.text = '';
            var str2 = LanguageManager.getlocal("playerview_rate") + App.StringUtil.changeIntToText4(Api.mainTaskVoApi.getHistoryMaxLevyRate()) + " / " + App.StringUtil.changeIntToText3(nextLvCfg.needRate);
    ...
    PlayerView.prototype.refreshUpgradeClip = function () {
        ...
        if (nextLvCfg && Api.playerVoApi.getPlayerExp() >= nextLvCfg.exp && Api.mainTaskVoApi.getHistoryMaxLevyRate() >= nextLvCfg.needRate) {
            if (!this._upgradeClip) {
                ...
                var idx = this._bottomnodeContainer.getChildIndex(this._upgradeOfficeBtn);
    ...
    PlayerView.prototype.clickUpgradeHandler = function (param) {
        ...
        var nextLvCfg = Config.MinlevelCfg.getCfgByMinLevelId(curLv + 1);
        if (!nextLvCfg) {
            //已升到最大官阶
