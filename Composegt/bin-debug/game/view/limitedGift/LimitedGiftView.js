    ...
    LimitedGiftView.prototype.showList = function () {
        ...
        for (var key in this._rechargeCfgMap) {
            ...
            if (vo && cfg && vo.isbuy == 0) {
                ...
                if (timeTmp > 0) {
                    var contentData = { color: this._rechargeCfgMap[key].color, rechargeId: key, isOpen: isOpen, title: this._rechargeCfgMap[key].title, img: this._rechargeCfgMap[key].img, cost: cfg.cost, reward: cfg.getReward, time: vo.st + cfg.lastTime, gemCost: cfg.gemCost };
    ...
    LimitedGiftView.prototype.refreshListData = function () {
        ...
        for (var key in this._rechargeCfgMap) {
            ...
            if (vo && cfg && vo.isbuy == 0) {
                var timeTmp = vo.st + cfg.lastTime - GameData.serverTime;
                if (timeTmp > 0) {
                    var contentData = { color: this._rechargeCfgMap[key].color, rechargeId: key, isOpen: isOpen, title: this._rechargeCfgMap[key].title, img: this._rechargeCfgMap[key].img, cost: cfg.cost, reward: cfg.getReward, time: vo.st + cfg.lastTime };
                    this.contentList.push(contentData);
                }
            }
        }
