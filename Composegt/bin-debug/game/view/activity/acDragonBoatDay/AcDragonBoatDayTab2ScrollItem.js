...
var AcDragonBoatDayTab2ScrollItem = (function (_super) {
    ...
    AcDragonBoatDayTab2ScrollItem.prototype.initItem = function (index, data, itemparam) {
        ...
        this.addChild(Txt1);
        this.setLayoutPosition(LayoutConst.verticalCenter, Txt1, this._detailBgImg);
        Txt1.x = this._detailBgImg.x + 20;
        if (Txt1.width > this._detailBgImg.width) {
            this._detailBgImg.width = Txt1.width + 100;
        }
        //创建奖励列表
        if (this._rechargeItem.getReward.indexOf('1049') > -1) {
            if (Api.servantVoApi.getServantObj('1049')) {
                this._rechargeItem.getReward = this._rechargeItem.getReward.replace('8_1049_1', view.cfg.exchange);
            }
        }
    ...
    AcDragonBoatDayTab2ScrollItem.prototype.refreshBtnStatus = function () {
        ...
        if (chargeTotal >= this._rechargeItem.needGem) {
            this._detailBgImg.texture = ResourceManager.getRes("accarnivalview_tab_green2");
        }
        else {
            this._detailBgImg.texture = ResourceManager.getRes("activity_charge_red");
        }
        //检查是否创建已经领取标签
    ...
    AcDragonBoatDayTab2ScrollItem.prototype.eventCollectHandlerCallBack = function (event) {
        ...
        var pos = AcDragonBoatDayTab2ScrollItem._lastPos;
