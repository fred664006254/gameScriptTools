 ...
 * author hyd
 * date 2019/9/24
 * @class AcChaseBanditRewardPopupView
 */
var AcChaseBanditRewardPopupView = (function (_super) {
    __extends(AcChaseBanditRewardPopupView, _super);
    function AcChaseBanditRewardPopupView() {
        return _super.call(this) || this;
    }
    AcChaseBanditRewardPopupView.prototype.initView = function () {
        this.aid = this.param.data.aid;
        this.code = this.param.data.code;
        var acCfg = Config.AcCfg.getCfgByActivityIdAndCode(this.aid, this.code);
        var wifeSkinCfg = Config.WifeskinCfg.getWifeCfgById(acCfg.superp);
        if (wifeSkinCfg) {
            this.showWifeSkinReward(wifeSkinCfg);
        }
        else {
            var titlecfg = Config.TitleCfg.getTitleCfgById(acCfg.superp);
            if (titlecfg) {
                this.showTitleReward(titlecfg);
            }
        }
    };
    AcChaseBanditRewardPopupView.prototype.showTitleReward = function (titlecfg) {
        var bg = BaseLoadBitmap.create("acthrowarrowview_servantskinbg");
        bg.width = 544;
        bg.height = 618;
        bg.setPosition(this.viewBg.x + this.viewBg.width / 2 - bg.width / 2, 30);
        this.addChildToContainer(bg);
        var userContainer = Api.playerVoApi.getPlayerPortrait(Number(titlecfg.id), Api.playerVoApi.getPlayePicId());
        userContainer.x = bg.x + bg.width / 2 - userContainer.width / 2;
        userContainer.y = bg.y + 5;
        //衣服出左边框裁掉
        var maskX = bg.x - userContainer.x;
        var maskWidth = userContainer.width - maskX;
        userContainer.mask = egret.Rectangle.create().setTo(maskX, 0, maskWidth, bg.y + bg.height - userContainer.y - 7);
        userContainer.name = "userContainer";
        this.addChildToContainer(userContainer);
        this.container.height = bg.y + bg.height;
        var topDesc = ComponentManager.getTextField(LanguageManager.getlocal("acChaseBanditRewardPopupViewTopDesc-" + this.code), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BROWN);
        topDesc.setPosition(bg.x + bg.width / 2 - topDesc.width / 2, bg.y - topDesc.height - 3);
        this.addChildToContainer(topDesc);
        var skinnamebg = BaseBitmap.create("public_infobg2");
        skinnamebg.rotation = -90;
        skinnamebg.setPosition(bg.x + 10, bg.y + 100);
        this.addChildToContainer(skinnamebg);
        var skinNameTxt = ComponentManager.getTextField(titlecfg.name, TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_WHITE);
        skinNameTxt.setPosition(bg.x + 30, bg.y + 55);
        this.addChildToContainer(skinNameTxt);
    };
    AcChaseBanditRewardPopupView.prototype.showWifeSkinReward = function (wifeSkinCfg) {
    ...
    AcChaseBanditRewardPopupView.prototype.dealAttrChangeInfo = function (skinId) {
    ...
    AcChaseBanditRewardPopupView.prototype.getResourceList = function () {
        return _super.prototype.getResourceList.call(this).concat([
            "skin_detail_namebg",
        ]);
    };
    AcChaseBanditRewardPopupView.prototype.getTitleStr = function () {
        return "acChaseBandit_rewardpopupview_title-" + this.param.data.code;
    };
    // protected getShowHeight() {
    // 	return 720;
    // }
    AcChaseBanditRewardPopupView.prototype.dispose = function () {
        this.aid = null;
        this.code = null;
        _super.prototype.dispose.call(this);
    };
    return AcChaseBanditRewardPopupView;
}(PopupView));
__reflect(AcChaseBanditRewardPopupView.prototype, "AcChaseBanditRewardPopupView");
