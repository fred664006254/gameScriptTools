...
var AcMergeActiveViewTab1 = (function (_super) {
    __extends(AcMergeActiveViewTab1, _super);
    function AcMergeActiveViewTab1() {
    ...
    Object.defineProperty(AcMergeActiveViewTab1.prototype, "cfg", {
        get: function () {
    ...
    Object.defineProperty(AcMergeActiveViewTab1.prototype, "vo", {
        get: function () {
    ...
    Object.defineProperty(AcMergeActiveViewTab1.prototype, "acTivityId", {
        get: function () {
    ...
    AcMergeActiveViewTab1.prototype.initView = function () {
        App.MessageHelper.addNetMessage(NetRequestConst.REQUEST_ACTIVITY_GETMERGEACTIVEITEML, this.getRewardCallback, this);
        // 描述背景
        var descBg = BaseBitmap.create("mergeactive_descbg");
        descBg.x = 0;
        descBg.y = 0;
        this.addChild(descBg);
        // 描述1
        var desc1 = ComponentManager.getTextField(LanguageManager.getlocal("acMergeActiveDesc1", [App.DateUtil.getFormatBySecond(AcMergeActiveViewTab1.mztime, 6)]), TextFieldConst.FONTSIZE_ACTIVITY_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        this.addChild(desc1);
        // 描述2
        var desc2 = ComponentManager.getTextField(LanguageManager.getlocal("acMergeActiveDesc2"), TextFieldConst.FONTSIZE_ACTIVITY_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        desc2.width = 408;
        desc2.lineSpacing = 5;
        ...
        // 签到背景
        var signBg = BaseBitmap.create("mergeactive_signbg");
        signBg.x = 0;
        signBg.y = descBg.y + descBg.height;
        this.addChild(signBg);
        var treePart = BaseBitmap.create("mergeactive_part2");
        treePart.x = signBg.x;
        treePart.y = signBg.y + 2;
        this.addChild(treePart);
        var centerPart = BaseBitmap.create("mergeactive_part1");
        centerPart.x = signBg.x;
        centerPart.y = signBg.y;
        this.addChild(centerPart);
        var yEndArr = [172, 232, 200, 262, 222, 192];
        var yStartArr = [67, 90, 104, 91, 82, 90];
        // 一共六天
        var signCount = 6;
        var changeHeight = 68 * Math.min(1, Math.max(0, 1 - (GameConfig.stageHeigth - 960) / (1136 - 960)));
        // 签到底
        this.signBg = [];
        for (var i = 0; i < signCount; i++) {
            var signPart1 = BaseBitmap.create("mergeactive_sign1");
        ...
        // 签到线
        for (var i = 0; i < signCount; i++) {
            var signPart2 = BaseBitmap.create("mergeactive_sign2");
        ...
        // 签到天数
        for (var i = 0; i < signCount; i++) {
            var signDay = ComponentManager.getTextField(LanguageManager.getlocal("acChristmasViewFloor" + (i + 1)), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        // 签到奖励
        for (var i = 0; i < signCount; i++) {
            var rewardArr = GameData.formatRewardItem(this.cfg.signTask[i].getReward);
            var itemicon = GameData.getItemIcon(rewardArr[0], false, false);
        ...
        this.signStat = [];
        for (var i = 0; i < signCount; i++) {
            var signStat = ComponentManager.getTextField(LanguageManager.getlocal("acMergeActiveSignStat3"), TextFieldConst.FONTSIZE_ACTIVITY_COMMON - 2, TextFieldConst.COLOR_LIGHT_YELLOW);
    ...
    // 刷新界面数据
    AcMergeActiveViewTab1.prototype.refreshView = function () {
        var signCount = 6;
        // 签到状态
    ...
    // 点击使用按钮
    AcMergeActiveViewTab1.prototype.clickUseBtnHandler = function (param) {
        if (this.canGetRewordDays != -1) {
    ...
    // 点击具体某一天
    AcMergeActiveViewTab1.prototype.clickSignBgHandler = function (param, param2) {
    ...
    AcMergeActiveViewTab1.prototype.signRequestDo = function (thedays) {
    ...
    AcMergeActiveViewTab1.prototype.getRewardCallback = function (event) {
        if (event) {
    ...
    AcMergeActiveViewTab1.prototype.tick = function () {
    ...
    AcMergeActiveViewTab1.prototype.dispose = function () {
    ...
    return AcMergeActiveViewTab1;
}(CommonViewTab));
__reflect(AcMergeActiveViewTab1.prototype, "AcMergeActiveViewTab1");
