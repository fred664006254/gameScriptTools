...
/**
  ...
  * author 张朝阳
  * date 2019/7/16
  * @class AcRyeHarvestActivityRewardShopScrollItem
  */
var AcRyeHarvestActivityRewardShopScrollItem = (function (_super) {
    __extends(AcRyeHarvestActivityRewardShopScrollItem, _super);
    function AcRyeHarvestActivityRewardShopScrollItem() {
     ...
     * 初始化itemview
     */
    AcRyeHarvestActivityRewardShopScrollItem.prototype.initItem = function (index, data, itemParam) {
        var _this = this;
        this._itemData = data;
        ...
        if (this._itemData.discount) {
            ...
            this.addChild(tag);
            var tagTxt = ComponentManager.getTextField(LanguageManager.getlocal('acRyeHarvestActivityRewardShopScrollItemDiscount-' + this._aidAndCode.code, [(this._itemData.discount * 10).toString()]), 18, TextFieldConst.COLOR_WARN_YELLOW);
            var tagnum = 10 - this._itemData.discount * 10;
            if (PlatformManager.checkIsEnLang() || PlatformManager.checkIsThSp()) {
                tagTxt.text = LanguageManager.getlocal('acRyeHarvestActivityRewardShopScrollItemDiscount-' + this._aidAndCode.code, [(tagnum * 10).toString()]);
            }
            tagTxt.width = 70;
            tagTxt.height = 20;
        ...
        var limitTF = ComponentManager.getTextField(LanguageManager.getlocal("acRyeHarvestActivityRewardShopScrollItemLimit-" + this._aidAndCode.code, [String(limitNum)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WARN_GREEN2);
        ...
        var shopBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "acRyeHarvestActivityRewardShopScrollItemshop-" + this._aidAndCode.code, function () {
            if ((!vo.isStart)) {
            ...
            if (needPartVo.type == 1) {
                ...
                if (Api.itemVoApi.getItemNumInfoVoById(needPartVo.id) < needPartVo.num) {
                    App.CommonUtil.showTip(LanguageManager.getlocal("acRyeHarvestActivityRewardShopScrollItemNotEnoughItem-" + _this._aidAndCode.code));
                    return;
                }
            }
            NetManager.request(NetRequestConst.REQUEST_RYEHARVEST_BUYRYEHARVESTSHOP, { activeId: vo.aidAndCode, shopId: Number(data.id) });
        }, this);
    ...
    AcRyeHarvestActivityRewardShopScrollItem.prototype.getSpaceY = function () {
        return 5;
    };
    AcRyeHarvestActivityRewardShopScrollItem.prototype.dispose = function () {
        this._itemData = null;
        _super.prototype.dispose.call(this);
    };
    return AcRyeHarvestActivityRewardShopScrollItem;
}(ScrollListItem));
__reflect(AcRyeHarvestActivityRewardShopScrollItem.prototype, "AcRyeHarvestActivityRewardShopScrollItem");
