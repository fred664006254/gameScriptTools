    ...
    (function (AcCfg) {
        var DailyChargeCfg = (function () {
            ...
            DailyChargeCfg.prototype.getList = function () {
                var dailyExtraVo = Api.acVoApi.getActivityVoByAidAndCode("dailyChargeExtra");
                if (dailyExtraVo) {
                    ...
                    if (dailyExtraVo.st <= GameData.serverTime && dailyExtraVo.et >= GameData.serverTime && dailyVo.st == dailyExtraVo.st) {
                        var cfgObj = Config.AcCfg.getCfgByActivityIdAndCode("dailyChargeExtra", dailyExtraVo.code);
                        if (cfgObj) {
                            var extraList = cfgObj.getList();
                            for (var key in this.itemListCfg) {
                                var tmpCfg = this.itemListCfg[key];
