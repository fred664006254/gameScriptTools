            ...
            for (var key in shopItemList[tmpV]) {
                if (sheetType == shopItemList[tmpV][key].sheetType) {
                    if (sheetType == 3 && shopItemList[tmpV][key].shopGiftName == 46) {
                        var vos = Api.acVoApi.getRanActives();
                        for (var index = 0; index < vos.length; index++) {
                            var type = vos[index].config.type;
                            if (type && type == 16) {
        ...
        Object.defineProperty(ShopItemCfg.prototype, "contentList", {
            ...
            get: function () {
                if (this.sheetType == 3) {
                    ...
                    {
                        var voList2 = Api.acVoApi.getActivityVoListByAid("rescue");
                        if (voList2.length > 0) {
                            for (var id in voList2) {
                                var acVo = voList2[id];
                                var code = acVo.code;
                                var changeReward = Config.AcCfg.getCfgByActivityIdAndCode("rescue", code).changeReward;
                                // if(!changeReward){
                                // 	break;
                                // }
                                if (changeReward && code >= 1 && acVo.isDuringActive) {
