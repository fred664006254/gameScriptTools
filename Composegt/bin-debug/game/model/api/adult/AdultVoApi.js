...
var AdultVoApi = (function (_super) {
    ...
    AdultVoApi.prototype.isVisited = function (cid) {
        var adultInfoVo = this.getAdultInfoVoById(cid);
        var visited = false;
        if (adultInfoVo && adultInfoVo.visit) {
            visited = true;
        }
        return visited;
    };
    ...
    AdultVoApi.prototype.isSadunCanVisit = function (uid) {
        if (Api.switchVoApi.checkopenSadun()) {
            ...
            if (this._sadunVo.info && adultvolist.length > 0) {
                for (var i in this._sadunVo.info) {
                    ...
                    if (this._sadunVo.info[i].times >= Config.SadunCfg.needNum && this._sadunVo.info[i].visiting == '' && flag) {
                        return true;
                    }
                }
            }
