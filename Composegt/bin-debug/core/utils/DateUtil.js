...
(function (App) {
    ...
    (function (DateUtil) {
        ...
        function getServerYear() {
            var date = new Date(GameData.serverTime * 1000 + (GameData.timeZone + new Date().getTimezoneOffset() / 60) * 60 * 60 * 1000);
            return date.getFullYear();
        }
        DateUtil.getServerYear = getServerYear;
        /** 获取服务器当前月份 0-11 */
        function getServerMonth() {
            var date = new Date(GameData.serverTime * 1000 + (GameData.timeZone + new Date().getTimezoneOffset() / 60) * 60 * 60 * 1000);
            return date.getMonth();
        }
        DateUtil.getServerMonth = getServerMonth;
