    ...
    var LogUtil = (function () {
        ...
        LogUtil.init = function () {
            if (PlatMgr.checkIsLocal() || GameData.isTest()) {
                var testshowlog = egret.getOption("testnotshowlog");
                LogUtil.isTestShowLog = (testshowlog && testshowlog == "true") ? false : true;
            }
        };
        /**
         * 输出log，debug模式会输出
        ...
        LogUtil.showTxt = function () {
            ...
            var _a = LogUtil.formatLogParams(logParams), firstParam = _a.firstParam, otherParams = _a.otherParams;
            if (!LogUtil._txt) {
                LogUtil._txt = ComponentMgr.getTextField(firstParam + " " + otherParams, 15);
