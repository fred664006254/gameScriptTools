...
var BaseTextField = (function (_super) {
    ...
    function BaseTextField(autoStroke) {
        ...
        _this._maxChars = 0;
        _this._originalTxt = '';
        _this._autoStroke = true;
        /** 正在format 俄语换行 */
        _this._inFormatWordWarp = false;
        _this._autoStroke = autoStroke;
        return _this;
    }
    /**
     * 添加触摸回调
    ...
    Object.defineProperty(BaseTextField.prototype, "textColor", {
        get: function () {
            return egret.superGetter(BaseTextField, this, "textColor");
        },
        set: function (value) {
            var change = false;
            ...
            egret.superSetter(BaseTextField, this, "textColor", value);
            change && this.checkSetStroke();
        },
        enumerable: true,
        configurable: true
    });
    ...
    BaseTextField.prototype.checkSetStroke = function () {
        var tf = this;
        if (tf._autoStroke) {
            if (tf.textColor == ColorEnums.white) {
                tf.strokeColor = ColorEnums.strokeBlack;
                var strokeValue = tf.size < 20 ? 1 : (tf.size / 10 - 1);
                if (tf.stroke != strokeValue) {
                    tf.stroke = strokeValue;
                }
            }
            else {
                tf.stroke = 0;
    ...
    BaseTextField.prototype.setGradient = function (beginColor, endColor) {
        this.textColor = ColorEnums.white;
