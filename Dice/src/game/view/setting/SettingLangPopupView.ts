    ...
    protected initView():void
    {
        ...
        for (const key in arr) 
        {
            if (arr.hasOwnProperty(key)) 
            {
                ...
                let checkBox=ComponentMgr.getCheckBox(GameConfig.localLangCfg[langItem]);
    ...
    private changeLangHandler(target:CheckBox,params:any[]):void
    {
        ...
        if(target.checkSelected())
        {
			ViewController.getInstance().openView(ViewConst.CONFIRMPOPUPVIEW, {
				msg: LangMger.getlocal("switchAreaLangTip"), title: LangMger.getlocal("sysTip"), needCancel: true, handler: this, callback: () => {
