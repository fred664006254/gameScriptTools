...
class BaseTextField extends egret.TextField implements base.Iinteractive,base.Ibase
{
    ...
    private _autoStroke:boolean=true;

    /** 正在format 俄语换行 */
    private _inFormatWordWarp:boolean=false;
	public constructor(autoStroke:boolean=true) 
	{
        super();
        this._autoStroke=autoStroke;
	}
    ...
    private checkSetStroke():void
    {
        ...
        if(tf._autoStroke)
        {
            if(tf.textColor == ColorEnums.white)
            {
                ...
                let strokeValue=tf.size < 20 ? 1 : (tf.size / 10 - 1);
