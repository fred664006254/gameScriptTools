...
namespace ComponentMgr 
{
	 ...
	 */
	export function getButton(buttonName:string,textStr:string,callback:Function,handler:any,param?:any[],noDownImgType?:number,fontSize?:number, fontColor?:number, buttonWidth?:number) :BaseButton
	{
		var textSize:number = fontSize;
		let textColor:number=ColorEnums.white;
	...
	export function getTabBarGroup(buttonName:string|string[],textArr:Array<string>,callback:({param:any,index:number})=>void,handler:any,param?:any,aligh?:string,buttonWidth?:number,showPic?:boolean,showWidth?:number,showHeight?:number):TabBarGroup
	{
		...
		let selectedColor:number = ColorEnums.white;
	...
	{
		let color:number = ColorEnums.black;
		let selectedColor:number = ColorEnums.black;
		var tbg:TabBarGroup = new TabBarGroup();
	 ...
	 */
	export function getCustomMovieClip(imageNamePre?:string,frameCount?:number,frameRate?:number,effectClass?:{new();}):CustomMovieClip
	{
	 ...
	 */
	export function getTextField(textStr:string,fontSize:number,color?:number,autoStroke:boolean=true):BaseTextField
	{
		let tf:BaseTextField = new BaseTextField(autoStroke);
		tf.text = textStr;
		if(!isNaN(color))
		{
			tf.setColor(color);
	...
	export function getTextureText(txtStr:string,texturePre:string):TextureText
	{
		let txt=new TextureText();
		txt.init(texturePre);
	...
	export function getBitmapText(textStr:string,fontName:string,fontColor?:number,fontSize?:number,isBMfont?:boolean):BaseBitmapText|BaseTextField
	{
		...
		else
		{
			...
			let tf:BaseTextField = ComponentMgr.getTextField(textStr,fontSize,fontColor);
			return tf;
	 ...
	 * @param desc 文字
	 */
	export function getCheckBox(desc?:string,res?:string,fontsize?:number):CheckBox
	{
	...
	export function  getInputTextField(textColor:number,fontSize:number,width:number,height:number,bgName:string,placeholder?:string,placeholderColor?:number,initStr?:string):BaseDisplayObjectContainer
	{
		...
		let tfBg:BaseBitmap = BaseBitmap.create(bgName);
		tfBg.width = width;
		tfBg.height = height;
		...
		if(placeholder){
			...
			// if (App.DeviceUtil.isWXgame()) {
				placeholderTxt=ComponentMgr.getTextField(placeholder,fontSize);
				// placeholderTxt.border=true;
				placeholderTxt.textColor = placeholderColor;
				placeholderTxt.x = tf.x + (tf.width - placeholderTxt.width)/2;
				placeholderTxt.y = tf.y + (tf.height - placeholderTxt.height)/2;
	 ...
	 * 
	 */
	export function getNameLabel(name:string,bgtype:string|number="1",size:number = 20,color:number = ColorEnums.black):NameLabel
	{
	...
	*/
	export function getLimitLengthServerName(str : string, fontSize : number, showWidth : number, toughObj : any, textColor = 0xffffff):BaseTextField{
		let serverTxt = ComponentMgr.getTextField(str, fontSize, textColor);
		if(serverTxt.textWidth > showWidth){
			...
			serverTxt.addTouchTap(()=>{
				let msg = LangMger.getlocal(`serverListServer4`, [str]);
				let msgTF:BaseTextField = ComponentMgr.getTextField(msg,TextFieldConst.SIZE_CONTENT_COMMON);
				msgTF.width = 480;
