...
class RewardFly extends BaseDisplayObjectContainer
{
	...
	{	
		...
		{	
			...
			if(icon.indexOf("public_")==0&&ResMgr.getRes(icon))
			{
		...
		let msgTF:BaseBitmapText|BaseTextField = ComponentMgr.getBitmapText(message,TextFieldConst.FONTNAME_ITEMTIP,NaN,TextFieldConst.SIZE_TITLE_COMMON);
		...
		if (isHalve)
		{
			let halve:BaseTextField = ComponentMgr.getTextField(LangMger.getlocal("halve"),TextFieldConst.SIZE_CONTENT_COMMON);
