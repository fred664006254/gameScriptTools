...
class ProgressBar extends BaseDisplayObjectContainer
{
	 ...
	 * @param textColor 文字颜色
	 */
	public setTextStrokeColor(strokeColor:number):void
	{
	...
	public changeRes(res : string, res2 : string, isClip : boolean = false, clipNum? : number, duration? : number, width?:number, height?:number):void{
		if(isClip){
			if(!this.progressBarClip){
				let progressFire = ComponentMgr.getCustomMovieClip(res, clipNum, duration);
