...
namespace App
{
	export class DisplayUtil
	{
		 ...
		 */
		public static useObjectPool:boolean=false;
		 ...
		 * @param cal 仅计算坐标
		 */
		public static setLayoutPosition(style:string,self:egret.DisplayObject|egret.DisplayObjectContainer,base:egret.DisplayObject|egret.DisplayObjectContainer,distance:Array<number>=[0,0],local:boolean = false, cal:boolean = false) : {x : number, y : number}{
			let view = this;
			let _x = self.x;
			let _y = self.y;
			let style_arr = style.split('|');
			for(let layout of style_arr){
				let basex = (local || self.parent == base) ? 0 : base.x;
				let basey = (local || self.parent == base) ? 0 : base.y;
				let dis1 = distance[0];
				let dis2 = distance[distance.length - 1];
				switch(layout){
					case LayoutConst.left:
						_x = basex - base.anchorOffsetX + dis1 + self.anchorOffsetX;
						break;	
					case LayoutConst.right:
						_x = basex - base.anchorOffsetX + base.width * Math.abs(base.scaleX) - dis1 - self.width * Math.abs(self.scaleX) + self.anchorOffsetX;
						break;	
					case LayoutConst.top:
						_y = basey - base.anchorOffsetY + dis2 + self.anchorOffsetY;
						break;	
					case LayoutConst.bottom:
						_y = basey - base.anchorOffsetY + base.height * Math.abs(base.scaleY) - dis2 - self.height * Math.abs(self.scaleY) + self.anchorOffsetY;
						break;
					case LayoutConst.horizontalCenter:
						_x = basex - base.anchorOffsetX + (base.width * Math.abs(base.scaleX) - self.width * Math.abs(self.scaleX)) / 2 + self.anchorOffsetX + dis1;
						break;	
					case LayoutConst.verticalCenter:
						_y = basey - base.anchorOffsetY + (base.height * Math.abs(base.scaleY) - self.height * Math.abs(self.scaleY)) / 2 + self.anchorOffsetY + dis2;
						break;	
				}
			}
			if(!cal){
