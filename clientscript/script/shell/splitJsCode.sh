...
if [[ $needSplit != '' ]];then
    ...
    for tjs in ${arr[@]}
    do
        cat $jsname | grep "var $tjs=" > maintemp.min.js || exit 1
        sed -i ""  "s/.*var $tjs=/var $tjs=/" maintemp.min.js || exit 1
        sed -i ""  "s/$tjs.prototype,\"$tjs\");.*/$tjs.prototype,\"$tjs\");/" maintemp.min.js || exit 1
        record "合并$tjs到"$egretjsname
        cat maintemp.min.js >> $egretjsname || exit 1
        rm -rf maintemp.min.js
    
        record "删除主js文件$tjs代码"
        sed -i ""  's/var $tjs=.*$tjs.prototype,"$tjs");//' $jsname || exit 1
