...
local lua_dir = '/data/autodeploy/gt_config'
local target_json_dir = '/data/autodeploy/gt_client/cfg/'
...
local cfgPath="/data/autodeploy/gt_config";
local jsonStr = get_file(target_json_dir.."gameconfig_cn.json")
local jsonData = json.decode(jsonStr)
local svnv = excute_cmd("svnversion -c "..cfgPath.." |sed 's/^.*://' |sed 's/[A-Z]*$//'");
jsonData.v=svnv;
print("cnjson",jsonData.v);
