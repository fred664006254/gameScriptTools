...
echo '目标svn库目录' $branches/PalaceWar_$version


svn list http://svn.rjoy.com:9080/repos/jt/client/$branches/PalaceWar_$version || canCreate='1'
# sh ./updatePublicLanguage.sh
if [[ $canCreate == '1' ]]; then
    echo '生成trunk配置'
    ssh -tt root@192.168.8.82  -p 22 'sudo bash /data/autodeploy/merge.sh' || exit 1
    echo '同步cn'
    sh $DIR/../publishClient/sync_language.sh || exit 1

    echo "创建tag $version 版本库"
    svn cp -m "打tag $version" http://svn.rjoy.com:9080/repos/jt/client/trunk/PalaceWar http://svn.rjoy.com:9080/repos/jt/client/$branches/PalaceWar_$version || exit 1
    ...
    if [[ -d $DIR/../../../client/trunk/config ]]; then
        ...
        echo '生成tag配置'
        ssh -tt root@192.168.8.82  -p 22 'sudo bash /data/autodeploy/merge_online.sh' || echo 'tag配置生成失败了，需要手动操作'
    fi
    echo '打tag成功'
    /usr/local/bin/node $DIR/../node/sendDingTalkMsg.js "create_tag" $operaName $version & echo '发送通知...'
    echo $operaName $version
else
