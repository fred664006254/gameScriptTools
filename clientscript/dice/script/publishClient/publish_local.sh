...
# else
    if [[ $childFile != '' ]];then
        # rsync -r --delete --exclude='*.svn*' --exclude='*tags' $filepath/../../../client/trunk/Dice/resource/config/language/ $filepath/../../../publishclient/localtest/local/gt_local/resource/config/language/
        fromChildFile=`sh $filepath/../cfg/clientFilePathCfg.sh trunk Dice $childFile`
    ...
    if [[ $plat == 'euilocal' ]];then
        sh $filepath/../../../client/tags/DiceEuiTag/publish_h5_all.sh $plat 'local' $childFile || exit 1
    else
        sh $filepath/../../../client/trunk/Dice/publish_h5_all.sh $plat 'local' $childFile || exit 1
    fi
# fi
