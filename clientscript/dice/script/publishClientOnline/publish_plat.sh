...
trunkfile=$filepath2/client/trunk/Dice/template
...
syncTestToZS()
{
    ...
    if [ "$cleanplat" = "" ];then
        ...
        # elif [ -f "$publishfile/gt_$plat/index.html" ]; then 
            ...
            then
                echo '不存在 /Library/WebServer/Documents/publish/dice 目录，无法生成最新index'
                exit 1
            else
                cp -rf $trunkfile/index.php /Library/WebServer/Documents/publish/dice/ || exit 1
                curl -d "publishplat=$plat" 127.0.0.1/publish/dice/index.php > $publishfile/gt_$plat/index.html || exit 1
