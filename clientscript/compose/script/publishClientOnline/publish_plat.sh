...
# cd ../
ls
../syncSvn/updatePublish.sh $plat || exit 1
ls

publishfile=$filepath2/publishclient/$plat/
trunkfile=$filepath2/client/trunk/Composegt/template
...
syncTestToZS()
{
    ...
    if [ "$cleanplat" = "" ];then
        svn up $trunkfile
        if [ ! -f "$publishfile/gt_$plat/index.php" ] && [ ! -f "$publishfile/gt_$plat/index.html" ]; then
            cp -r $trunkfile/web/index.html $publishfile/gt_$plat/index.html || exit 1
        fi
        if [ -f "$publishfile/gt_$plat/index.php" ]; then
            ...
            then
                ...
                if [[ $manifestline != '' ]];then
                 ...
                 cp -rf "$publishfile/gt_$plat/index.html" "$publishfile/gt_$plat/index2.html" || exit 1
            fi
        fi
    else
        echo "清理资源不生成index"
    ...
    then
        echo "开始提交svn"
        # svn commit -m "tijiaozhengshimulu，$tagname"
         svn commit -m "tijiaozhengshimulu"
    else
        echo "存在第二个参数，不提交"
    fi
}
