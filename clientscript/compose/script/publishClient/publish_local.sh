...
# else
    if [[ $childFile != '' ]];then
        # rsync -r --delete --exclude='*.svn*' --exclude='*tags' $filepath/../../../client/trunk/Composegt/resource/config/language/ $filepath/../../../publishclient/localtest/local/gt_local/resource/config/language/
        fromChildFile=`sh $filepath/../cfg/clientFilePathCfg.sh trunk Composegt $childFile`
    ...
    sh $filepath/../../../client/trunk/Composegt/publish_h5_all.sh $plat 'local' $childFile || exit 1
# fi
