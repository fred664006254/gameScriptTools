...
echo '目标svn库目录' $branches/Composegt_$version


svn list http://svn.rjoy.com:9080/repos/compose/client/$branches/Composegt_$version || canCreate='1'
# sh ./updatePublicLanguage.sh
if [[ $canCreate == '1' ]]; then
    echo '生成trunk配置'
    ssh -tt root@192.168.8.95  -p 22 'sudo bash /data/autodeploy/merge.sh' || exit 1
    echo '同步cn'
    sh $DIR/../publishClient/sync_language.sh || exit 1

    echo "创建tag $version 版本库"
    svn cp -m "打tag $version" http://svn.rjoy.com:9080/repos/compose/client/trunk/Composegt http://svn.rjoy.com:9080/repos/compose/client/$branches/Composegt_$version || exit 1
    ...
    if [[ -d $DIR/../../../client/trunk/config ]]; then
        ...
        echo '生成tag配置'
        ssh -tt root@192.168.8.95  -p 22 'sudo bash /data/autodeploy/merge_online.sh' || echo 'tag配置生成失败了，需要手动操作'
    fi
    echo '打tag成功'
else
