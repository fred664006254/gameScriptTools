...
local lua_dir = '/Users/publish/h5/compose/config'
local target_json_dir = '/Users/publish/h5/compose/client/trunk/Composegt/resource/config/json/'
local cfg_md5_mark_path = '/Users/publish/h5/compose/clientscript/script/lua/tool/cfg_md5_mark'

if arg[1]=="tag" then
    lua_dir="/Users/publish/h5/compose/client/trunk/config"
    target_json_dir="/Users/publish/h5/compose/client/trunk/config/json/"
...
function lua2bigJson(language,pathinfo)
    ...
    local newspecialtarget3 = pathinfo[4]
    ...
    -- if newspecialtarget3 then
    --     merge_new_special3(language, newspecialtarget3)
    --      print(newspecialtarget3,"   updated")
    -- end
end
...
function merge_old_files(language,originpath,targetname)
    ...
    for encodekey,value in pairs(encodeCfg) do
        print(string.lower(encodekey));
        local str=json.encode(big_json[string.lower(encodekey)])
        for strkey,strv in pairs(value) do
            str=(string.gsub(str, strv, "#"..words[strkey]))
            print(strv,"#"..words[strkey])
        end
        big_json[string.lower(encodekey)]=json.decode(str);
    end
...
function find_file_by2(file_name)
    local nouse,end_pos,_ = string.find(file_name, "([a-zA-Z]_[0-9])")
...
function merge_new_special3(language,targetname)
    local newConfigCfg = require('newConfigCfg_3')
    local newFiles = {}
    for fileName,newInfo in pairs(newConfigCfg) do
        if newInfo[language] then
            newFiles[fileName] = 1
...
local mergePath = {
    ['cn'] = {lua_dir, target_json_dir.."gameconfig_cn.json", target_json_dir.."gameconfig_cn_new.json",target_json_dir.."gameconfig_cn_new_3.json"},
    -- ['cn_wx'] = {lua_dir, target_json_dir.."gameconfig_wx.json", target_json_dir.."gameconfig_wx_new.json"},
    -- ['cn_wxmg'] = {lua_dir, target_json_dir.."gameconfig_wxmg.json", target_json_dir.."gameconfig_wxmg_new.json"},
    -- ['devtest'] = {lua_dir, target_json_dir.."gameconfig_devtest.json", target_json_dir.."gameconfig_devtest_new.json"},
    -- ['cn_newwd'] = {lua_dir, target_json_dir.."gameconfig_newwd.json", target_json_dir.."gameconfig_newwd_new.json"},
    -- ['cn_newhope'] = {lua_dir, target_json_dir.."gameconfig_newhope.json", target_json_dir.."gameconfig_newhope_new.json"},
