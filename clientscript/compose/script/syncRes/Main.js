...
var Main = {
    main:function () {
        ...
        defaultGen.defaultGen(trunkPath + "/Composegt/resource");
    },
    // 打印进度
    nextProgress(info) {
        this.procedCount ++;
            ...
            if (info.needDelete.hasOwnProperty(language)) {
                ...
                for (const filePath in element) {
                    if (element.hasOwnProperty(filePath)) {
                        var cmd = "rm -f " + info.trunkPath + ((language === "cn")?"/Composegt/resource/assets":("/resoucres_multi/assets_"+language)) + filePath;
                        console.log(cmd);
                        shell.shell(cmd);
                    }
                }
    ...
    procCn:function (info) {
        ...
        for (var i = 0; i < cnProcArr.length; i++) {
            ...
            var f3 = info.trunkPath + "/Composegt/resource/assets" + f1;
