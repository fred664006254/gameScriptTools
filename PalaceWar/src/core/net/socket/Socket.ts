...
class Socket extends egret.EventDispatcher
{
	...
	/** 尝试连接超时时间 */
	private _connectTimeout:number=20000;
    ...
    public connect(host:string,port:number):void
	{
		...
		if(NetManager.checkHttps()||(PlatformManager.checkIsWxSp()&&this._host.indexOf("192.168.")<0)||App.TestUtil.getTestPlat()=="wx")
		{
			...
			let url="wss://"+this._host+":"+String(Number(this._port)+addValue);
			let proxy=this.getProxy(this._host,Number(this._port)+addValue);
			if(proxy)
			{
				url="wss://"+proxy;
			}
		...
		{
			let proxy=this.getProxy(this._host,this._port);
			if(proxy)
			{
				this._socket.connectByUrl("ws://"+proxy);
			}
			else
			{
	...
	private getProxy(host:string,port:number):string
	{
		let url:string="";
		let proxy=window['gameproxyline'];
		if(proxy)
		{
			let tmpHost=host.split(".")[0];
			url=proxy+":"+port+"/"+tmpHost+"/";
		}
		return url;
	}
