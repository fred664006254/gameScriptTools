...
class ScrollList extends ScrollView
{
	...
	private _changeCallBackObj:any;
	private _contentScaleX:number = 1;
	private _contentScaleY:number = 1;
	private _changeHandlerRect:egret.Rectangle = new egret.Rectangle();
	/**
	 * 每次刷新数量，就是每页数量
	 */
	...
	private initListData(stIdx:number,endIdx:number):void
	{
		...
		if(scrollListItem)
		{
			this.content.width=scrollListItem.width*this._contentScaleX;
		}
		this.content.height=(scrollListItem?this._lastY+scrollListItem.height:this._lastY)*this._contentScaleY;
	}

	public setContentScale(scaleX:number, scaleY:number):void{
		let oldScaleX = this._contentScaleX;
		let oldScaleY = this._contentScaleY;
		this._contentScaleX = scaleX;
		this._contentScaleY = scaleY;
		this.content.scaleX = scaleX;
		this.content.scaleY = scaleY;
		if (scaleY < oldScaleY){
			this._lastPos.y = this._lastPos.y * scaleY /oldScaleY;
		}
		
		// this.scrollRect.y = this.scrollRect.y * scaleY /oldScaleY;
		let scrollTop = this.scrollTop * scaleY / oldScaleY;
		if (oldScaleX > scaleX){
			this.scrollLeft = 0;
		}
		if (this._scrollListItemArr.length >= this._dataList.length){
			this.content.width = this.content.width * scaleX / oldScaleX;
			this.content.height = this.content.height * scaleY / oldScaleY;
			if (scrollTop > this.content.height - this.scrollRect.height){
				scrollTop = this.content.height - this.scrollRect.height;
			}
		}
		if (scrollTop <= 0){
			scrollTop = 0;
		}
		this.setScrollTop(scrollTop, 10);
	...
	private changeHandler(e:egret.Event):void
	{
		if(this._lastPos==null||this.scrollRect.y!=this._lastPos.y)
		{
			...
			if(this._lastPos==null||this.scrollRect.y>this._lastPos.y)
			{
				...
				{
					this._changeHandlerRect.setTo(this._scrollListHeightArr[i].x * this._contentScaleX, this._scrollListHeightArr[i].y * this._contentScaleY, this._scrollListHeightArr[i].width * this._contentScaleX, this._scrollListHeightArr[i].height* this._contentScaleY);
					if(useRect.intersects(this._changeHandlerRect))
					{
						if(isNaN(containMin))
						{
							containMin=i;
	 ...
	 * @param index 元素索引
	 * @param duration 缓动时间 
	 * @param isMoveTop 是否滑到顶部对齐
	 */
	public setScrollTopByIndex(index:number,duration?:number,isMoveTop?:boolean):void
	{
		...
		// 	{
		// 		this.initNextPage();
		// 		return this.setScrollTopByIndex(index,duration,isMoveTop);
		// 	}
		// 	else
		// 	{
		// 		index=this._scrollListItemArr.length-1;
		...
		if (isMoveTop){
			if(index+this._pageNum >=this._scrollListItemArr.length)
			{
				if(this._scrollListItemArr.length>=this._dataList.length)
				{
					index=this._scrollListItemArr.length-1;
			...
			let endRect:egret.Rectangle=this._scrollListHeightArr[this._scrollListHeightArr.length-1];
			let maxY:number=endRect.y * this._contentScaleY + endRect.height * this._contentScaleY ;
			let isTop:boolean=false;
			let isEnd:boolean=false;
			let scrollTop:number=this._scrollListHeightArr[index].y * this._contentScaleY ;
			if (scrollTop < 0){
				scrollTop = 0;
			}
			else if(scrollTop>maxY-this._scrollRect.height)
			{
				scrollTop=maxY-this._scrollRect.height;
			}
			else
			{
				scrollTop=this._scrollListHeightArr[index].y * this._contentScaleY ;
			}
			...
			let endRect:egret.Rectangle=this._scrollListHeightArr[this._scrollListHeightArr.length-1];
			let maxY:number=(endRect.y+endRect.height)* this._contentScaleY;
			let isTop:boolean=false;
			let isEnd:boolean=false;
			let scrollTop:number=this._scrollListHeightArr[index].y* this._contentScaleY+this._scrollListHeightArr[index].height* this._contentScaleY/2;
	...
	/**
	 * 检测是否在底部需要发送请求 针对于翻页排行榜
	 */
	public checkIsAtButtomRequest():boolean
	{
		if(this._scrollListHeightArr.length>0)
		{
			let lastIndex:number=this._scrollListHeightArr.length-1;	
			let maxY:number = this._scrollListHeightArr[lastIndex].y+this._scrollListHeightArr[lastIndex].height-this._scrollRect.height + 120;
			if(this.scrollTop>=maxY-this._scrollListItemArr[lastIndex].getSpaceY())
			{
				return true;
			}
	...
	/**
	 * 检测是否到顶部需要发送请求 针对于翻页排行榜
	 */
	public checkIsAtTopRequest():boolean{
	...
	/**
	 * 最大滑动区域 此处不能重写引擎的 getMaxScrollTop()方法，注意区分
	 */
	public getCurrMaxScrollTop():number{
		if(this._scrollListHeightArr.length>0)
		{
			let lastIndex:number=this._scrollListHeightArr.length-1;	
			let maxY:number = this._scrollListHeightArr[lastIndex].y+this._scrollListHeightArr[lastIndex].height-this._scrollRect.height;
			return maxY-this._scrollListItemArr[lastIndex].getSpaceY();
		}
		return 0;
	}
	...
	public moveChangeHandler():void{
		if (this._changeCallBack){
			this._changeCallBack.call(this._changeCallBackObj);
		}
	}

	public bindChangeCallback(callback:Function,callbackObj:any):void{
		this._changeCallBack = callback;
		this._changeCallBackObj = callbackObj;
	}
	...
	public dispose():void
	{
		...
		this._contentScaleX = 1;
		this._contentScaleY = 1;
		this._changeHandlerRect = null;
	}
}
