class WelfareViewMonthCard extends WelfareViewTab
{
	...
	private _descTF2:BaseTextField = null;

	private _offsetY:number = 0;
	/** 最近一次点击购买钻石的时间（防止连点） */
	private _lastClickBuyTime:number = 0;
	private _welfareWifebit:BaseBitmap =null;
	private _cardTimedeTxt:BaseTextField = null;

	private _img7:BaseBitmap =null;
	...
	protected init():void
	{
		...
		if(Api.switchVoApi.checkOpenNewMonthCardAndYearCard())
		{
			...
			{	
				...
				let rewardcfg =  Config.RechargeCfg.getRechargeItemCfgByKey("g7").getReward;
				if(acMouthBasevo)
				{
				...
				if(rewardcfg)
				{
					let rewardVo = GameData.formatRewardItem(rewardcfg);
					let rewardSp = GameData.getItemIcon(rewardVo[0],true,true);
					...
					let newSkindId = rechargeItemCfg.getReward.substr(3,4);
					if(newSkindId)
					{
		...
		let descSp = BaseBitmap.create(descSpStr);
