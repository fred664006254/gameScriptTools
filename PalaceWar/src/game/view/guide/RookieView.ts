...
class RookieView extends CommonView
{
	...
	private _curBgName:string = null;
	...
	private _curEffect:string = null;
	...
	private showPage():void
	{
		...
		if (this._curConfig.descId) {
			let descId = this._curConfig.descId;
			//描述 
			...
			{	
				...
				let effectName:string="effect_rookietalk_"+soundId;
			...
			if (this._curConfig.personPic == 1 || this._curConfig.personPic == 999 ) {
				...
				if (this._curConfig.personBone && !Api.switchVoApi.checkCloseBone() && RES.hasRes(this._curConfig.personBone+"_ske") && App.CommonUtil.check_dragon())
				{
					...
					if (this._curConfig.personPic.substring(0,4) == "wife") {
						servant.scaleX = 0.8;
						servant.scaleY = 0.8;
						servant.mask = new egret.Rectangle(-554, -609-100, 1114+120, 680);
						servant.x = 352;
						servant.y = GameConfig.stageHeigth - 140;
					}
					else
					{
		...
		if (this._curConfig.clickRect) {
			...
			{
				let curCfg=Config.SceneCfg.getSceneCfgBySceneName(this._curConfig.isScenePos,Api.otherInfoVoApi.getCurSceneId(this._curConfig.isScenePos));
				tempClickRect.x = curCfg.posCfg[this._curConfig.sceneKey].x - 10;

				
				let nameY:number = curCfg.namePosCfg[this._curConfig.sceneKey+"name"]?curCfg.namePosCfg[this._curConfig.sceneKey+"name"].y:curCfg.posCfg[this._curConfig.sceneKey].y;
				if (this._curConfig.clickRect.fromBottom)
				{
		...
		if (this._curConfig.branchMale && Api.switchVoApi.checkOpenBlueWife()&&Api.gameinfoVoApi.getSexnum()==2) {
			...
			let wordbg = BaseBitmap.create("searchstoryview_bottom");
			...
			let femalebtn = ComponentManager.getButton(`guidechoosesexbtn2`, `chooseSexTxt2`, ()=>{
					this._sexType = 0;
					this.freshView();
					this.clickSelectedWomanHandler();
			}, this, null, 3);
			...
			let descText= ComponentManager.getTextField(LanguageManager.getlocal("rookieStoryBranchMale1"),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_BROWN);
			...
			let descTextTip= ComponentManager.getTextField(LanguageManager.getlocal("rookieStoryBranchMale1_tip"),TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WARN_RED);
	...
	private clickSelectedManHandler():void
	{
	...
	private clickSelectedWomanHandler():void
	{
	...
	{	
		...
		let bgName:string = this._curBgName;
