...
class CouncilRewardRankItem extends ScrollListItem
{
    ...
    {	
        ...
        let bg = BaseBitmap.create('discussline');
        bg.width = view.width;
        bg.height = 38
        ...
        let serverTxt = ComponentManager.getTextField(LanguageManager.getlocal('councilRankListParam3', [data.exp]),TextFieldConst.FONTSIZE_CONTENT_COMMON);
		App.DisplayUtil.setLayoutPosition(LayoutConst.horizontalCentertop, serverTxt, bg, [0,bg.height + 10]);
        this.addChild(serverTxt);

		let scoreTxt = ComponentManager.getTextField(LanguageManager.getlocal('councilRankListParam4', [data.bookexp]),TextFieldConst.FONTSIZE_CONTENT_COMMON);
