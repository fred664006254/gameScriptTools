class PrisonView  extends CommonView
{
	...
	private _timeout1:number = 0;
	private _timeout2:number = 0;
	private _timeout3:number = 0;
	private _timeout4:number = 0;
	...
	private clipCallback():void
	{
		...
		{	
			...
			if(this.rewardsData)
			{
				let actTimeArr = App.CommonUtil.playRewardFlyAction(GameData.formatRewardItem(this.rewardsData.rewards));
	...
	public useCallback(event: egret.Event): void {		
		...
		else if (event.data.ret == true) 
		{
			...
			// this.clipCallback();
			this._timeout1 =egret.setTimeout(()=> 
			{
				this._timeout1 = 0;
				// 死亡
				if(this.prisonId!=0&&Api.prisonVoApi.getIndexPrisonNum(this.prisonId)==0)
				{
					...
					this._timeout2 =egret.setTimeout(()=> 
					{
						this._timeout2 = 0;
						//最后一个犯人的时候
	...
	{		
			this._timeout3 =egret.setTimeout(()=> 
			{
				this._timeout3 = 0;
			...
			for(var i:number=l-1;i>=0;i--)
			{
				...
				egret.Tween.get(this.currIconArr[i]).wait(100).to({factor:1},700).call(function(icon:BaseDisplayObjectContainer){
					...
					this._timeout4 =egret.setTimeout(()=> 
					{
						this._timeout4 =0;
						this.restPrisonView();
						icon.dispose();
	...
	private _closeHandler():void
	{
		...
		this._mainTaskHandKey = App.MainTaskHandUtil.addHandNode(
			this.container,
			this.prisonerImage.x + this.prisonerImage.width/2 - 10,
			this.prisonerImage.y + this.prisonerImage.height/2,
			[this.prisonerImage],
			116,
			true,
			function(){
