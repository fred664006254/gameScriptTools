 ...
 * @class ExamView
 */
 class ExamView extends CommonView{
    public SATURDAY = 6;
    public SUNDAY = 0;
    /**倒计时背景 */
    ...
    /**玩家排名 */
    public _playerRank = null;
    /**玩家积分 */
    public _playerScore = null;
    /**新科状元头像 */
    public _roleTitle:BaseBitmap = null;
    /**新科状元btn */
    public _titleBtn:BaseButton = null;
    public _isShowRank:boolean = false;
    /**会试 */
    public _meetExam:ExamBannerItem = null;
    /**殿试 */
    public _impreialExam:ExamBannerItem = null;
    /**排行榜数据 */
    public _rankData:any = null;
    /**等待结束显示信息 */
    public _waitContainer:BaseDisplayObjectContainer = null;
    public _playerRankContainer:BaseDisplayObjectContainer = null;
    public _playerRankBg:BaseBitmap = null;
    public _roleTitleIcon:BaseBitmap = null;
    public _numberOneContainer = null;
    ...
    public initView():void{
        ...
        //顶部背景
        let topBg:BaseBitmap = BaseBitmap.create("examview_infobg");
        topBg.y = this.titleBg.height - 7;
        this.addChildToContainer(topBg);
        
        let roleTitle:BaseBitmap = BaseBitmap.create("examview_role_title");
        roleTitle.y = topBg.y + topBg.height - roleTitle.height - 5;
        this.addChildToContainer(roleTitle);
        this._roleTitle = roleTitle;

        //称号
        let titleIcon = BaseBitmap.create("examview_champion_title1");
        ...
        //活动时间
        let examEndTimeStr = Api.examVoApi.getStartAndEndTime();
        let examEndTime = ComponentManager.getTextField(LanguageManager.getlocal("examinationTime", [examEndTimeStr.st, examEndTimeStr.et]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
        examEndTime.setPosition(roleTitle.width + 7, roleTitle.y + 5);
        this.addChildToContainer(examEndTime);

        //活动介绍
        let examDesc:BaseTextField = new BaseTextField();
        ...
        //玩家个人排名信息
        let playerRankContainer = new BaseDisplayObjectContainer();
        ...
        let playerRankBg = BaseBitmap.create("qingyuanitemtitlebg");
        playerRankBg.y = 3;
        playerRankContainer.addChild(playerRankBg);
        this._playerRankBg = playerRankBg;

        let rankStr = LanguageManager.getlocal("examinationNotInRank");
        let playerRank = ComponentManager.getTextField(LanguageManager.getlocal("examinationMyRank", [rankStr]), TextFieldConst.FONTSIZE_TITLE_SMALL, TextFieldConst.COLOR_WHITE);
        playerRankContainer.addChild(playerRank);
        this._playerRank = playerRank;

        let playerScore = ComponentManager.getTextField(LanguageManager.getlocal("examinationMyScore", [String(Api.examVoApi.getScore())]), TextFieldConst.FONTSIZE_TITLE_SMALL, TextFieldConst.COLOR_WHITE);
        ...
        let examBg:BaseBitmap = BaseBitmap.create("examview_bg");
        this.addChildToContainer(examBg);
        
        let examInfo1 = Api.examVoApi.getExamPhaseByDay(this.SATURDAY);
        let examSat = new ExamBannerItem();
        ...
        let examInfo2 = Api.examVoApi.getExamPhaseByDay(this.SUNDAY);
        let examSun = new ExamBannerItem();
        ...
        let rankBtn = ComponentManager.getButton("btn_big_yellow", "examinationRankBtnName", this.rankBtnClick, this);
    ...
    public showWaitView():void{
        if (Api.examVoApi.isFinishExamByDay(this.SUNDAY) && (Api.examVoApi.getEndTimeByDay(this.SUNDAY) > GameData.serverTime)){
            if (!this._waitContainer){
                ...
                let tipBg = BaseBitmap.create("wifestatus_namebg")
                this._waitContainer.addChild(tipBg);

                let waitInfo1 = ComponentManager.getTextField(LanguageManager.getlocal("examinationFinished"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_QUALITY_GREEN);
                this._waitContainer.addChild(waitInfo1);
                let waitInfo2 = ComponentManager.getTextField(LanguageManager.getlocal("examinationWait"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_QUALITY_YELLOW);
    ...
    public showNumberoneView():void{
        ...
        let level = this._rankData.rankArr[0].level;
        let pic = this._rankData.rankArr[0].pic; 
        // let rect = new egret.Rectangle(0, 0, 239, 149);
        let rect = new egret.Rectangle(0, 0, 300, 220);
        let container = Api.playerVoApi.getPlayerPortrait(level, pic);
    ...
    /**点击排行榜 */
    public rankBtnClick():void{
        this._isShowRank = true;
        NetManager.request(NetRequestConst.REQUEST_EXAM_RANK,{});
    }

    /**打开排行榜 */
    public rankBtnHandler(event:egret.Event):void{
        if (event.data.ret){
            let data = event.data.data.data;
            this._rankData = data;
	...
	{
        let pahseSat = Api.examVoApi.getExamPhaseByDay(this.SATURDAY);
        let pahseSun = Api.examVoApi.getExamPhaseByDay(this.SUNDAY);
		return [
