 ...
 */
class QingyuanNewItemRewardTab2 extends CommonViewTab{
    ...
    public initView():void{
        ...
        let offH = 0;
        let offY = 0
        let data = this.param.data.task;
        let isOpen = this.param.data.taskOpen;
        let len = 1;
        if (isOpen){
            len = data.length;
        }
        ...
        let openBtn = ComponentManager.getButton(openBtnImg, "", ()=>{
            App.MessageHelper.dispatchEvent(MessageConst.MESSAGE_ENCOUNTER_FRESHUI, {type: this.param.data.type, tabType: 2, isFreshData: true});
        }, this);
    ...
    public getItemContainer(itemData:any):BaseDisplayObjectContainer{
        ...
        let currData:any = Api.encounterVoApi.getTaskProcessByType(this.param.data.type, itemData.type, itemData.id);
        let currHaveNumKey = "qingyuantaskprocess1";
        if (currData.have >= currData.need){
            currHaveNumKey = "qingyuantaskprocess2";
        }
        let currNumStr = LanguageManager.getlocal(currHaveNumKey, [""+currData.have, ""+currData.need]);
        let titleTxt = ComponentManager.getTextField(numTitleStr + LanguageManager.getlocal(`qingyuantasktype`+itemData.type, [""+itemData.task_Value]) + currNumStr, TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        let txtcolor = TextFieldConst.COLOR_BROWN;
