	...
	protected initItem(index:number,data:any,itemParm:any):void
	{
		...
		if(this._uiType == FriendScrollItem.UITYPE1 || this._uiType == FriendScrollItem.UITYPE7){
			...
			if(Api.switchVoApi.checkOpenPlayerComeBack() && Api.newrebackVoApi.isInviteUid(this._uiData.uid)){
				let tipTxt = ComponentManager.getTextField(LanguageManager.getlocal(`playercomebackuserinfo5`),20,TextFieldConst.COLOR_WARN_GREEN2);
				...
				let tipTxt = ComponentManager.getTextField(LanguageManager.getlocal(`newinviteuserinfo5`),20,TextFieldConst.COLOR_WARN_GREEN2);
				this.addChild(tipTxt);
				App.DisplayUtil.setLayoutPosition(LayoutConst.lefttop, tipTxt, nameTxt, [nameTxt.textWidth+ 5,0]);
			}	
