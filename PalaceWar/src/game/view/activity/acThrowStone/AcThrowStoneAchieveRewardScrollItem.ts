 ...
 */
class AcThrowStoneAchieveRewardScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acthrowstoneBoxRewardInfo-" + this.getTypeCode(code), [String(data.needNum)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        let textStr = LanguageManager.getlocal("acthrowstoneBoxRewardNum-"+this.getTypeCode(code), [""+vo.getScore(), ""+data.needNum]);
		let textColor = TextFieldConst.COLOR_WHITE;
		progressBar.setPercentage(percent, textStr, textColor);
		progressBar.scaleX = 0.95;

		let titelTxt = ComponentManager.getTextField(LanguageManager.getlocal("acthrowstoneBoxRewardItemProgress-" + this.getTypeCode(code)), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BLACK);
