/**
  ...
  */
class AcMotherDayActivityRewardTaskScrollItem extends ScrollListItem {
	private _itemData: Config.AcCfg.MotherDayDailyTaskItemCfg = null;
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acMotherDayActivityRewardTaskScrollItemTaskType" + this._itemData.questType + "-" + (this.isSceneCode(this._aidAndCode.code) ? `3`:this._aidAndCode.code), [String(this._itemData.value)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		if (vo.getTaskFlag(String(this._aidAndCode.dayId), this._itemData.questType)) {
			...
			if (vo.getTaskValue(String(this._aidAndCode.dayId), this._itemData.questType) >= this._itemData.value) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "taskCollect", () => {
					let v = <AcMotherDayVo>Api.acVoApi.getActivityVoByAidAndCode(this._aidAndCode.aid, this._aidAndCode.code);
					if ((!v.isStart)) {
						App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
						return;
					}
					NetManager.request(NetRequestConst.REQUEST_MOTHERDAY_GETMOTHERDAYTASK, { activeId: vo.aidAndCode, taskId: Number(data.id), diffday: this._aidAndCode.dayId });
				}, this);
				...
				if (this._itemData.questType == "1004") {
					...
					let goBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_RED, "taskGoBtn", this.goBtnClick, this);
