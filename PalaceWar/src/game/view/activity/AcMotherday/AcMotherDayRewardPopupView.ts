 ...
 * author qianjun
 */
class AcMotherDayRewardPopupView  extends PopupView
{
	...
	{	
        ...
        let strTitle:string = `motherdayrewardtip1-${view.getUiCode()}`;
        if(rewardId == 11){
        ...
        let rewardX = rbg.x +  (500 - 108 * rewardArr.length - 10 * (rewardArr.length - 1)) / 2;
        let rewardY = rbg.y +7;
