 ...
 * author 钱竣
 */
class AcMotherDaySkinPopupView extends PopupView {
	public constructor() {
		super();
	}
	...
	protected initView(): void {
		...
		let isWifeskin = obj[view.getUiCode()] == `wife`;
		if(isWifeskin){
			...
			let topDesc = ComponentManager.getTextField(LanguageManager.getlocal(`acMotherDaySKinTip1-${view.getUiCode()}`,[view.cfg.bigPrize.needValue.toString(), Config.WifeskinCfg.getWifeCfgById(view.cfg.getSkin(view.code)).name]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			...
			let topDesc = ComponentManager.getTextField(LanguageManager.getlocal(`acMotherDaySKinTip1-` + this.getUiCode(), [view.cfg.bigPrize.needValue.toString(), view.cfg.getSkinName(view.code)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			topDesc.setPosition(topbg.x + topbg.width / 2 - topDesc.width / 2, topbg.y + topbg.height / 2 - topDesc.height / 2);
			this.addChildToContainer(topDesc);
