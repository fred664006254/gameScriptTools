 ...
 * author qianjun
 */
class AcArenaRankView extends PopupView
{
	// 滑动列表
	public constructor() 
	{
	...
	{		
		...
		let scoreStr =  ComponentManager.getTextField(LanguageManager.getlocal(`acArenaRankNum-${view.code}`,[addV.toString()]), 22, TextFieldConst.COLOR_WARN_GREEN); 
		...
		let titleTxt3 = ComponentManager.getTextField(LanguageManager.getlocal(`acArenaRankNeed-${view.code}`, [this.cfg.rankNeed.toString()]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
	...
	// "acRankPop_title3_11":"积分",
		...
		let titleTxt4 = ComponentManager.getTextField(LanguageManager.getlocal(`acArenaIconNum-${view.code}`), titleTxt1.size, TextFieldConst.COLOR_LIGHT_YELLOW)
