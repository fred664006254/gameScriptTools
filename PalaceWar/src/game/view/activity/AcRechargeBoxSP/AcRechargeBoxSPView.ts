...
class AcRechargeBoxSPView extends AcCommonView {
	...
	private boxInfoList: { "boxBg":BaseBitmap;"needGem": string;"lightBM": BaseBitmap;"boxBM": BaseLoadBitmap;"rechargeTF": BaseTextField;"rechargebg":BaseBitmap; "rechargeLine":BaseBitmap; "rechargeDiscount":BaseTextField, "boxCanGetEffect":CustomMovieClip;} []= null;
	private _roleIcon:BaseLoadBitmap = null;
	...
	public initView()
	{
		...
		if (this.getTypeCode() == "1"){
			...
			let wifeId = (showData[1].idType).split("_")[1];
		...
		if (this.getTypeCode() == "1"){
			...
			touchPos.addTouchTap(() => {
				if (this.getTypeCode() == "1"){
					...
					let wifeTopMsg = LanguageManager.getlocal("acRechargeBoxSpWifeTopMsg-"+this.code, [""+wifeRechargeCfg.gemCost]);
					let servantTopMsg = LanguageManager.getlocal("acRechargeBoxSpWifeSkinTopMsg-"+this.code, [""+servantRechargeCfg.gemCost]);
					...
					let showType = showData[1].idType;
					if (!this._isWife){
		...
		for(let i = 0;i < boxList.length;i++)
		{
			...
			let rechargeLine = BaseBitmap.create("shopview_line");
			...
			let rechargeDiscount = ComponentManager.getTextField(String(boxCfg.limit), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WARN_GREEN3);
			...
			let boxInfo:{ "boxBg":BaseBitmap;"needGem": string;"lightBM": BaseBitmap;"boxBM": BaseLoadBitmap;"rechargeTF": BaseTextField;"rechargebg":BaseBitmap; "rechargeLine":BaseBitmap; "rechargeDiscount":BaseTextField, "boxCanGetEffect":CustomMovieClip;} = {"boxBg":boxBg,"needGem": boxCfg.needGem,"lightBM":light,"boxBM": box,"rechargeTF": receiveNum,"rechargebg":rechargebg, "rechargeLine":rechargeLine, "rechargeDiscount":rechargeDiscount, "boxCanGetEffect":boxCanGetEffect};
	...
	private changeRoleIcon():void{
		...
		if (this.getTypeCode() == "1"){
			...
			if (this._isWife){
				...
				let roleRect = new egret.Rectangle(0, 0, 212/this._roleIcon.scaleX, 220/this._roleIcon.scaleY);
				this._roleIcon.mask = roleRect;
			}
			else{
				let servantId = (showData[0].idType).split("_")[1];
