//
class AcBattleHistoryRankVIewTab1 extends CommonViewTab
{
	...
	private get uiCode():string{
		let baseview : any = ViewController.getInstance().getView('AcBattleHistoryRankVIew');
	...
	protected initView():void
	{
		...
		let scrollList = ComponentManager.getScrollList(AcBattleHistoryPRankScrollItem,arr,tmpRect,this.param.data.code);
		...
		scrollList.bindMoveCompleteCallback(()=>{
			...
			if(this._rankindex != index){
				this.request(NetRequestConst.REQUEST_BATTLEGROUND_GETANK, {activeId:this.param.data.aid+"-"+this.param.data.code, index : index,test:1,  needround : this.param.data.round});
			}	
