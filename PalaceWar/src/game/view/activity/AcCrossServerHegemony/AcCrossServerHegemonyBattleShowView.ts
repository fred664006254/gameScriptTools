 ...
 * author qianjun
 */
class AcCrossServerHegemonyBattleShowView extends CommonView
{
	...
	protected receiveData(rdata:any):void{
		...
		if(Object.keys(data.tinfo).length == 0  || !data.info || Object.keys(data.info).length == 0 || !data.tinfo.info || Object.keys(data.tinfo.info).length == 0){
			...
			let tipsStr = LanguageManager.getlocal('acCrossServerHegemonyChangeTip6');
			if(Win<=0){
			...
			for(let i in arr){
				let info = null;
				let sourceObj = null;
				if(Api.allianceVoApi.getAllianceVo() && Number(data.tinfo.id) == Number(Api.allianceVoApi.getAllianceVo().id)){
					view._self = true;
	...
	public initView():void{
		...
		let logo = BaseBitmap.create('achegemony_logo-1');
		logo.setScale(0.6);
