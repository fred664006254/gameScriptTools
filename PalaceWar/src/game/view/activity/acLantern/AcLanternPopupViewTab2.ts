...
desc : 活动进度
*/
class AcLanternPopupViewTab2 extends CommonViewTab
{
	//滑动列表
	...
	{	
        ...
        let vo = this.vo;
		let objList = vo.getArr("answerList");//
		let arr = view.updateArr(objList);
 		let tmpRect =  new egret.Rectangle(0,0,530,Bg.height - 10);
		let scrollList = ComponentManager.getScrollList(AcLanternProcessItem,arr,tmpRect,{
			code : view.code,
	...
	private update():void{
		...
		let arr = view.updateArr(view.vo.getArr("answerList"));;//
