...
desc : 活动兑换
*/
class AcLanternPopupViewTab3 extends CommonViewTab
{
	//滑动列表
	...
	protected initView():void{
		...
		let skinId = view.cfg.coreReward;//;
		...
		if(isWifeskin){
			...
			let str = LanguageManager.getlocal(`acCommonSkinGet2`, [view.cfg.getCoreRewardGemNum(view.code).toString(), skinCfg.name]);
			...
			let topDesc = ComponentManager.getTextField(LanguageManager.getlocal(`aclanterntip9-${view.getUiCode()}`, [view.cfg.getCoreRewardGemNum(view.code).toString(), skinCfg.name]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			topDesc.setPosition(topbg.x + topbg.width / 2 - topDesc.width / 2, topbg.y + topbg.height / 2 - topDesc.height / 2);
			this.addChild(topDesc);
			...
			let img = App.CommonUtil.getServantSkinFlagById(this.cfg.coreReward);
			if(img){
