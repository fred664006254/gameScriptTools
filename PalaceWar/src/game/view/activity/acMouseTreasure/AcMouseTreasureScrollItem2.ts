 ...
 * @author sl
 */
class AcMouseTreasureScrollItem2 extends ScrollListItem {
    ...
    protected initItem(index: number, data: any, itemParam: any) {
        ...
        //title txt
        let titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("mouseTreasureProcessTitle", this.getTypeCode()), [""+data.needScore]);;
        ...
        let currNum = this.vo.ainfo.v;
        let progressStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acLotusDetailProcessNum", this.getTypeCode()), [String(currNum), String(data.needScore)]);
        progress.setPercentage(currNum / data.needScore, progressStr, TextFieldConst.COLOR_WHITE);
        ...
        if (this.vo.isGetAchieveRewardById(data.id)) {
            ...
            let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
                ...
                NetManager.request(NetRequestConst.REQUEST_MOUSETREASURE_GETACHIEVEMENT, { activeId: this.vo.aidAndCode, rkey: data.id}); 
            }, this);
