 ...
 */
class AcWorshipGetRewardPopupView extends BaseView {
    ...
    private _isHasCri: boolean = false;
    private _aidCode:string = null;
    public constructor() {
        super();
    }
    ...
    protected initView(): void {
        ...
        let criArr = this.param.data.criArr;
        if (rewards_data) {
    ...
    private initBuffers(buffers: any) {
        ...
        let msgStr = "acWorshipGetRewardPopupViewBuffer-"+ this.param.data.code;
        let msgBg = null;
        ...
        let wealthLuckTxt2 = ComponentManager.getTextField(LanguageManager.getlocal(msgStr, [String(v)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
    ...
    private createItem(count: number) {
        ...
        if (this._isPlayAni) {
            rewardDB.setScale(0);
            egret.Tween.get(rewardDB, { loop: false }).wait(100).to({ scaleX: 1.2, scaleY: 1.2 }, 200).to({ scaleX: 1.0, scaleY: 1.0 }, 50).call(() => {
                egret.Tween.removeTweens(rewardDB);
                if (this.param.data.criArr) {
                    let cri = this.param.data.criArr[count];
                    if (cri && String(cri) == "2") {
                        let criCloud = BaseBitmap.create("acworshipview_cricloud");
                        criCloud.setPosition(rewardDB.x + rewardDB.width / 2 - criCloud.width - 3, rewardDB.y - rewardDB.height / 2 + 3);
                        this.addChildToContainer(criCloud);
                        let criStr = "acworshipview_cri";
                        if (this._aidCode && this._aidCode == "yiyibushe-1"){
                            criStr = "acyiyibushe_good_text";
                        }
                        let cri = BaseBitmap.create(criStr);
                        cri.setPosition(criCloud.x + criCloud.width - cri.width + 2, criCloud.y + criCloud.height - cri.height - 2);
                        this.addChildToContainer(cri);
                    }
                }
            }, this);
            this._count++;
