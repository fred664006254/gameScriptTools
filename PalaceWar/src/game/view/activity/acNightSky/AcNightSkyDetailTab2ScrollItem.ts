 ...
 * 奖池item
 */
class AcNightSkyDetailTab2ScrollItem extends ScrollListItem {
    ...
    protected initItem(index: number, data: any, itemParam: any) {
        ...
        //title txt
        let titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acNightSkyPoolTitle"+data.id, this.getTypeCode()));
        ...
        if (data.id == poolId){
            ...
            let selBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, App.CommonUtil.getCnByCode("acGoodMatchPoolSelect", "1"), ()=>{
                ...
                if (this.vo.getPoolRewardId()!=0){
                    App.CommonUtil.showTip(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acNightSkyPoolTip3", this.getTypeCode())));
                    return ;
                }
                NetManager.request(NetRequestConst.REQUEST_ACNIGHTSKY_SETTYPE, {activeId: this.vo.aidAndCode, rtype: data.id});
            }, this);
