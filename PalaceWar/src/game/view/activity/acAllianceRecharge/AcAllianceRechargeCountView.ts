 ...
 */
class AcAllianceRechargeCountView extends AcCommonView
{
    ...
    protected get acVo():AcAllianceRechargeCountVo
	{
		...
		return <AcAllianceRechargeCountVo>Api.acVoApi.getActivityVoByAidAndCode(this.aid,currCode);
	}
    ...
    public initView()
    {
        ...
        this.addChild(this._activityTimerText);
      	...
      	//倒计时文本 
		let acCDTxt = ComponentManager.getTextField("acAlliance_acCD", TextFieldConst.FONTSIZE_CONTENT_COMMON, 0xff0000);
       ...
       this.refreshBtnStatus();
        ...
        let tmpRect = null;
        let keys =this.cfg.countReward;
        tmpRect = new egret.Rectangle(0,0,GameConfig.stageWidth,bg5.height - 10);
        let scrollList = ComponentManager.getScrollList(AcAllianceRechargeScrollItem,keys,tmpRect, this.getUiCode());
	...
	{
		let ruleStr=this.getClassName().toLowerCase().replace("view","")+"RuleInfo_"+this.getUiCode();
