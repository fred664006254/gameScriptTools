/**
  ...
  */
 class AcBirdBridgeScrollItem1 extends ScrollListItem {
	 ...
	 * 初始化itemview
	 */
	public initItem(index: number, data: Config.AcCfg.BirdBridgeRechargeItem, itemParam: any): void {
		this._itemData = data;
		...
		if (this.vo.isGetRechargeRewardById(data.id)) {
			...
			if (currChargeGem >= data.needGem) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_AC_BIRDBRIDGEGETRECHARGE, { activeId: this.vo.aidAndCode, rkey: data.id});
				}, this);
