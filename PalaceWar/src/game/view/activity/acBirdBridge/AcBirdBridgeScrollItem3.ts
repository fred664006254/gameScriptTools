/**
  ...
  */
 class AcBirdBridgeScrollItem3 extends ScrollListItem {
	private _itemData:Config.AcCfg.BirdBridgeAchievementAllItem= null;
	 ...
	 * 初始化itemview
	 */
	public initItem(index: number, data: Config.AcCfg.BirdBridgeAchievementAllItem, itemParam: any): void {
		this._itemData = data;
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acBirdBridgeProcessTitle2", itemParam.uicode) , [String(this._itemData.needNum2)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let currChargeGem = this.vo.ainfo[1].v;
		let progressStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acLotusDetailProcessNum", "1"), [String(currChargeGem), String(data.needNum2)]);
		...
		if (this.vo.isGetAchievementAllRewardById(data.id)) {
            ...
            {	
                ...
                let locktext = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acBirdBridgeLocked", itemParam.uicode) , [String(this.vo.getAchievementAllNeedAchievementOneTimesById(data.id))]), 20, TextFieldConst.COLOR_WARN_RED2);
                locktext.width = 160;
            ...
            else
            {
                let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_AC_BIRDBRIDGEGETACHIEVE, { activeId: this.vo.aidAndCode, rkey: data.id,all:1});
				}, this);
