 ...
 * @author sl
 */
class AcBirdBridgeScrollItem2 extends ScrollListItem {
    private _aid:string = null;
    private _code:string = null;
    private _uicode:string = null;
    ...
    protected initItem(index: number, data: Config.AcCfg.BirdBridgeAchievementOneItem, itemParam: any) {
        ...
        //title txt
        let titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acBirdBridgeProcessTitle", this.getTypeCode()), [""+data.needNum]);;
        ...
        let currNum = this.vo.ainfo[0].v;
        let progressStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acLotusDetailProcessNum", "1"), [String(currNum), String(data.needNum)]);
        ...
        if (this.vo.isGetAchievementOneRewardById(data.id)) {
            ...
            let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
                ...
                NetManager.request(NetRequestConst.REQUEST_AC_BIRDBRIDGEGETACHIEVE, { activeId: this.vo.aidAndCode, rkey: data.id,all:0}); 
            }, this);
