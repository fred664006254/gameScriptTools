...
class AcTreasureHuntAVGView extends CommonView
{
	...
	protected getResourceList():string[]{	
        ...
        let guidePic:string[] = [];
        let allAvg = view.AVGDialog[view._mapId][view._key][view._curIdx];
	...
	{	
		...
		let bgname = `treasurebg-${this.param.data.code}`;
	...
	private showPage():void
	{
		...
		if (this._curConfig.descId) {
			...
			if (1) {
                ...
                let str = `acTreasurecarTalk${this._mapId}npc${this._key}_${this._curConfig.descId}-${this.param.data.code}`;
	...
	{	
		...
		if(this._curConfig.needFight){
			let idx = view._mapId == 23 && view._key == 5 ? '2' : '1';
			let npc = '';
			let npcName = '';
			for(let i in view.AVGDialog[view._mapId][view._key]){
				let unit = view.AVGDialog[view._mapId][view._key][i];
