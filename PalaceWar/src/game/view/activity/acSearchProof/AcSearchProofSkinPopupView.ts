 ...
 */
class AcSearchProofSkinPopupView extends PopupView {
	public constructor() {
		super();
	}
	private aid: any;
	...
	protected initView(): void {
		...
		let proofNum = Api.itemVoApi.getItemNumInfoVoById(GameData.formatRewardItem(acCfg.mustGet2)[0].id);
		...
		let topDesc = ComponentManager.getTextField(LanguageManager.getlocal("acSearchProofSkinPopupViewTopDesc-" + this.code, [String(rewardsVo.num)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
		...
		let btn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "acSearchProofSkinPopupViewExchange-" + this.code, () => {
			...
			if (this._progress.getPercent() < 1) {
				App.CommonUtil.showTip(LanguageManager.getlocal("acSearchProofViewNotEnoughProof-" + this.code));
				return;
			}
