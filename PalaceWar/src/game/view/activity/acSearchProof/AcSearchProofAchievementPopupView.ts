 ...
 */
class AcSearchProofAchievementPopupView extends PopupView {
	...
	private weathRewardHandle(event: egret.Event) {
		if (event.data.ret) {
			...
			let vo = <AcSearchProofVo>Api.acVoApi.getActivityVoByAidAndCode(this.param.data.aid, this.param.data.code);
			let processCfg = vo.getSortAchievementCfg();
			processCfg.sort((a, b) => { return a.sortId - b.sortId });
