class AcAnnualPrayPrayItem extends ScrollListItem {
	...
	protected initItem(index:number,data:Config.AcCfg.AnnualPrayProcessItemCfg,itemParam:any)
    {	
        let view = this;
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode(`acAnnualPrayPrayTip5`, code), [String(data.ratetime)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let percent = vo.getPrayNum() / data.ratetime;
		let textStr = vo.getPrayNum() + "/" + data.ratetime;
