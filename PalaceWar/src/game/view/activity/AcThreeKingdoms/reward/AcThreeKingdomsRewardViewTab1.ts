//赛季个人
class AcThreeKingdomsRewardViewTab1 extends CommonViewTab
{
	...
	protected initView():void{
		...
		for (let index = 0; index < rankList.length; index++) {
			...
			//特出处理第一名
			if(index == 0){
				let specialerewardbg = BaseBitmap.create(`threekingdomsranktoprewardbg`);
				...
				let role = BaseBitmap.create(`threekingdomsranktoprewardrole`);
				...
				let title = App.CommonUtil.getTitlePic(3151,10);
				...
				let startY = specialerewardbg.y + specialerewardbg.height+ 10;
				tmpX = 82;
				scroStartY = startY;
				for(let i = 0; i < 4; ++ i){
					let element = rIcons[i];
					element.x = tmpX;
				...
				tmpX = 20;
				scroStartY += 123;
				for (let innerIdx = 4; innerIdx < len; innerIdx++) {
		...
		//我的赛季分数
		let mypoint = this.vo.getMyPrankSeasonPoints();
		...
		//赛季排名
		let rankstr = ``;
		let rankV = view.vo.getMyPrankSeaonRank();
