...
desc : 佳人预览
*/
class AcDechuanshidaiPopupViewTab4 extends AcCommonViewTab
{
	//滑动列表
	...
	{	
		...
		let wifeId = view.cfg.getSkin(code);
		...
		if (isWife){
			...
			let str = LanguageManager.getlocal(`acDechuanshidaitip3-${code}`,[String(need), wifeCfg.name]);
			...
			let str = LanguageManager.getlocal(`acDechuanshidaitip2-${code}`,[String(need), servantCfg.name]);
			let topDesc = ComponentManager.getTextField(str, TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
			topDesc.setPosition(topbg.x + topbg.width / 2 - topDesc.width / 2, topbg.y + topbg.height / 2 - topDesc.height / 2 - 2);
			this.addChild(topDesc);
