...
class AcLocTombBattleView extends BaseBattleView
{
	...
	{	
		...
		if(iskill){
			...
			let upHeroPic:string = view._bossConfig.getnpcPic(view.code);
			...
			let servantName:BaseTextField = ComponentManager.getTextField(view._bossConfig.getnpcName(view.code),TextFieldConst.FONTSIZE_TITLE_SMALL,TextFieldConst.COLOR_LIGHT_YELLOW);
			...
			let shopBtn = ComponentManager.getButton(`tombshopbtn-${this.code}`, '', ()=>{
			...
			if(view._bossConfig.id == 7){
				let tombbosseff = ComponentManager.getCustomMovieClip(`tombbosseff${view.code}-`, 10, 80);
