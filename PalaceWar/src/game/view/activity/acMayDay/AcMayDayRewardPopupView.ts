class AcMayDayRewardPopupView  extends BaseView
{
    ...
    protected initView():void
	{
        ...
        let itemid = this.cfg._getReward ? this.cfg._getReward.split(`_`)[1] : '1001';
        let itemcfg = Config.ItemCfg.getItemCfgById(itemid);
        let goldNum = rewardTab.length > 2 ? 10 : 1;
        let getTip = ComponentManager.getTextField(LanguageManager.getlocal("acTailGetTip2",[itemcfg.name]),24);
        let sliverIcon = BaseLoadBitmap.create(itemcfg.icon);
