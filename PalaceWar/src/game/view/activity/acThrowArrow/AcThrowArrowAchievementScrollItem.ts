 ...
 */
class AcThrowArrowAchievementScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let id = itemParam.id;
		let vo = <AcThrowArrowVo>Api.acVoApi.getActivityVoByAidAndCode(aid, code);
		let cfg = <Config.AcCfg.ThrowArrowCfg>Config.AcCfg.getCfgByActivityIdAndCode(aid, code);
		this.width = 530;
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acThrowArrowAchievementPopupViewItemTitle-" + code, [String(data.needNum)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let percent = vo.getLotteryValue() / data.needNum;
		let textStr = vo.getLotteryValue() + "/" + data.needNum;
		let textColor = TextFieldConst.COLOR_WHITE;
		progressBar.setPercentage(percent, textStr, textColor);
		progressBar.scaleX = 0.95;

		let titelTxt = ComponentManager.getTextField(LanguageManager.getlocal("acThrowArrowAchievementPopupViewItemProgress-" + code), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BLACK);
