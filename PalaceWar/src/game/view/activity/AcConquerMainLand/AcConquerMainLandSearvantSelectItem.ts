...
class AcConquerMainLandSearvantSelectItem extends ScrollListItem
{
	...
	private initServantIcon(data:any):void{	
		...
		let costTxt = ComponentManager.getTextField(`${costObj.cost}`, 18, TextFieldConst.COLOR_LIGHT_YELLOW)
        ...
        let freeTxt = ComponentManager.getTextField(LanguageManager.getlocal(`acConquerMainLandArmyFree-${code}`, [costObj.freeNum.toString()]), 18, TextFieldConst.COLOR_LIGHT_YELLOW)
        ...
        let flag = BaseBitmap.create(attend == data.army ? `mlservantselected-${view.getUiCode()}`: `awservantstate1`);
    ...
    public refreshData(data?:any){
		...
		let tmpx = (92- view._costIcon.width - 3 - view._costTxt.textWidth) / 2;
