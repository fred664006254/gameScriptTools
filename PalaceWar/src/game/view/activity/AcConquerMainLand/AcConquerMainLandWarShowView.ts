...
class AcConquerMainLandWarShowView extends CommonView
{
	...
	private _servantCardBgleft : BaseLoadBitmap = null;
	private _servantCardBgright : BaseLoadBitmap = null;
	...
	public initView():void{
		...
		let logo = BaseBitmap.create(`mlfighttitle-${view.uiCode}`);
		App.DisplayUtil.setLayoutPosition(LayoutConst.horizontalCentertop, logo, view);
		view._topGroup.addChild(logo);

		let mask = BaseBitmap.create(`mlfightmask-${view.uiCode}`);
		App.DisplayUtil.setLayoutPosition(LayoutConst.horizontalCentertop, mask, logo, [0,45]);
		view._topGroup.addChild(mask);
		
		let cityTxt = ComponentManager.getTextField(LanguageManager.getlocal(`CountryWarVsTitle`, [view.param.data.cityName]), 22, TextFieldConst.COLOR_WARN_YELLOW);
	...
	private createWarInfo(type : string):void{
        ...
        //职员信息
		let infobg = BaseBitmap.create(type == `left` ? `crossservantvsmask2` : `crossservantvsmask1`);
		infobg.height = 140;
		...
		let teamNameTxt = ComponentManager.getTextField(wardata.name, 20);
		...
		let titleImg = null;
		let titleinfo = App.CommonUtil.getTitleData(wardata.titleId);
		if(titleinfo.title !== ''){
		...
		let zidTxt = ComponentManager.getBitmapText(Api.mergeServerVoApi.getAfterMergeSeverName(null,true,wardata.zid), TextFieldConst.FONTNAME_ITEMTIP);
		...
		let descTxt =ComponentManager.getTextField(str, 20, TextFieldConst.COLOR_BLACK);
		descTxt.width = 200;
		...
		let playernamebg = BaseBitmap.create(type == `left` ? 'awnamebg2' : 'awnamebg1');
		...
		let cardbg = BaseLoadBitmap.create(``);
		cardbg.width = 91; 
		cardbg.height = 81; 
		cardbg.setScale(0.8);
	...
	private showRoundAnti(type : string, func?:Function, obj?:any, showskin?,showskinfunc?):void{
		...
		let cardbg : BaseLoadBitmap = view[`_servantCardBg${type}`];
		...
		if(roundData){
			...
			if(view._round == 1){
				...
				egret.Tween.get(rolegroup).to({x : isOnleft ? -248 : 508, y : view._rolePosY}, 250).call(()=>{
					...
					role.setload(img);
					cardbg.setload("servant_alv_" + data.clv)
