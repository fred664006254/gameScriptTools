 ...
 * @author ycg
 */
class AcLotusDetailPopupViewTab1ScrollItem extends ScrollListItem {
    ...
    protected initItem(index: number, data: any, itemParam: any) {
        ...
        let titleTxtStr = "";
        if (index < 3){
            titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acLotusDetailRank"+(index+1), this.getTypeCode()));
        }
        else{
            titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acLotusDetailRank4", this.getTypeCode()), [""+data.minRank, ""+data.maxRank]);
        }
