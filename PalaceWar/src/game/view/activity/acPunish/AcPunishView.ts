...
class AcPunishView extends AcCommonView {
	...
	private _descBg:BaseBitmap;
	private _self:any=null;
	private _bigBg:BaseBitmap =null;
	private _dgbone:any =null
	private _dgbone2:any =null
	private _tyitemClip:any =null;

	//code 12 新增
	private _timeBg:BaseBitmap = null;
	private _acTimeTf:BaseTextField = null;
	private _skillBtnList:any[] = [];
	private _energyNum:BaseTextField = null;
	private _isSelectSkill:boolean = false;
	private _critPos:number = 1;
	private _selectPos:number = 1;
	private _boneTw:egret.Tween = null;

	//商铺
	private _shopButton:BaseButton = null;
	...
	protected getTitleBgName():string{
		if (this.getTypeCode() == "12"){
			let resStr = ResourceManager.hasRes("punish_titlebg-"+this.getTypeCode()) ? "punish_titlebg-"+this.getTypeCode() : "punish_titlebg-12";
			return resStr;
		}
	...
	protected initView(): void {
		...
		let params:string[]=App.DateUtil.getLocalTimeZoneTime(cfg.activeTime[0],cfg.activeTime[1]);
	...
	{			
		...
		for(var i:number=0;i<buttonNameArr.length;i++)
		{
			let btncfg =  buttonNameArr[i]; 
			let imgBtn = ComponentManager.getButton(btncfg.name,"",this._self.bottomBtnClickHandler,this._self,[btncfg]);
	...
	public bottomBtnClickHandler(data:any):void
	{
	...
	private showText(): void {
		...
		if (this.getTypeCode() == "12"){
			...
			let topBgStr = ResourceManager.hasRes("punish_infobg-"+this.getTypeCode()) ? "punish_infobg-"+this.getTypeCode() : "punish_infobg-12";
		...
		if (this.code == "1"){
			...
			if(App.CommonUtil.check_dragon()){
				let	dgbone = App.DragonBonesUtil.getLoadDragonBones('acpunishview_dragon',0,"idle");  
				dgbone.playDragonMovie('idle',0);
				dgbone.x = GameConfig.stageWidth / 2;
				dgbone.y = GameConfig.stageHeigth;
				this.addChildToContainer(dgbone);
				this._dgbone = dgbone;
			}
			else{
			...
			let bone = "acpunishview_fist_1";
			let boneName = bone+"_ske";
			if (App.CommonUtil.check_dragon() && ResourceManager.hasRes(boneName)){
				let	dgbone = App.DragonBonesUtil.getLoadDragonBones(bone , 0, "Standby");  
				dgbone.playDragonMovie('Standby',0);
				dgbone.x=320;
				dgbone.y = GameConfig.stageHeigth - 90; 
				dgbone.setScale(0.85);
				this.addChildToContainer(dgbone);
				this._dgbone = dgbone;
			}
			else{
		...
		let newScoreBg:BaseBitmap = null;
		if (this.getTypeCode() == "12"){
		...
		if (this.getTypeCode() == "12"){
			...
			//活动介绍
			let acDesc = ComponentManager.getTextField(LanguageManager.getlocal("acPunishDesc-"+this.getTypeCode()), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_LIGHT_YELLOW);
			acDesc.width = 600;
		...
		if(this.code == "3")
		{
			...
			let buyBtn = ComponentManager.getButton("punish9shop","",this.butItemBtnCilck,this);
			App.DisplayUtil.setLayoutPosition(LayoutConst.lefttop, buyBtn, bottomBg, [0,-buyBtn.height]);
			this.addChild(buyBtn);
			this._shopButton=buyBtn;

			let bossman = BaseBitmap.create(`punish_bossman`);
		...
		{	
			if(this.code == '9'){
				let imgBtn = ComponentManager.getButton(`reunion_rank`,"",this.rankCilck,this);
		...
		let lvStr = "";
		let hp = cfg.getHp(this._acVo.day);
		if(this.code == "2")
		{
		...
		//底部按钮
		if (this.getTypeCode() == "12"){
			let shopBtnRes = ResourceManager.hasRes("punish_shop_btn-"+this.getTypeCode()) ? "punish_shop_btn-"+this.getTypeCode() : "punish_shop_btn-12";
			let shopBtn = ComponentManager.getButton(shopBtnRes, "", this.shopBtnClick, this);
			shopBtn.setPosition(30, bottomBg.y + 20);
			this.addChildToContainer(shopBtn);
			this._shopButton = shopBtn;
			this.checkBtnRed();

			let rewardBtnRes = ResourceManager.hasRes("punish_rank_reward_btn-"+this.getTypeCode()) ? "punish_rank_reward_btn-"+this.getTypeCode() : "punish_rank_reward_btn-12";
			let rewardBtn = ComponentManager.getButton(rewardBtnRes, "", this.rankRewardBtnCilck, this);
			rewardBtn.setPosition(GameConfig.stageWidth - 30 - rewardBtn.width, bottomBg.y + 20);
			this.addChildToContainer(rewardBtn);

			let fightNumInfo = ComponentManager.getTextField(LanguageManager.getlocal("acPunishFightInfo-"+this.getTypeCode()), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
			...
			let fightNum = ComponentManager.getTextField(""+this.vo.energy, TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
	...
	//元宵特效
	private showYanwu():void
	{		
		 
		 let steamBg1: BaseBitmap = BaseBitmap.create("steambg1");
		 steamBg1.width =379;
		 steamBg1.height =179;
		 ...
		 let steambg2: BaseBitmap = BaseBitmap.create("steambg2");
		 steambg2.width =379;
		 steambg2.height =179;
		...
		{	
			let itemClip = ComponentManager.getCustomMovieClip("steam_",10,80);  
			itemClip.width =164;
			itemClip.height =170; 
	...
	{	
		...
		if(PlatformManager.checkIsEnLang() || PlatformManager.checkIsRuLang())
		{
			...
			let heightDesc = this._boxDesc.height;
	...
	private doYanHuaAni()
	{
		...
		let lightSp = BaseLoadBitmap.create("itemicon"+idx);
	...
	private dragonHitAni():void{
		...
		let lightSp = BaseBitmap.create(`punish9boom${this._selectIndex < 2 ? 1 : 2}`);
		let lightContainer = new BaseDisplayObjectContainer();
		...
		egret.Tween.get(lightContainer,{loop:false}).wait(100).to({scaleX:1,scaleY:1},100).call(()=>{
			this.removeChildFromContainer(lightContainer);
			lightContainer = null;
			lightSp = null;
		},this);
		...
		if(this._dgbone){
			...
			if(this._headPic){
				...
				egret.Tween.get(this._headPic,{loop:false}).wait(500).call(()=>{
					this._headPic.texture = ResourceManager.getRes("punish_boss9")
				},this);
			}
			let key = (this._selectIndex+1).toString();
	...
	private dragonAni(){
		let view = this;
		let idx = this._selectIndex;
		let lightSp = BaseBitmap.create("punish9item"+(idx+1));
		let lightContainer = new BaseDisplayObjectContainer();
		lightContainer.anchorOffsetX = lightSp.width/2;
		lightContainer.anchorOffsetY = lightSp.height/2;
		...
		lightSp.alpha = 0;
		egret.Tween.get(lightSp,{loop:false}).to({alpha:1},200).wait(300).call(()=>{
			// this._touchSwitch = true;
		},this);
		egret.Tween.get(lightContainer,{loop:false}).
		to({
		...
		if (data.data.cmd == NetRequestConst.REQUEST_ACTIVITY_USEPUNISHITEM) {
			...
			if (this.getTypeCode() == "12"){
				...
				if (this._critPos == this._selectPos){
					//飘暴击
					let boomPic:BaseBitmap = BaseBitmap.create("atkrace_crit_text");
					boomPic.anchorOffsetX=boomPic.width/2;
					boomPic.anchorOffsetY=boomPic.height/2;
					boomPic.setPosition(GameConfig.stageWidth/2, 550);
					this.addChildToContainer(boomPic);
					boomPic.setScale(0.7);
					egret.Tween.get(boomPic).wait(100).to({scaleX:1.1 ,scaleY:1.1 , y:boomPic.y - 80}, 100).wait(200).to({scaleX:0.7 ,scaleY:0.7, alpha:0}, 150).call(()=>{
						// if (this._dgbone){
	...
	private getItem(data):BaseDisplayObjectContainer
	{
		...
		let hasNum:number = Api.itemVoApi.getItemNumInfoVoById(Number(data.item));
		let numLb:BaseTextField = ComponentManager.getTextField( hasNum.toString(),18);
	...
	private showNewList():void{
		for (let i=0; i < 4; i++){
			let skillContainer = new BaseDisplayObjectContainer();
			this.addChildToContainer(skillContainer);
			let btnImg = ResourceManager.hasRes("punish_skill-"+this.getTypeCode()+"_"+(i+1)) ? "punish_skill-"+this.getTypeCode()+"_"+(i+1) : "punish_skill-12_"+(i+1);
			let skillBtn = ComponentManager.getButton(btnImg, "", this.skillBtnClick, this, [i]);
			...
			let btnEffect = ComponentManager.getCustomMovieClip("acpunish_fireeffect", 10, 70);
			...
			let skillBtnData = {skillContainer:skillContainer, skillBtn:skillBtn, btnEffect:btnEffect};
			this._skillBtnList[i] = skillBtnData;
		}
	}

	private skillBtnClick(index:number):void{
		App.LogUtil.log("this._isSelectSkill: "+this._isSelectSkill);
		// if (this._isSelectSkill){
			// return;
		// }
		App.LogUtil.log("skillBtnClick: "+index);
		let needEnergy = this.vo.config.needEnergy;
		...
		if (this._dgbone){
			...
			this._boneTw = null;
			let actName = this.getBoneActByType(index+1);
			this._dgbone.playDragonMovie(actName, 1);
			this._dgbone.setDragonBoneEventListener(dragonBones.EventObject.COMPLETE, ()=>{
				// this._isSelectSkill = false;
				if (this._boneTw){
					egret.Tween.removeTweens(this._dgbone);
					this._boneTw = egret.Tween.get(this._dgbone).wait(1000).call(()=>{
						this._dgbone.playDragonMovie("Standby", 0);
						this._boneTw = null;
					});
				}
			...
			let bodyPic = "punish_role-"+this.getTypeCode()+"_"+(index+1);
			...
			egret.Tween.get(this._headPic).wait(1000).call(()=>{
				this._headPic.setRes("punish_role-"+this.getTypeCode()+"_0");
			});
		}
	...
	private getBoneActByType(index:number):string{
		let act = "Standby";
		if (index == 1){
			act = "fist";
		}
	...
	private refreshSkillState():void{
		//新增
		let state = this.vo.inDayOpenState();
		App.LogUtil.log("state:**** "+state);
		for (let i=0; i < this._skillBtnList.length; i++){
			if (state == 1){
	...
	public buyCallback(event: egret.Event): void {
		...
		for (var index = 0; index < this._punishItemList.length; index++) {
			...
			let numBg =  <BaseBitmap>this._punishItemList[index].getChildByName("numBg");
	...
	private checkBtnRed():void
	{
		if (this._acVo.checkHasGoldenTimes())
		{
			...
			let red = this._shopButton.getChildByName("reddot");
