 ...
 */
class AcThreekingdomsRechargeAchieveScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acThreekingdomsRechargekillItemInfo-" + this.getTypeCode(code), [String(data.specialnum)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let progressBar = ComponentManager.getProgressBar("progress3", "progress3_bg", 375-20);
		progressBar.setPosition(rewardbg.x + 2, rewardbg.y + rewardbg.height + 32);
		this.addChild(progressBar);
		let percent = vo.getProcessNum() / data.specialnum;
        let textStr = LanguageManager.getlocal("acThreekingdomsRechargeKillProNum-"+this.getTypeCode(code), [""+vo.getProcessNum(), ""+data.specialnum]);
