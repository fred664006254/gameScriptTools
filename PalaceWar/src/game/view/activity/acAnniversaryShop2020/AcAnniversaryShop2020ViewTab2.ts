 ...
 */
class AcAnniversaryShop2020ViewTab2 extends AcCommonViewTab{
    ...
    public initView(){
        ...
        let bgStr = ResourceManager.hasRes("anniversaryshop_siglegfit_bottombg-"+this.getTypeCode()) ? "anniversaryshop_siglegfit_bottombg-"+this.getTypeCode() : "anniversaryshop_siglegfit_bottombg-1";
        let bg = BaseBitmap.create(bgStr);
        bg.width = this.width;
        bg.height = this.height - 10;
        this.addChild(bg);

        let dataList = this.cfg.shop2DataList;
        App.LogUtil.log("datalist.lenth:"+dataList.length + "this.height: "+this.height);
        let rect =  new egret.Rectangle(0, 0, this.width - 12, this.height - 35);
        let scrollList = ComponentManager.getScrollList(AcAnniversaryShop2020Tab2ScrollItem, dataList, rect, {aid:this.aid, code:this.code, dataLength: dataList.length -1});
    ...
    private refreshView():void{
        if (this._scrollList){
            let data = this.cfg.shop2DataList;
