/**
  ...
  */
 class AcRecoveryDetailTab1ScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let vo = <AcRecoveryVo>Api.acVoApi.getActivityVoByAidAndCode(this._aidAndCode.aid, this._aidAndCode.code);
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acRecoveryChargeItem", [String(this._itemData.needGem)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		//进度条
		let progress = ComponentManager.getProgressBar("progress5", "progress3_bg", 340);
		...
		let progressStr = LanguageManager.getlocal("acRecoveryProcessNum-"+this.getTypeCode(), [String(currChargeGem), String(data.needGem)]);
