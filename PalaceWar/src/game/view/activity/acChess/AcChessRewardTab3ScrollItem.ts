 ...
 */
class AcChessRewardTab3ScrollItem extends ScrollListItem
{
    ...
    private get vo() : AcChessVo{
        return <AcChessVo>Api.acVoApi.getActivityVoByAidAndCode(this.code.aid, this.code.code);
    }
    ...
    {	
        ...
        let titleTxt = ComponentManager.getTextField(LanguageManager.getlocal("chessquestType"+data.questType, [String(data.value)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        titleTxt.x = bottom2.x+10;
        titleTxt.y = bottom2.y+13; 
        this.addChild(titleTxt);          

        let currNum = this.vo.getTaskNumByType(data.fid,data.sid);
        let needStrKey = "acchess_taskprocess";
        ...
        let needStrKey2 = "acchess_taskpro";
        // if (currNum >= data.value){
        ...
        if (currNum >= data.value){
            if (this.vo.isGetTaskById(data.fid,data.sid)){ 
                ...
                let reviceBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_CHESS_GETTASK, { activeId: this.vo.aidAndCode, pos: [data.fid,data.sid] });
				}, this);
