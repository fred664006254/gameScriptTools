/**
  ...
  */
 class AcChessRewardTab1ScrollItem extends ScrollListItem {
		...
		if (this.vo.isGetRechargeById(data.id)) {
			...
			if (currChargeGem >= data.needGem) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_CHESS_GETRECHARGE, { activeId: this.vo.aidAndCode, rkey: Number(data.id) });
				}, this);
