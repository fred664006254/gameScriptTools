class AcDrinkTeaRewardTab1ScrollItem extends ScrollListItem {
		...
		if (this.vo.isGetRecharge(String(data.id))) {
			...
			if (currChargeGem >= data.needGem) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_DRINKTEA_GETRECHARGE, { activeId: this.vo.aidAndCode, rkey: Number(data.id) });
				}, this);
