//衣装预览
class AcCrossServerIntimacyCheerViewTab1 extends CommonViewTab
{
	...
	protected initView():void
	{
        ...
        let bg = BaseLoadBitmap.create(App.CommonUtil.getResByCode(`crossserverintibg`, this.getUiCode()));
        this.addChild(bg);
        ...
        let skinEff = App.CommonUtil.getWifeSkinFlagById(skinId);
        if (skinEff){
        ...
        //skin info
        let skinGetInfo = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acCrossServerImacySkinGet", this.getUiCode(), "7")), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_BROWN);
        ...
        let exchangeTip = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acCrossServerImacySkinChangeInfo", this.getUiCode(), "7"), [""+toolItemVo.num, toolItemVo.name]), TextFieldConst.FONTSIZE_ACTIVITY_COMMON, TextFieldConst.COLOR_BROWN);
    ...
    private showSkinDetailInfo():void{
        ...
        container.addChild(skinTipTxt);
			...
			let addValues = Api.wifeSkinVoApi.getWifeSkinProAdd(skinCfg.id, true);
			let txt = [
				{
