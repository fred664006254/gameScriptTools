 ...
 * 奖池item
 */
class AcWeaponPrisonPopupScrollItem3 extends ScrollListItem {
    ...
    protected initItem(index: number, data: any, itemParam: any) {
        ...
        //title txt
        let titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acWeaponPrisonPoolTitle"+data.id, this.getTypeCode()));
        ...
        if (data.id == poolId){
            ...
            let selBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, App.CommonUtil.getCnByCode("acGoodMatchPoolSelect", "1"), ()=>{
                if (!this.vo.isInActivity()){
                    this.vo.showAcEndTip();
                    return ;
                }
                if (this.vo.getPoolRewardId()!=0){
                    App.CommonUtil.showTip(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acWeaponPrisonPoolTip3", this.getTypeCode())));
                    return ;
                }
                NetManager.request(NetRequestConst.REQUEST_WEAPONPRISON_SETTYPE, {activeId: this.vo.aidAndCode, rtype: data.id});
            }, this);
