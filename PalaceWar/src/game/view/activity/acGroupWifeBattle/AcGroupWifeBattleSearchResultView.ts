...
class AcGroupWifeBattleSearchResultView  extends BaseView
{
    ...
    private showInitView()
    {
        ...
        let title = BaseBitmap.create("acgroupwifebattle_enemytip");
        ...
        if (App.CommonUtil.check_dragon() && !Api.switchVoApi.checkCloseBone() && RES.hasRes("hydzsz_ske")) {
            ...
            let shanzi = BaseLoadBitmap.create("acgroupwifebattlesearch_bg");
            shanzi.width = 640;
            shanzi.height = 418;
        ...
        if (App.CommonUtil.check_dragon() && !Api.switchVoApi.checkCloseBone() && RES.hasRes(boneName)  && Api.wifeVoApi.isHaveBone(boneName)) {
            let mask = BaseBitmap.create("acgroupwifebattle_enemymask");
            mask.name = "mask";
            ...
            .to({y:728-35,alpha:1},200).call(()=>{
                let talkBg = BaseBitmap.create("acgroupwifebattle_talkbg"); 
                ...
                let closeText:BaseTextField=ComponentManager.getTextField(LanguageManager.getlocal("acGroupWifeBattle_clickToFight"),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
