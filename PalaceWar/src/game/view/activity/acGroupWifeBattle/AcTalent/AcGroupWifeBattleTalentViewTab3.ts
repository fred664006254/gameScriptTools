 ...
 * author qianjun----wxz
 */
class AcGroupWifeBattleTalentViewTab3 extends CommonViewTab {
	...
	public initView():void
	{
		...
		let acTime = this.vo.getAcLocalTime(true);
		...
		let tiptxt = ComponentManager.getTextField(LanguageManager.getlocal("acGroupWifeBattleTalentTip1"), 20, TextFieldConst.COLOR_BLACK);
		...
		if(this.checkHaveBuff()){
			...
			if(curlv < wifeBattleBuff.length){
				...
				nextAdd = wifeBattleBuff[wifeBattleBuff.length-1].rankBuff;
				lvStr2 = LanguageManager.getlocal("wifeTalent_bufftxt2",["1",""+nextAdd]);
				percV = 1.0;
				percStr =paramStr;
			}
		...
		let rect = new egret.Rectangle(0,0,title.width+10,720-(1136-GameConfig.stageHeigth));
		...
		if(this.vo.wifebattlecross && this.vo.wifebattlecross.info && this.vo.wifebattlecross.info.tmpattr && this.vo.wifebattlecross.info.tmpattr.wifeadd_arr){
			for(let i in this.vo.wifebattlecross.info.tmpattr.wifeadd_arr){
				let unit = this.vo.wifebattlecross.info.tmpattr.wifeadd_arr[i];
				let level = Number(Api.wifestatusVoApi.getWifestatusLevelById(unit.wid));
				if(level == 1){
					continue;
				}
				infoList2.push(unit);
