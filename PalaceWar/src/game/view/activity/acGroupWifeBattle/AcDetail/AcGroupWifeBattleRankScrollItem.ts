...
class AcGroupWifeBattleRankScrollItem extends ScrollListItem {
	...
	protected initItem(index: number, data: any,bigdata:any) 
	{
		...
		// scoreDesc.x = 544;
		scoreDesc.text = (data.value ? data.value : (data.alive ? 0 : (LanguageManager.getlocal(`acGroupWifeBattleOut-1`))))+"";
		if(data.value == -9999)
		{
			// if(this.vo.getCurperiod() < 2)
			// {
			// 	scoreDesc.text = LanguageManager.getlocal(`acGroupWifeBattleRankNoAttend-1`);
			// }else
			// {
			// 	scoreDesc.text = LanguageManager.getlocal(`acGroupWifeBattleRankOut-1`);
			// }
			scoreDesc.text = LanguageManager.getlocal(`acGroupWifeBattleRankOut-1`);
		}
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~type 2 状态~~~~~~~~~~~~~~~~~~~~~~~~~~~
		...
		{	
			...
			if(data.alivemn==0 && this.vo.getCurperiod() > 1)
			{	
				scoreDesc.text = LanguageManager.getlocal("acGroupWifeBattleOut-1");
			...
			{
				scoreDesc.text = LanguageManager.getlocal("acBattlealivemn",[data.mn,data.mn]);
			}
		}
		//区服
		if(this._type==2)
		...
		if(this._type==1)
		{
			...
			// {	
				...
				if((this.vo.getCurperiod() >= 2 && data.value == -9999))
				{
					scoreDesc.text = LanguageManager.getlocal(`acGroupWifeBattleOut-1`);
					scoreDesc.textColor = 0xaa825e;
					teliminateDesc.textColor =scoreDesc.textColor;
