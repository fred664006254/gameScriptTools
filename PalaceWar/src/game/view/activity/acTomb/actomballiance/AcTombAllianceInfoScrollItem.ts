 ...
 * desc : 帮会敌情itemrender
 */
class AcTombAllianceInfoScrollItem extends ScrollListItem
{
    ...
    {	
		...
		let name = data.type == 1 ? LanguageManager.getlocal('acwipeBossAllNpcInfo', [cfg.getnpcName(code), str.toString()]) : cfg.getnpcName(code);
		...
		if(data.type == 1){
			let posTxt = ComponentManager.getTextField(LanguageManager.getlocal(`loctombpos-${code}`, [data.x,data.y]), 18);
	...
	private fightHandler(evt:egret.TouchEvent):void
	{
		...
		if(boxdata){
			...
			let infoview = ViewController.getInstance().getView('AcTombAllianceInfoView');
			infoview.hide();
		}
		else{
			let arr = [];
