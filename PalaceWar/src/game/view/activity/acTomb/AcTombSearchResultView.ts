 ...
 * desc:东海皇陵宝箱飞行展示画面
*/
class AcTombSearchResultView extends CommonView
{
	...
	public initView():void{	
		...
		let scanEffect = ComponentManager.getCustomMovieClip(`tombboxscan${code}-`, 10, 70);
		...
		let keyTxt = ComponentManager.getTextField(`(${LanguageManager.getlocal(`tombbossnameKey${data.foeId}-${code}`)}${keynum}/1)`, 22, 0xff3c3c);
		...
		if(from){
			...
			egret.Tween.get(Img).to({scaleX : 1, scaleY : 1, x : tmpX, y : tmpY}, 500).call(()=>{
				let light = BaseBitmap.create(`tomboxhighlight-${code}`);
				...
				let circle = BaseBitmap.create(`tombboxcircle${data.foeId}-${code}`);
