//
class AcTombRewardViewTab3 extends CommonViewTab
{
	...
	protected initView():void
	{
		...
		let rankList : Config.AcCfg.TombFoeItemCfg[] = view.vo.getArr('foe');
		rankList.sort((a,b)=>{
			if(a.type == b.type){
				return b.id - a.id;
			}
		...
		for (let index = 0; index < rankList.length; index++){
			...
			let winbg = BaseBitmap.create(`tombrewardrankbg-${view.getUicode()}`);
			winbg.y = scroStartY;
			...
			let txt = ComponentManager.getTextField(rItem.getnpcName(view.getUicode()),24,TextFieldConst.COLOR_LIGHT_YELLOW);
