 ...
 * desc:东海皇陵海底寻宝界面
*/
class AcTombSeaView extends CommonView
{
	...
	{	
		...
		let boatscan = ComponentManager.getCustomMovieClip(`tombboatscan${code}-`, 13, 70);
		...
		let boatalpha = BaseBitmap.create(`tombrankalpha-${code}`);
		...
		//海平面特效
		let sealine = ComponentManager.getCustomMovieClip(`tombsea${code}-`, 12, 130);
		sealine.width = 639;
		sealine.height = 37;
		...
		if(qulaity){
			...
			let scrollList = ComponentManager.getScrollList(AcTombSeaFloorItem, arr, rect, view.code, 30);
			...
			let str = LanguageManager.getlocal(`tombtimetip${period}-${code}`, [App.DateUtil.getFormatBySecond(view.vo.getCountDownTime())]);
			let wordsText:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal(`tombnoattend-${code}`, [str]), TextFieldConst.FONTSIZE_CONTENT_COMMON);
		...
		let top2 = BaseBitmap.create(`tombbg5-${code}`);
        ...
        if(1){
            ...
            if(!showStr)
            {
				...
				let emoticonStr = Api.emoticonVoApi.chatStrChangeLocal(showStr.content.message);
	...
	private showBublle():void{
		...
		*/
		let num = 15;
		for(let i = 0; i < num; ++ i){
			let bubble = BaseBitmap.create(`shuizhu0${code}`);
	...
	protected getRuleInfo():string{
		let rule = `tombrule-${this.getUicode()}`;
	...
	private enterInHandler() : void{
		...
		if(unit.num){
			...
			let message = LanguageManager.getlocal(`tombbuysearchtip-${code}`,[String(needNum)]);
			let mesObj = {
				confirmCallback:()=>{
					let searchBuyNum = view.vo.getBuySearchNum();
					let needNum = view.cfg.buyNumCost[Math.min(searchBuyNum,9)];
	...
	private digCallBack(evt : egret.Event):void{
		...
		if(data && data.map){
			...
			let strList = [];
			if(data.score){
				let flyStr1 = LanguageManager.getlocal("acPunishGetScoreTxt4",[String(data.score)]);
				strList.push({tipMessage:flyStr1});
			}

			if(data.shopscore){
				let flyStr2 = LanguageManager.getlocal("acPunishGetScoreTxt3",[String(data.shopscore)]);
