/**
  ...
  */
 class AcWeaponMazeDetailPopupScrollItem1 extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acWeaponMazeDetailRechargeItemInfo", this.getTypeCode()) , [String(this._itemData.needGem)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let currChargeGem = this.vo.getRechargeNum();
		...
		if (this.vo.isGetRechargeRewardById(data.id)) {
			...
			if (currChargeGem >= data.needGem) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					NetManager.request(NetRequestConst.REQUEST_ACWEAPONMAZE_GETRECHARGE, { activeId: this.vo.aidAndCode, rkey: data.id});
				}, this);
