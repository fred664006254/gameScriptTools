class AcWeaponHousePopupViewTab1ScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acDrinkTeaRechargeItemInfo", this.getTypeCode()) , [String(this._itemData.needGem)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		if (this.vo.isGetRecharge(String(data.id))) {
			...
			if (currChargeGem >= data.needGem) {
				...
				let reviceBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, "taskCollect", () => {
					...
					App.LogUtil.show("servertime:"+GameData.serverTime+"-----"+"endtime:"+this.vo.et);	
					NetManager.request(NetRequestConst.REQUEST_WEAPONHOUSE_GETRECHARGE, { activeId: this.vo.aidAndCode, rkey: Number(data.id) });
				}, this);
