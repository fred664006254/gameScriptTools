 ...
 */
class AcWeaponHouseRankPopupViewTab2 extends CommonViewTab{
    ...
    public initView():void{
        ...
        let dataList = this.cfg.getRankAllItemList();
        let rect = new egret.Rectangle(0, 0, 530, 570);
        let scrollList = ComponentManager.getScrollList(AcWeaponHouseRankScrollItem2, dataList, rect, {aid:this.aid, code:this.code});
        ...
        let myRankTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acWeaponHouse_gang_title", this.getTypeCode()), [String(myRank)]),TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        myRankTF.setPosition(bottomBg.x + 40, bottomBg.y + 30);
        this.addChild(myRankTF);
    
        //积分
        let myScoreTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acWeaponHouse_rankingList_title", this.getTypeCode()), [String(score)]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
        myScoreTF.setPosition(myRankTF.x, myRankTF.y + myRankTF.height + 15);
        this.addChild(myScoreTF);

        //rank btn
        let rankBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, App.CommonUtil.getCnByCode("acWeaponHouse_rankAllList_title", this.getTypeCode()), ()=>{
            if (!this.vo.isStart){
                this.vo.showAcEndTip();
                return;
            }
            ViewController.getInstance().openView(ViewConst.POPUP.ACWEAPONHOUSERANKSHOWPOPUPVIEW, {tab:2,aid: this.aid, code: this.code});
        }, this);
