class AcWeaponHousePopupViewTab2ScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let currChargeGem = this.vo.getTask(this._itemData.id1, Number(this._itemData.id2)-1 );
		let value:any;
		let title:string;
		if(data.needNum)
		{
		...
		let tasknum = Object.keys(this.cfg.scheduleAll[this._itemData.id1]).length;
	...
	private collectHandler(evt:egret.TouchEvent):void
	{
		...
		let taskNum = vo.getTask(this._itemData.id1, Number(this._itemData.id2)-1 );
