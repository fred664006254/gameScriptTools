 ...
 * author wxz
 * date 2020.8.4
 * @class AcFlowerMoonRewardPopViewTab4
 */
class AcFlowerMoonRewardPopViewTab4 extends CommonViewTab
{
    ...
    public initView():void
    {
        ...
        App.MessageHelper.addNetMessage(NetRequestConst.REQUEST_ACFLOWERMOON_EXCHANGE2, this.changeHandle, this);

        let container = new BaseDisplayObjectContainer();
        container.width = GameConfig.stageWidth-60;
					
        let skinId = this.cfg.change[1].getReward.split("_")[1];
        ...
        let str:string = this.cfg.change[1].needItem;
        let itemCfg : Config.ItemItemCfg = Config.ItemCfg.getItemCfgById(str.split("_")[1]);
        let showstr = LanguageManager.getlocal(`acFlowerMoonExchangeTopMsg2`, [str.split("_")[2],itemCfg.name]);        
        ...
        let exchangeBg = BaseBitmap.create('public_scrolllistbg');
        exchangeBg.width = bg.width;
        exchangeBg.height = 70;
        App.DisplayUtil.setLayoutPosition(LayoutConst.horizontalCenterbottom, exchangeBg, bg, [0,0]);
        container.addChild(exchangeBg);	       

        let itemicon = BaseLoadBitmap.create("itemicon" + itemCfg.id);
        itemicon.width = 60;
        itemicon.height = 60;
        itemicon.setPosition(buttomBg.x+7, exchangeBg.y + exchangeBg.height/2-itemicon.height/2);
        ...
        let btn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "acFlowerMoonExchangeBtnTxt", () => {
            ...
            NetManager.request(NetRequestConst.REQUEST_ACFLOWERMOON_EXCHANGE2, { activeId: this.vo.aidAndCode });
        }, this);
	...
	{
        let change:string = this.cfg.change[1].needItem;
    ...
    public dispose(){
        ...
        App.MessageHelper.removeNetMessage(NetRequestConst.REQUEST_ACFLOWERMOON_EXCHANGE2, this.changeHandle, this);         
    }
}