 ...
 * @author wxz
 */
class AcFlowerMoonRewardTab2ScrollItem extends ScrollListItem {
    ...
    protected initItem(index: number, data: any, itemParam: any) {
        ...
        //title txt
        let titleTxtStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acFlowerMoonPoolTitle"+data.id, this.getTypeCode()));
        ...
        rewards = "6_"+this.cfg.change[0].needItem.split("_")[1]+"_1|" + rewards;
        ...
        if (data.id == poolId){
            ...
            let selBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, App.CommonUtil.getCnByCode("acFlowerMoonPoolBtn", this.getTypeCode()), ()=>{
                if (!this.vo.isInActivity()){
                    this.vo.showAcEndTip();
                    return ;
                }
                NetManager.request(NetRequestConst.REQUEST_ACFLOWERMOON_SETTYPE, {activeId: this.vo.aidAndCode, rtype: data.id});
            }, this);
            selBtn.setPosition(bg.x + bg.width - selBtn.width - 15, bg.y + bg.height/2 - selBtn.height/2 + 20);
            this.addChild(selBtn);
            if (poolId > 0){
                selBtn.setEnable(false);
            }
            else{
                selBtn.setEnable(true);
            }
        }
