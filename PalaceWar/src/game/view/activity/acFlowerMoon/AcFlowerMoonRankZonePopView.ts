 ...
 * author wxz
 * date 2020.8.4
 * @class AcFlowerMoonRankZonePopView
 */
class AcFlowerMoonRankZonePopView extends PopupView{
    ...
    public initView():void
    {
        ...
        // 玩家昵称  /  区服
        let rankPlayNameTF  = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acFlowerMoonRankListTopZone", this.getTypeCode())),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        //排行榜的ScrollList
        let data = this._rankData.zranks;
		let rect = new egret.Rectangle(0, 0, topContainer.width, bg.height - topContainer.height-30);
		let rankScrollList = ComponentManager.getScrollList(AcFlowerMoonRankZoneItem, data, rect, {aid: this.aid, code: this.code});
        ...
        //昵称
        let nickName = Api.mergeServerVoApi.getAfterMergeSeverName();
        let rankNiceNameTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acFlowerMoonRankListPlayerZone", this.getTypeCode()), [nickName]), TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let myRank:any = null;
		let score = 0;
        if(this._rankData && this._rankData.myzrank)
        {
            myRank = this._rankData.myzrank;
            if (myRank > 10000){
                myRank = "10000+";
            }
            if (this._rankData.myzscore){
                score = this._rankData.myzscore;
            }
        }
        else{
            //未上榜
		...
		let tip = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acFlowerMoonRankListTip2", this.getTypeCode())), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_LIGHT_YELLOW);
