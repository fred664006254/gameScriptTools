class AcFlowerMoonRankPersonItem extends ScrollListItem
{
    ...
    public initItem(index:number, data:any, itemParam:any)
    {
        ...
        name.x = 205 - name.width/2;
        name.y = offHeight/2 - name.height/2;
        this.addChild(name);

        let zone = ComponentManager.getTextField(LanguageManager.getlocal("ranserver2",[data.zid+""]), TextFieldConst.FONTSIZE_CONTENT_COMMON, tarColor);
        zone.x = 315 - zone.width/2;
        zone.y = offHeight/2 - zone.height/2;
        this.addChild(zone);        

        let score = ComponentManager.getTextField("", TextFieldConst.FONTSIZE_CONTENT_COMMON, tarColor);
        if(data.score > 0){
            score.text = App.StringUtil.changeIntToText(data.score, 1);
