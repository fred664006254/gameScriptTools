 ...
 */
class AcThreekingdomsOfWifeDetailTab2ScrollItem extends ScrollListItem {
	...
	public initItem(index: number, data: any, itemParam: any): void {
		...
		let id = itemParam.id;
		let vo = <AcThreekingdomsOfWifeVo>Api.acVoApi.getActivityVoByAidAndCode(aid, code);
		this.width = 530;
		...
		let titleTF = ComponentManager.getTextField(LanguageManager.getlocal("acThreekingdomsOfWifeAchieveRewardTitle-" + this.getTypeCode(code), [String(data.specialnum)]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        let textStr = LanguageManager.getlocal("acThreekingdomsOfWifeAchieveCurrProNum-"+this.getTypeCode(code), [""+vo.getProcessNum(), ""+data.specialnum]);
