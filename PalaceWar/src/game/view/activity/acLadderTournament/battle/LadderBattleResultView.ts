class LadderBattleResultView extends BaseView
{
    ...
    {
        //1胜利 2失败
        let score:number[] = Api.laddertournamentVoApi.reportVoApi.getAllBattleResult();
        let winflag = score[0]>score[1]? 1 : 2;
        let point = Api.laddertournamentVoApi.reportVoApi.score;
        ...
        let tip:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal("acLadder_BattleResult"+winflag,[String(score[0]),String(score[1])]),22,TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        let scoreTip = ComponentManager.getTextField(LanguageManager.getlocal("acLadder_BattleResultScore"+winflag,[String(point)]),22,0x17f925);
        ...
        {   
            let getcfg:Config.AcCfg.LTBattleGetCfg;
            let str:string = "";
            if(winflag == 1)
            {   
                getcfg = <Config.AcCfg.LTBattleGetCfg>Api.laddertournamentVoApi.cfg.battleGet["1"];
            }
            else
            {
                getcfg = <Config.AcCfg.LTBattleGetCfg>Api.laddertournamentVoApi.cfg.battleGet["2"];
            }
            if (getcfg)
            {
            ...
            let getpoint = ComponentManager.getTextField(str,22,0x17f925);
