class AcRedPackView extends PopupView
{
    ...
    private get vo() : AcRedPackVo{
        return <AcRedPackVo>Api.acVoApi.getActivityVoByAidAndCode(this.aid,this.code);

    }
    private  get cfg ():Config.AcCfg.RedPackCfg
    {
		let cfg = <Config.AcCfg.RedPackCfg>Config.AcCfg.getCfgByActivityIdAndCode(this.aid, this.code);
		return cfg;
	}
	...
	{	
        ...
        if (!Api.switchVoApi.checkOpenBMFont()){
            let goldText = <BaseTextField>this.goldText;
            goldText.bold = true;
            this.goldText.y = this.goldbg.y +55;

            let residueCount = <BaseTextField>this.residueCount;
        ...
        let t = this.vo.getAcResidueTime();
        if(t < 0){
            t = 0;
        }
    ...
    private initScroll():void
    {
        ...
        if(!this.loginfo ||!this.loginfo.length ){
            let emptyText = ComponentManager.getTextField(LanguageManager.getlocal("acRedPackEmpty"),22,0xe7d98f);
		...
		for(let i = 0;i < this.loginfo.length; i ++){
            ...
            let nameText = ComponentManager.getTextField(LanguageManager.getlocal("acRedPackLoop",[infoObj["lname"],infoObj["gemnum"].toString()]),22,0xe7d98f);
            nameText.x = 40;
