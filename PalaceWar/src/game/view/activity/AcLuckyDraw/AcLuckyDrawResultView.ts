...
class AcLuckyDrawResultView extends PopupView
{   
    ...
    protected initView():void
    {
		...
		let data = view.param.data.batchList;
		let info = [];
		for(let i in data){
			info.push({
				card : data[i][1],
		...
		{	
			...
			let resultStr:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal(`acLuckyDrawGetAllItemInfo-${view.code}`,[String(index), unit.card, rewardStr]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
