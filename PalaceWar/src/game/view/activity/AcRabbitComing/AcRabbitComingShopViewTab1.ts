...
desc : 累积充值
*/
class AcRabbitComingShopViewTab1 extends CommonViewTab
{
	//滑动列表
	...
	{	
		...
		let scrollList = ComponentManager.getScrollList(AcRabbitComingShopRechargeItem,arr,tmpRect,view.code);
	...
	private rewardCallBack(evt : egret.Event):void{
		...
		if(evt.data.ret){
			...
			let str =  `1050_1_${cfg.specialGift}`;
			if(rewards){
