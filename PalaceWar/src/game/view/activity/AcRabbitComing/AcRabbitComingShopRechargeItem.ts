 ...
 */
class AcRabbitComingShopRechargeItem  extends ScrollListItem
{
    //item数据
    ...
    private _rechargeItem : Config.AcCfg.RabbitComingRechargeItemCfg = null;
	...
	protected initItem(index:number,data:Config.AcCfg.RabbitComingRechargeItemCfg,itemparam:any)
    {	
        let view = this;
        ...
        let str = `1050_1_${data.specialGift}`;
    ...
    protected refreshBtnStatus()
    {
        ...
        else
        {
            if (chargeTotal >= this._rechargeItem.needGem)
            {
                let collectBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW,"ac_recharge_Btntxt2",this.eventCollectHandler ,this);        
