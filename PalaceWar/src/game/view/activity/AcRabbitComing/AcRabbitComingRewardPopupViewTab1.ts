class AcRabbitComingRewardPopupViewTab1 extends CommonViewTab{  
    ...
    protected initView():void{
        ...
        let vo = this.vo;
		let arr = vo.getArr("individualRank");//
 		let tmpRect =  new egret.Rectangle(0,0,530,Bg.height - 10);
		let scrollList = ComponentManager.getScrollList(AcRabbitComingPRankItem,arr,tmpRect,view.code);    
		...
		let rankTxt = ComponentManager.getTextField(LanguageManager.getlocal(`acBattleRoundRank-1`, [String(0xffffff), str]), 24, TextFieldConst.COLOR_LIGHT_YELLOW);
		view.addChild(rankTxt);

		let scoreTxt = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode(`acrabbitcomingtip7`,view.getUiCode()), [view.vo.getMyPScore().toString()]), 24, TextFieldConst.COLOR_LIGHT_YELLOW);
		...
		let btn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, `acPunishRankTab1`, ()=>{
