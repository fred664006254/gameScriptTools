/**
  ...
  */
class AcMarryRewardScrollItem extends ScrollListItem {
    ...
    public initItem(index: number, data: Config.AcCfg.BattleUseItemCfg, itemParam: any): void {
        ...
        let titleTFStr = "acMarryRewardInfoViewItemTitleType" + data.id;
        if(itemParam.code != "1")
        {
            titleTFStr = "acMarryRewardInfoViewItemTitleType" + data.id + "-" + itemParam.code;
        }
        let titleTF = ComponentManager.getTextField(LanguageManager.getlocal(titleTFStr), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        ...
        let descTFStr = "acMarryRewardInfoViewItemDescType" + data.id;
        if(itemParam.code != "1")
        {
            descTFStr = "acMarryRewardInfoViewItemDescType" + data.id + "-" + itemParam.code;
        }
        let descTF = ComponentManager.getTextField(LanguageManager.getlocal(descTFStr), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BLACK);
