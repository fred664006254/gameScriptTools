...
class Courier2ScrollItem  extends ScrollListItem
{
    ...
    {	
		...
		if (PlatformManager.checkIsTextHorizontal()){
			...
			//中国结
			var chineseknot:BaseBitmap = BaseBitmap.create("accourier_chineseknot2_"+this.nowCode);   
			...
			if(data.questType=="1002")
			{
				...
				//礼包名字 
				let dayNum = Courier2ScrollItem.TADAY;
			...
			{	
				...
				let str = 'acNewYearquestType'+data.questType;
		...
		{	 
			...
			{	
				...
				if(this._tadayTaskTxt){
					let str = 'acNewYearquestType'+this._data.questType;
	...
	{	 
	 	let tmpVo = <AcCourierVo>Api.acVoApi.getActivityVoByAidAndCode(AcConst.AID_COURIER,this._code); 
		let tmpCfg = <Config.AcCfg.NewYearCfg> Config.AcCfg.getCfgByActivityIdAndCode(AcConst.AID_COURIER, this._code);
		if(this._data.day < tmpVo.diffday){
			App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
			return
		}
		...
		else
		{
			...
			let itemName:string = LanguageManager.getlocal("acCourierPackageName"+ Courier2ScrollItem.TADAY+"_"+this.nowCode)
