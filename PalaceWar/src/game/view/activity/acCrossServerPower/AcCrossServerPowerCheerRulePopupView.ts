//规则
class AcCrossServerPowerRulePopupView extends PopupView
{
	...
	public initView():void
	{
        ...
        let msgTF1:BaseTextField = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acCrossserverPowerFlagRule1", this.getUiCode())),20,TextFieldConst.COLOR_BROWN);
		msgTF1.width = 460;
		...
		for(let i = scoreRebate.length - 1 ; i >= 0; i --){
			...
			let msgStr = LanguageManager.getlocal(App.CommonUtil.getCnByCode("acCrossserverPowerFlagRule2", this.getUiCode()) ,[String(rebateCfg.rank[0]), String(rebateCfg.rank[1]),String(rebateCfg.multiplying * this.cfg.flagScoreNum1)]);
