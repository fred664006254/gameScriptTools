 ...
 */
class AcMouseGoldRewardPopViewTab1 extends CommonViewTab{
    ...
    public initView():void{
        ...
        let baseView = <AcMouseGoldRewardPopView>ViewController.getInstance().getView("AcMouseGoldRewardPopView");
        let rankData = baseView.getMyRankData();

        let dataList = this.cfg.getRankItemCfg();
        let rect = new egret.Rectangle(0, 0, 530, 560);
        let scrollList = ComponentManager.getScrollList(AcMouseGoldRewardTab1ScrollItem, dataList, rect, {aid:this.aid, code:this.code});
        ...
        let myRankTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acMouseGoldRankMyRank", this.getTypeCode()), [String(myRank)]),TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
        myRankTF.setPosition(bottomBg.x + 40, bottomBg.y + 30);
        this.addChild(myRankTF);
    
        //积分
        let myScoreTF = ComponentManager.getTextField(LanguageManager.getlocal(App.CommonUtil.getCnByCode("acMouseGoldRankMyScore", this.getTypeCode()), [String(score)]),TextFieldConst.FONTSIZE_CONTENT_COMMON,TextFieldConst.COLOR_LIGHT_YELLOW);
        myScoreTF.setPosition(myRankTF.x, myRankTF.y + myRankTF.height + 15);
        this.addChild(myScoreTF);

        //rank btn
        let rankBtn = ComponentManager.getButton(ButtonConst.BTN2_SMALL_YELLOW, App.CommonUtil.getCnByCode("acMouseGoldDetailRankBtn", this.getTypeCode()), ()=>{
            if (!this.vo.isStart){
                this.vo.showAcEndTip();
                return;
            }
            ViewController.getInstance().openView(ViewConst.POPUP.ACMOUSEGOLDRANKDETAILPOPUPVIEW, {aid: this.aid, code: this.code});
        }, this);
