...
class NewAtkraceCorssRankItem extends ScrollListItem
{
	public constructor() {
		super();
    }
	...
	protected initItem(index:number,data:any)
    {
        ...
        let point = String(data.point);
        if(data.point < -8888)
        {
            point = LanguageManager.getlocal("newatkraceRankLess");
        }

		let scoreTxt = ComponentManager.getTextField(point,TextFieldConst.FONTSIZE_CONTENT_COMMON);
