class NewAtkracecrossArrestView extends CommonView
{	
	...
	{	
		...
		//江湖名望
		let fameBg = BaseBitmap.create("newcrossatkrace_battleinfobg");
		this.addChildToContainer(fameBg);
		fameBg.width = 320;
		fameBg.x = GameConfig.stageWidth/4 - fameBg.width/2;

		let isShowAtkInfo = true;
		let baseAtkStr = "";
		let extraAtkStr = "";
		let acVo = <AcNewCrossServerAtkRaceVo>Api.acVoApi.getActivityVoByAidAndCode("newCrossServerAtkRace", Api.atkracecrossVoApi.newcrossCode);
		if (myAtkInfo.director){
			let myInfo = acVo.getFameCfgByLine(myAtkInfo.director.x);
			if (myAtkInfo.fdirector){
				if (myAtkInfo.director.x < myAtkInfo.fdirector.x){
					baseAtkStr = LanguageManager.getlocal("newatkrackcross_BattleFameAtk", [""+Math.floor(myInfo.baseCfg.baseBuff * 100)]);
					extraAtkStr = LanguageManager.getlocal("newatkrackcross_BattleFameExtraAtk", [""+Math.floor(myInfo.baseCfg.addBuff * 100)]);
				}
					...
					if (myAtkInfo.director.y < myAtkInfo.fdirector.y){
						isShowAtkInfo = true;
						baseAtkStr = LanguageManager.getlocal("newatkrackcross_BattleFameAtk", [""+Math.floor(myInfo.baseCfg.baseBuff * 100)]);
						extraAtkStr = LanguageManager.getlocal("newatkrackcross_BattleFameExtraTip3", [""+Math.floor(myInfo.baseCfg.addBuff * 100)]);
					}
		...
		let fameAtk = ComponentManager.getTextField("", TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WHITE);
		this.addChildToContainer(fameAtk);
		let fameExtraAtk = ComponentManager.getTextField("", TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_LIGHT_YELLOW);
