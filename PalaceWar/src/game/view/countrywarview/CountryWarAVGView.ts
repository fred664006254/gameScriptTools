...
class CountryWarAVGView extends CommonView
{
	...
	private showPage():void
	{
		...
		if (this._curConfig.descId) {
			...
			if (this._curConfig.clickContinue) {
                ...
                let str = `CountryWarAVGTalk${this._curConfig.descId}`;;
                let param = [];
                let titleId = Api.playerVoApi.getTitleid();
                //自己说话
                if(Number(this._curConfig.personPic) == 1){
                    ...
                    //元芳说话 特殊处理
                    if(this._curConfig.needDeal){
                        let callTitle = LanguageManager.getlocal(`playerNpcName4`);
                        if(titleId){
                            let title = Config.TitleCfg.getTitleCfgById(titleId);
                            if(title && title.isTitle == 1 && title.titleType){
			...
			if (this._curConfig.personPic == 1 || this._curConfig.personPic == 999 ) {
                ...
                let servantNpc = 'servant_full_1001';
                let servantId = this._curConfig.personPic;
                let equip = Api.servantVoApi.getservantSkinIdInWear(servantId);
                if(equip){
                    servantNpc = Config.ServantskinCfg.getServantSkinItemById(equip).body;
                }
				
				let npcMan:BaseLoadBitmap = BaseLoadBitmap.create(servantNpc,rect1);
