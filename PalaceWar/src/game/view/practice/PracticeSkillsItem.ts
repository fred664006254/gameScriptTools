 ...
 */
class PracticeSkillsItem extends BaseDisplayObjectContainer
{
	...
	public init(servantId:string,bottomH):void
	{
		...
		for (var index = 0; index < 2 ; index++) {
			...
			let skillIcon =  BaseLoadBitmap.create("practice_skill_" + (index+1));
			skillIcon.width = 108;
		...
		let downTxt = ComponentManager.getTextField(LanguageManager.getlocal("praceticedownDes"),18,TextFieldConst.COLOR_BROWN);
	...
	protected refreshUIInfo(event?:egret.Event)
	{
		...
		this._lv1TipStr = "";
		this._lv2TipStr = "";
		let skillExp = Api.practiceVoApi.getExp; 
		...
		let txt4 = LanguageManager.getlocal("practice_skilllevelupTxt4");
		let txt5 = LanguageManager.getlocal("practice_skilllevelupTxt5");
		...
		//修身技能等级
		let skill = Api.practiceVoApi.curSkill;
		...
		// 每次增加
		let skillValue1:number =  practiceSkill.skillF.addValue;  
		let skillValue2:number =   practiceSkill.skillS.addValue;
