class ItemViewTab4 extends CommonViewTab
{
	// 当前选中的索引
	...
	// 道具icon
	private _selectedIcon:any = null;
	// 道具iconbg
	...
	// 时间文本
	private _selectedTimeTextF:BaseTextField = null;	
	// 使用按钮
	...
	private initButtomContainer(bg1:BaseBitmap):void
	{
		...
		let tfg = Config.TitleCfg.getTitleCfgById(this._selectedItemInfoVo.id);
		if(tfg && tfg.isChangePic()){
			let lv = this._selectedItemInfoVo.lv;
		...
		let eff = App.CommonUtil.getHeadEffect(this._selectedItemInfoVo.id.toString());
		...
		this._selectedTimeTextF = ComponentManager.getTextField("",TextFieldConst.FONTSIZE_CONTENT_SMALL,TextFieldConst.COLOR_WARN_RED2);
		this._selectedTimeTextF.x = this._selectedDropTextF.x;
		this._selectedTimeTextF.y = this._selectedDropTextF.y + 20 + 7;
		this._buttomContainer.addChild(this._selectedTimeTextF);
	...
	private updateItem(index:number):void
	{
		...
		{
			if(this._selectedIndex >= 0)   //直接去getItemByIndex如果在看不见的地方拿不到
			{
		...
		this._selectedTimeTextF.visible = this._selectedItemInfoVo.et && this._selectedItemInfoVo.et > 0;
		if(this._selectedTimeTextF.visible)
		{
			let day = Math.floor((this._selectedItemInfoVo.et - GameData.serverTime)/3600) + 1;
			if(day > 0)
			{
				this._selectedTimeTextF.text = LanguageManager.getlocal("sixSection1HoldTitleEffectTitleEndTime",[String(day)])
			}else
			{
				this._selectedTimeTextF.visible = false;
			}
		}
	...
	public dispose():void
	{
		...
		if(this._selectedTimeTextF)
		{
			this._buttomContainer.removeChild(this._selectedTimeTextF);
			this._selectedTimeTextF.dispose();
			this._selectedTimeTextF = null;
		}		
		if(this._useBtn)
		{
