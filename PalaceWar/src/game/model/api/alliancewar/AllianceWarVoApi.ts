...
class AllianceWarVoApi extends BaseVoApi{
	...
	*/
	...
	/**获得当前的是周几---指向派遣or战斗阶段 */
	public getWeeksWithDate():string
	{
		let weekStr: string = null;
		//服务器返回周一的时间
		let mondayStr = this.getWeekStrbyWeek(App.DateUtil.getServerDate(this.monday).getDay());
		//服务器返回周三的时间
		let wednesdayStr = this.getWeekStrbyWeek(App.DateUtil.getServerDate(this.monday + 2 * 86400).getDay())
		//服务器返回周五的时间
		let fridayStr = this.getWeekStrbyWeek(App.DateUtil.getServerDate(this.monday + 4 * 86400).getDay())
		weekStr = mondayStr + ", " + wednesdayStr + ", " + fridayStr;

		return weekStr;

	}
	private getWeekStrbyWeek(week:number)
	{
		switch (week) {
			case 0:
