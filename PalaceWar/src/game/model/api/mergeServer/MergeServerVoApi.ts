class MergeServerVoApi extends BaseVoApi
{
    ...
    */
     ...
     * 通过zid获得区服
     */
    public getSeverName(zid:number)
    {
        let str = '';
    ...
    */
    ...
    public getFuByZid(sid?: number):number{
        if (!sid)
        {
            sid = this.getTrueZid();
        }
        if(this.mergeZidCfg[sid]){
            let newzoneid = this.mergeZidCfg[sid].split('-');
    ...
    public getRankServerName(uid?):string{
		let server = "";
		let zid = Api.mergeServerVoApi.getTrueZid(uid);
