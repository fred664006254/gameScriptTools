	...
	export namespace AcCfg
	{
		...
		{	
           //获胜区奖励
			...
			//任务
			private _taskList:CrossServerPowerTaskItemCfg[] = [];
			//积分商店
			private _flagScoreShopList:CrossServerPowerFlagScoreItemCfg[] = [];
			//道具商店
			private _flagScoreShop2List:CrossServerPowerFlagScoreItemCfg[] = [];	

			//点兵遣将
			public solider:Object[] = null;
			//权势赏金
			private _sjList:CrossServerPowerSjItemCfg[] = [];	
				...
				for (let key in data){
					...
					if (key == "task"){
						...
						for (let k in data[key]){
							let item = new CrossServerPowerTaskItemCfg();
						...
						for (let k in data[key]){
							let item = new CrossServerPowerFlagScoreItemCfg();
							item.initData(data[key][k]);
							item.id = Number(k) + 1;
							item.isItem = false;
						...
						this._sjList = [];
						for (let k in data[key]){
							let item = new CrossServerPowerSjItemCfg();
							item.initData(data[key][k]);
							item.id = Number(k) + 1;
							this._sjList.push(item);
						}
					}									
				}
			...
			//task
			public getTaskList():CrossServerPowerTaskItemCfg[]{
				return this._taskList;
			}

			//shop
			public getShopList():CrossServerPowerFlagScoreItemCfg[]{
				return this._flagScoreShopList;
			}

			//shop2
			public getShop2List():CrossServerPowerFlagScoreItemCfg[]{
				return this._flagScoreShop2List;
			}	

			//赏金
			public getSjList():CrossServerPowerSjItemCfg[]{
				return this._sjList;
			}					
		}

		//任务
        class CrossServerPowerTaskItemCfg extends BaseItemCfg
        {
            public id:number;
            //跳转
		...
		//旗帜积分商店
        class CrossServerPowerFlagScoreItemCfg extends BaseItemCfg
        {
            public id:number;
            //排行榜上限
            public cost:number;
        ...
        class CrossServerPowerSjItemCfg extends BaseItemCfg
        {
            public id:number;
