namespace Config
{
	 ...
	 * 珍器坊配置
	 */
	export namespace ZhenqifangCfg{
		/*--珍器坊任务-个人
		...
		*/
		export let individual : any = null;
		 ...
		 *  --freeFresh:每日免费更换任务次数
    		...
    		--gemBuyTask:元宝刷新数组(超过上限取上限)
		*/
		export let friend : any = null;

		/** 
		 * 亲家加成
		*/
		export let rltvByMarr : number[] = [];
		...
		*/
		...
		export function getTaskCfgById(quality,id):any{
			return this.indivT[quality][id];
		}

		export function getTaskHouseCfgByLevel(level):any{
			return this.taskHouse[level];
		}

		export function getInviTaskNeedByType(type,id):any{
			return this.indivR[type][id];
		}

		export let needItem = 2040;
	}
}