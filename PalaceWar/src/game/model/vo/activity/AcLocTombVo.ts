class AcLocTombVo extends AcBaseVo
{
	...
	public get cfg() : Config.AcCfg.LocTombCfg{
    ...
    */
    ...
    public getActDayTimeCount():string{
	...
	*/
	...
	public getNextOpenTime():number{
		...
		let today0 = App.DateUtil.getWeeTs(GameData.serverTime);
        let time = 0;
        let opentime = this.cfg.getActTime();
	...
	public getArr(key):Array<any>{	
		let arr:Array<any> =[];
		let cfg : Config.AcCfg.LocTombCfg = Config.AcCfg.getCfgByActivityIdAndCode(this.aid, this.code);
		if(!cfg)
		{
			return [];
		}
    ...
    */
	public getCrossServer():string{
		...
		for(let i in this.zidarr){
			let zidname = Api.mergeServerVoApi.getAfterMergeSeverName(null,true, Number(this.zidarr[i]));
