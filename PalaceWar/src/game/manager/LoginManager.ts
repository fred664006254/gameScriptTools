...
namespace LoginManager 
{
    ...
    export let isNewGuideComplete:boolean=false;
	...
	function gameSocketConnectSuccessHandler():void
	{
        ...
        let apkey="8619EBC7EB8B87F34D1DD8EB563F0F64";
        var tmpTsStr: string = Date.now().toString();
        let sign=App.MD5.md5(apkey+uname+tmpTsStr);
    ...
    function requestUseLogin():void
    {
        if(GameConfig.isLoaded)
        {
            ...
            let parms:any = {plat:channel,enter:true};
    ...
    export function checkAndCreateScene():void
    {
        ...
        console.log("checkAndCreateScene"+App.LoginResLoader.isLoginResLoaded+isLoginSuccess+isNewGuideComplete+App.LoginResLoader.isHomesceneResLoaded);
        if(App.LoginResLoader.isLoginResLoaded&&isLoginSuccess&&isNewGuideComplete && App.LoginResLoader.isHomesceneResLoaded)
        {
            if(!isCreateScene)
            {
