 ...
 */
var SevenDaysSignUpFqScrollItem = (function (_super) {
    __extends(SevenDaysSignUpFqScrollItem, _super);
    function SevenDaysSignUpFqScrollItem() {
        return _super.call(this) || this;
    }
    SevenDaysSignUpFqScrollItem.prototype.initItem = function (index, data, itemParam) {
        ...
        if (data.id == "signUp3") {
            ...
            var titleBg = BaseBitmap.create("shopview_greenbar");
            titleBg.setPosition(itembg.x, itembg.y + 5);
            this.addChild(titleBg);
            var titleTxt = ComponentManager.getTextField(LanguageManager.getlocal("sevenDaysSign3Title"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
            titleTxt.setPosition(titleBg.x + 40, titleBg.y + titleBg.height / 2 - titleTxt.height / 2);
            this.addChild(titleTxt);
            var rewards = Config.SevendayssignupCfg.getSignup3CfgById(itemParam.day).getReward;
            ...
            if (itemParam.day <= Api.sevenDaysSignupLoginVoApi.nowDay()) {
                var isReceive = Api.sevenDaysSignupLoginVoApi.checkReadReward(itemParam.day);
                if (isReceive) {
        ...
        var fqbg = BaseLoadBitmap.create("sevendayssignupview_gamebg_" + itemParam.day + "_" + data.id);
        fqbg.width = 576; //576 × 290
        fqbg.height = 290;
        fqbg.setPosition(itembg.x + itembg.width / 2 - fqbg.width / 2, itembg.y + 12);
        this.addChild(fqbg);
        var offsetHeight = 0;
        for (var i = 1; i <= data.titleNumber; i++) {
            var singleHeight = 0;
            ...
            this.addChild(titlebg);
            var titleTF = ComponentManager.getTextField(LanguageManager.getlocal("sevenDaysSignUpViewGameTitle_" + itemParam.day + "_" + data.id + "_" + i), TextFieldConst.FONTSIZE_BUTTON_COMMON, TextFieldConst.COLOR_LIGHT_YELLOW);
            ...
            var desc = ComponentManager.getTextField(LanguageManager.getlocal("sevenDaysSignUpViewGameDesc_" + itemParam.day + "_" + data.id + "_" + i), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BLACK);
    ...
    SevenDaysSignUpFqScrollItem.prototype.dispose = function () {
        _super.prototype.dispose.call(this);
    };
    return SevenDaysSignUpFqScrollItem;
}(ScrollListItem));
__reflect(SevenDaysSignUpFqScrollItem.prototype, "SevenDaysSignUpFqScrollItem");
//# sourceMappingURL=SevenDaysSignUpFqScrollItem.js.map