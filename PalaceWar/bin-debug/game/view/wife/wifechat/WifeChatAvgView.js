 ...
 */
var WifeChatAvgView = (function (_super) {
    __extends(WifeChatAvgView, _super);
    function WifeChatAvgView() {
    ...
    WifeChatAvgView.prototype.getTitleStr = function () {
        return "wifeskinchat";
    };
    WifeChatAvgView.prototype.getResourceList = function () {
        return _super.prototype.getResourceList.call(this).concat([
            "wifechatview", "tailor_iconBtn"
        ]);
    };
    WifeChatAvgView.prototype.getRuleInfo = function () {
        return "wifechatrule";
    };
    WifeChatAvgView.prototype.initView = function () {
        var view = this;
        view._init = true;
        ...
        if (curid > 1) {
            ...
            while (nextId && nextId <= curid) {
                ...
                if (dialogCfg[nextId] && dialogCfg[nextId].choice) {
                    var selectarr = dialogCfg[nextId].choice;
                    var jumparr = dialogCfg[nextId].next;
    ...
    WifeChatAvgView.prototype.hide = function () {
        if (this._stopTouch) {
            return;
        }
    ...
    //获取数据
    WifeChatAvgView.prototype.getAvgDialog = function () {
        var view = this;
        view._tmp = [];
        var skincfg = Config.WifeskinCfg.getWifeCfgById(view.param.data.skinId);
        var wifeSkinVo = Api.wifeSkinVoApi.getWifeskinInfoVoById(skincfg.wifeId);
        var lv = view.param.data.lv;
        var id = skincfg.levelUp[lv - 1].levelUpUnlock;
        this._tmp = Config.WifeconversationCfg.getConversatiById(id);
        return this._tmp;
    };
    //文本处理
    WifeChatAvgView.prototype.initNextDialog = function (tween, juqingid, isend) {
        ...
        if (isend) {
            ...
            var lv = view.param.data.lv;
            var id = skincfg.levelUp[lv - 1].levelUpUnlock;
            var text = LanguageManager.getlocal("wifeskin" + skinId + "_" + id + "_" + avgid);
            var role = 0;
            ...
            if (role == 1) {
                ...
                pic.addChild(iconBg);
                var iconStr = "";
                var cfg = Config.WifeCfg.getWifeCfgById(skincfg.wifeId);
                iconStr = cfg.icon;
                // if(skincfg)
                // {
    ...
    WifeChatAvgView.prototype.initSelect = function (isend, notween) {
        var _this = this;
        ...
        var isself = role == 1;
        if (isend) {
            var resetcost = Config.WifechatCfg.resetCost;
            var rewardvo_1 = GameData.formatRewardItem(resetcost)[0];
            var btn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "wifeskinchat4", function () {
                if (rewardvo_1.num > Api.itemVoApi.getItemNumInfoVoById(rewardvo_1.id)) {
                    App.CommonUtil.showTip(LanguageManager.getlocal("acPunishBuyTitle"));
                }
                else {
                    ViewController.getInstance().openView(ViewConst.POPUP.CONFIRMPOPUPVIEW, {
                        msg: LanguageManager.getlocal("wifeskinleveluptip5", [rewardvo_1.num.toString()]),
            ...
            var str1 = ComponentManager.getTextField(LanguageManager.getlocal("wifeskinlvupcost"), 20);
            var icon = BaseLoadBitmap.create(rewardvo_1.icon);
            icon.width = 108;
            icon.height = 108;
            icon.setScale(40 / 108);
            var str2 = ComponentManager.getTextField(LanguageManager.getlocal("wifeskinlvupcostnunm", [rewardvo_1.num.toString(), Api.itemVoApi.getItemNumInfoVoById(rewardvo_1.id).toString()]), 20);
            ...
            if (dialogCfg[avgid] && dialogCfg[avgid].choice) {
                ...
                if (selectarr.length) {
                    ...
                    var _loop_1 = function (i) {
                        ...
                        var text = LanguageManager.getlocal("wifeskin" + skinId + "_" + id + "_" + avgid + "_" + i);
    ...
    WifeChatAvgView.prototype.initEnd = function (reward) {
        var view = this;
        var skinId = (view.param.data.skinId);
        var skincfg = Config.WifeskinCfg.getWifeCfgById(skinId);
        //给奖励
    ...
    WifeChatAvgView.prototype.upgradeCallback = function (evt) {
        var view = this;
        if (!evt.data.ret) {
            return;
        }
    ...
    WifeChatAvgView.prototype.resetCallback = function (evt) {
        var view = this;
        if (!evt.data.ret) {
            return;
        }
    ...
    WifeChatAvgView.prototype.dispose = function () {
        var view = this;
    ...
    return WifeChatAvgView;
}(CommonView));
__reflect(WifeChatAvgView.prototype, "WifeChatAvgView");
//# sourceMappingURL=WifeChatAvgView.js.map