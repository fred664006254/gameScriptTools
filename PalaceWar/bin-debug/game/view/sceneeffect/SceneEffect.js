...
var SceneEffect = (function (_super) {
    ...
    function SceneEffect() {
        ...
        _this._sceneId = null;
        _this._personTab = [];
        _this._bgResGroupName = null;
        _this._isResLoaded = false;
        _this.sceneType = null; // 1 , 2 ,3 ,4 这种type
        _this._bone = null;
        _this.nightcfg = {
            glow: [
                //1
    ...
    SceneEffect.prototype.showSceneEffect = function (key, sceneId) {
        if (sceneId === void 0) { sceneId = "202"; }
        ...
        for (var i = 0; i < this._keys.length; i++) {
            var names = this._keys[i].split("_");
    ...
    SceneEffect.prototype.showCommonSceneEffect = function (sceneId) {
        if (sceneId == "" || this._sceneId == sceneId) {
            return;
        }
        this.hideSceneEffect();
        this._sceneId = sceneId;
        var cfg = Config.SceneeffectCfg.getCommonCfg(sceneId);
        var resArray = [];
        for (var i in cfg) {
            var name_1 = i;
            var unit = cfg[i];
            //帧动画
            if (unit.frameNum) {
                for (var j = 1; j <= unit.num; ++j) {
                    var lampCfg = unit[j];
                    var clip = ComponentManager.getCustomMovieClip(name_1, unit.frameNum, unit.frameTime);
                ...
                for (var j = 1; j <= unit.num; ++j) {
                    var lampCfg = unit[j];
                    var img = BaseBitmap.create(name_1);
    ...
    SceneEffect.prototype.loadCompleteHandler = function () {
        this._isResLoaded = true;
        for (var i = 0; i < this._keys.length; i++) {
            var person = new ScenePerson();
        ...
        if (Config.SceneeffectCfg.hasSceneBone(this.sceneType, this._sceneId)) {
            var animName = Config.SceneeffectCfg.getBoneCfg(this.sceneType, this._sceneId);
            var boneName = animName + "_ske";
    ...
    SceneEffect.prototype.hideSceneEffect = function () {
    ...
    SceneEffect.prototype.showNightEffect = function () {
        //挂灯
        for (var i = 0; i < this.nightcfg.glow.length; i++) {
            var cfg = this.nightcfg.glow[i];
            var glow = BaseBitmap.create("scene_eff_glow");
        ...
        //光
        for (var i = 0; i < this.nightcfg.light.length; i++) {
            var cfg = this.nightcfg.light[i];
            var glow = BaseBitmap.create("scene_eff_light" + cfg.pic);
            glow.setPosition(cfg.p.x, cfg.p.y);
            glow.anchorOffsetX = glow.width / 2;
            glow.anchorOffsetY = glow.height / 2;
        ...
        var _loop_1 = function (i) {
            ...
            var denglong = BaseBitmap.create("scene_eff_denglong");
            ...
            this_1._personTab.push(node);
            this_1._personTab.push(dengxu);
            this_1._personTab.push(denglong);
            node.anchorOffsetX = node.width / 2;
            node.anchorOffsetY = node.height / 2;
            node.rotation = 35;
            var rt1 = [-3, 4, -8, 4, -8, -3];
            var t1 = [0, 0.2 * 2, 0.53 * 2, 0.87 * 2, 1.2 * 2, 1.33 * 2];
            var rt2 = [2.5, -2.5, 2.5, -2.5, 2.5];
            var t2 = [0, 0.33 * 2, 0.67 * 2, 1 * 2, 1.33 * 2];
            var isFirst = true;
        ...
        // }
        //飞灯
        for (var i = 0; i < this.nightcfg.deng.length; i++) {
            _loop_1(i);
        }
    };
    SceneEffect.prototype.dispose = function () {
        this.hideSceneEffect();
        this._sceneId = null;
        if (this._bone) {
            this._bone.dispose();
...
__reflect(SceneEffect.prototype, "SceneEffect");
