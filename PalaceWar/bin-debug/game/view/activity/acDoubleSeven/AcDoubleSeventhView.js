...
var AcDoubleSeventhView = (function (_super) {
    ...
    function AcDoubleSeventhView() {
        ...
        _this._lampNanguaEye = null;
        _this._lampNanguaMouth = null;
        _this._gquan = null;
        _this._showAVG = false;
        _this._acCDTxt = null;
        _this._countDownTimeBg = null;
        _this._skinPreviewBtn = null;
        _this._particleEffArr = [];
    ...
    Object.defineProperty(AcDoubleSeventhView.prototype, "codeResStr", {
        get: function () {
            var str = "";
    ...
    Object.defineProperty(AcDoubleSeventhView.prototype, "ruleCode", {
        get: function () {
    ...
    AcDoubleSeventhView.prototype.initView = function () {
        ...
        var top = BaseBitmap.create("acseventopbg" + this.codeResStr);
        if (this.code == '4') {
        ...
        if (String(this.code) === "1") {
            var man = BaseBitmap.create('wife_skinhalf_1091');
            man.setScale(0.9);
            view.setLayoutPosition(LayoutConst.leftbottom, man, top, [0, 30]);
            view.addChild(man);
        }
        //活动时间
        var ac_timeTF = ComponentManager.getTextField(LanguageManager.getlocal("acDoubleSeventhTime", [view.vo.acTimeAndHour]), TextFieldConst.FONTSIZE_CONTENT_SMALL);
        ...
        //活动规则文本
        var acruleTxt = ComponentManager.getTextField(LanguageManager.getlocal("acDoubleSeventhRule", [LanguageManager.getlocal('acDoubleSeventhRuleDesc' + this.ruleCode)]), TextFieldConst.FONTSIZE_CONTENT_SMALL);
        ...
        if (this.code == "5") {
            var iconname = "scene_preview_icon_" + this.cfg.getExchangeSceneId();
            var skinPreviewBtn = ComponentManager.getButton(iconname, null, function () {
            ...
            var itemnum = this.cfg.getExchangeNeedItemNum();
            this._itemNumText = ComponentManager.getTextField(LanguageManager.getlocal("acDoubleSeventhItemNum", [String(itemnum)]), TextFieldConst.FONTSIZE_CONTENT_SMALL);
        ...
        if (String(this.code) === "2") {
            // 南瓜车
            var nanguache = ComponentManager.getCustomMovieClip("nanguache", 12, 83);
            var startX = -200;
            var startY = 870 + Math.floor(view.viewBg.y);
            var xMove = 2000;
            var yMove = -xMove / 6.2;
            var time = xMove / GameConfig.stageWidth * 15000; // 15秒x移动一屏幕的宽度像素
            ...
            // 南瓜车 某建筑的遮罩
            var nanguachezhezhao = BaseBitmap.create("nanguachezhezhao");
        ...
        if (String(this.code) === "3" || String(this.code) === "4") {
            var rippleEff = ComponentManager.getCustomMovieClip("ripple", 8, 100);
            rippleEff.setPosition(0, top.y + top.height + 229 + Math.floor(view.viewBg.y));
            rippleEff.playWithTime(-1);
            view.addChild(rippleEff);
            var bgpng = BaseBitmap.create("acsevenbg_png");
            bgpng.setPosition(0, this.viewBg.y);
            bgpng.touchEnabled = false;
            view.addChild(bgpng);
        }
        //建筑
        var isTh = PlatformManager.checkIsThSp();
        // 是不是万圣节
        var isCode2 = String(this.code) === "2";
        // 是不是水灯节
        var isCode3 = String(this.code) === "3" || String(this.code) === "4";
        // 是不是水灯节
        var isCode4 = String(this.code) === "5";
        var effpos_arr = {};
        var pos_arr = {
        ...
        if (String(this.code) === "1" || isCode3) {
            ...
            for (var i in pos_arr) {
                var unit = pos_arr[i];
                var buildPic = BaseBitmap.create("acsevenbuilding" + unit.buildId + codeStr);
                ...
                if (codeStr == "_code3") {
                    // this._particleEffArr = [];
                    var particleEff = ComponentManager.getCustomMovieClip("particle", 8, 56);
        ...
        //当前进度与显示
        var lqBtn = ComponentManager.getButton('acsevenlampgray' + this.codeResStr, '', view.lqBtnClick, view);
        view.setLayoutPosition(LayoutConst.horizontalCenterbottom, lqBtn, view, [0, 10]);
        view.addChild(lqBtn);
        view._lqbtn = lqBtn;
        var acchargetipTxt = ComponentManager.getTextField('', TextFieldConst.FONTSIZE_CONTENT_SMALL);
        view._acchargetipTxt = acchargetipTxt;
        view.addChild(acchargetipTxt);
        view.swapChildren(view.closeBtn, top); //view.closeBtn
        //查看奖励
        var ckjliBtn = ComponentManager.getButton('acsevenckjl', '', view.ckjlCall, view);
        view.setLayoutPosition(LayoutConst.leftbottom, ckjliBtn, view, [20, 30]);
        view.addChild(ckjliBtn);
        //充值
        var czhiBtn = ComponentManager.getButton('acsevenczhi', '', view.czhiCall, view);
        ...
        var font = BaseBitmap.create('acsevendliang' + this.codeResStr);
        ...
        // 倒计时
        if (this.code == "5") {
            var countDownBg = BaseBitmap.create("public_9_bg61");
            countDownBg.y = top.y + top.height - countDownBg.height / 2;
            view.addChild(countDownBg);
            this._countDownTimeBg = countDownBg;
            var deltaT = Math.max(0, this.acVo.et - GameData.serverTime);
            var acCDTxt = ComponentManager.getTextField("", 18, TextFieldConst.COLOR_WHITE);
            ...
            view.addChild(countDownBg);
            var deltaT = Math.max(0, this.acVo.et - GameData.serverTime);
            var acCDTxt = ComponentManager.getTextField("", 20, TextFieldConst.COLOR_WHITE);
            view.addChild(acCDTxt);
            this._acCDTxt = acCDTxt;
            this._acCDTxt.text = LanguageManager.getlocal("acFourPeople_acCD", [App.DateUtil.getFormatBySecond(deltaT, 1)]);
    ...
    AcDoubleSeventhView.prototype.initBuildBg = function () {
        ...
        // }
        var curCfg = Config.SceneCfg.getSceneCfgBySceneName("searchScene", "303");
        var buildBgCfg = curCfg.buildBgCfg;
        if (buildBgCfg) {
            for (var key in buildBgCfg) {
                var resName = void 0;
                ...
                if (this._maskTab[Number(key) - 1]) {
                    var mask = this._maskTab[Number(key) - 1];
    ...
    AcDoubleSeventhView.prototype.avgendCallback = function () {
        ...
        var pos = new egret.Point(view._lqbtn.x + (view._lqbtn.width / 2), view._lqbtn.y + (view._lqbtn.height / 2));
    ...
    AcDoubleSeventhView.prototype.fresh_jindu_OfBuild = function () {
        var view = this;
        var curJindu = view.vo.getCurJindu();
        var chargeNum = view.vo.getChargeNum();
        for (var i = 1; i <= curJindu; ++i) {
    ...
    AcDoubleSeventhView.prototype.hide = function () {
        if (this._showAVG) {
            return;
        }
        else {
    ...
    AcDoubleSeventhView.prototype.tick = function () {
        ...
        if (this._acCDTxt && deltaT > 0) {
            ...
            if (String(this.code) == "5") {
                ...
                this._acCDTxt.setPosition(this._countDownTimeBg.x + this._countDownTimeBg.width / 2 - this._acCDTxt.width / 2, this._countDownTimeBg.y + this._countDownTimeBg.height / 2 - this._acCDTxt.height / 2);
                this.checkScene();
            }
            return true;
        }
    ...
    AcDoubleSeventhView.prototype.checkScene = function () {
    ...
    AcDoubleSeventhView.prototype.resetSceneBg = function () {
        if (!this.viewBg) {
            return;
        }
        var view = this;
        var mybg = this.viewBg;
        this.viewBg = null;
    ...
    AcDoubleSeventhView.prototype.update = function () {
...
//# sourceMappingURL=AcDoubleSeventhView.js.map