...
var AcConquerMainLandDetailViewTab2Tab2 = (function (_super) {
    ...
    AcConquerMainLandDetailViewTab2Tab2.prototype.initView = function () {
        ...
        var str = '';
        var rankList = view.cfg.serverRank;
        var tmpX = 20;
        var scroStartY = 3;
        var rankmax = view.vo.zidarr.length;
        for (var index = 0; index < rankList.length; index++) {
            var id = index;
            var rItem = rankList[index];
            var key = index + 1;
            var ranknum = 0;
            var rank = rItem.serRank;
            if (rankmax < rank[0]) {
                continue;
            }
            if (Number(key) < 4) {
    ...
    AcConquerMainLandDetailViewTab2Tab2.prototype.getCountTimeStr = function (time) {
        if (time < 0) {
            time = 0;
        }
...
//# sourceMappingURL=AcConquerMainLandDetailViewTab2Tab2.js.map