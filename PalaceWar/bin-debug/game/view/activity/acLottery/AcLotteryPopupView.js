...
var AcLotteryPopupView = (function (_super) {
    ...
    AcLotteryPopupView.prototype.initView = function () {
        ...
        for (var i = 0; i < this._nameList.length; i++) {
            ...
            var nameText = ComponentManager.getTextField(nameObj[1], 22, TextFieldConst.COLOR_WHITE);
            nameText.x = 20 + 30;
            nameText.y = startY + 30 * i;
            var gemText = ComponentManager.getTextField(nameObj[2], 22, TextFieldConst.COLOR_WHITE);
            gemText.x = 180;
            gemText.y = startY + 30 * i;
            var timeText = ComponentManager.getTextField(nameObj[3], 22, TextFieldConst.COLOR_WHITE);
            timeText.x = 290 + 20;
        ...
        if (this._nameList.length == 0) {
            var emptyText = ComponentManager.getTextField(LanguageManager.getlocal("acLotteryEmpty"), 24, TextFieldConst.COLOR_WHITE);
            ...
            //玩家昵称
            var title = ComponentManager.getTextField(LanguageManager.getlocal("ranknickName"), 24, TextFieldConst.COLOR_LIGHT_YELLOW);
            title.x = 50 + GameData.popupviewOffsetX;
            title.y = 15;
            this.addChildToContainer(title);
            //奖励
            var title2 = ComponentManager.getTextField(LanguageManager.getlocal("acLotteryReward"), 24, TextFieldConst.COLOR_LIGHT_YELLOW);
            title2.x = 220 + GameData.popupviewOffsetX;
            title2.y = title.y;
            this.addChildToContainer(title2);
            //时间 
            var title3 = ComponentManager.getTextField(LanguageManager.getlocal("acLotterytime"), 24, TextFieldConst.COLOR_LIGHT_YELLOW);
...
//# sourceMappingURL=AcLotteryPopupView.js.map