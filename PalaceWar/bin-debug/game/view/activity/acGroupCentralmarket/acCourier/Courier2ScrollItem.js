 ...
 * 攀升item
 */
var Courier2ScrollItem = (function (_super) {
    __extends(Courier2ScrollItem, _super);
    function Courier2ScrollItem() {
    ...
    Object.defineProperty(Courier2ScrollItem.prototype, "nowCode", {
        /**
         * 使用的code 仅仅使用资源，cn
         */
        get: function () {
    ...
    Courier2ScrollItem.prototype.initItem = function (index, data, itemParam) {
        this._data = data;
        ...
        if (PlatformManager.checkIsTextHorizontal()) {
            ...
            if (data.questType == "1002") {
                ...
                //礼包名字 
                var dayNum = Courier2ScrollItem.TADAY;
                var str1 = LanguageManager.getlocal("acCourierPackageName" + dayNum + "_" + this.nowCode);
                ...
                if (AcCourierViewTab2.isStarBoo) {
                    gobtnTypeNum_1 = tmpVo.getIdflag(data.questType, this._data.day);
                    if (gobtnTypeNum_1 == 2) {
                ...
                var str = 'acNewYearquestType' + data.questType;
                if (Number(data.questType) == 1003) {
                    str = "acCourierType1003";
                }
                tadayTaskTxt.text = LanguageManager.getlocal(str, [num1 + "", data.value]);
                ...
                if (this._goBtn2 && GameData.serverTime > tmpVo.et - 86400) {
                    App.DisplayUtil.changeToGray(this._goBtn2);
                }
                if (AcCourierViewTab2.isStarBoo) {
                    this.update();
            ...
            if (data.questType == "1002") {
                ...
                var gobtnTypeNum = 0;
                if (AcCourierViewTab2.isStarBoo) {
                    gobtnTypeNum = tmpVo.getIdflag(data.questType, this._data.day);
                    if (gobtnTypeNum == 2) {
                        var collectflag = BaseBitmap.create("collectflag");
                        collectflag.x = 450;
                        collectflag.y = 50;
        ...
        if (GameData.serverTime > this._tmpVo.et - 86400 || this._data.day < tmpVo.diffday) {
            if (this._goBtn2) {
                App.DisplayUtil.changeToGray(this._goBtn2);
            }
            if (this._goBtn) {
        ...
        if (GameData.serverTime >= this._tmpVo.et) {
            if (this._goBtn3) {
    ...
    Courier2ScrollItem.prototype.onPackageHandler = function (evt) {
        var tmpVo = Api.acVoApi.getActivityVoByAidAndCode(AcConst.AID_COURIER, this._code);
        if (this._data.day < tmpVo.diffday) {
    ...
    Courier2ScrollItem.prototype.collectHandler = function (evt) {
        var tmpVo = Api.acVoApi.getActivityVoByAidAndCode(AcConst.AID_COURIER, this._code);
        if (GameData.serverTime >= tmpVo.et) {
            App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
            return;
        }
        if (this._data.day < tmpVo.diffday) {
            ...
            // 	NetManager.request(NetRequestConst.REQUEST_ACTIVITY_GETCOURIERREWARD,{"activeId":AcConst.AID_COURIER+"-"+this._code,"questType":this._data.questType+"","ftype":1});
            // }
            // else{
            // 	App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
            // }
    ...
    Courier2ScrollItem.prototype.refreshUIInfo = function (evt) {
        if (evt.data.ret == true) {
            this.update();
        }
    };
    Courier2ScrollItem.prototype.update = function () {
        // 0不可领 1未领取 2已领取
        if (this._data && this._data.questType) {
            var gobtnTypeNum = this._tmpVo.getIdflag(this._data.questType, this._data.day);
        ...
        if (this._data.questType != "1002") {
                ...
                var num1 = 0;
            {
                if (this._tmpVo.taskinfo.info[this._data.questType] && this._tmpVo.taskinfo.info[this._data.questType].v) {
                    ...
                    if (this._tadayTaskTxt) {
                        var str = 'acNewYearquestType' + this._data.questType;
                        if (Number(this._data.questType) == 1003) {
                            str = "acCourierType1003";
                        }
                        this._tadayTaskTxt.text = LanguageManager.getlocal(str, [num1 + "", this._data.value]);
                    }
                }
            }
        }
        if (GameData.serverTime > this._tmpVo.et - 86400 || this._data.day < this._tmpVo.diffday) {
            if (this._goBtn2) {
                App.DisplayUtil.changeToGray(this._goBtn2);
            }
            if (this._goBtn) {
    ...
    Courier2ScrollItem.prototype.buyHandler = function (param) {
        var tmpVo = Api.acVoApi.getActivityVoByAidAndCode(AcConst.AID_COURIER, this._code);
        var tmpCfg = Config.AcCfg.getCfgByActivityIdAndCode(AcConst.AID_COURIER, this._code);
        if (this._data.day < tmpVo.diffday) {
            App.CommonUtil.showTip(LanguageManager.getlocal("acPunishEnd"));
            return;
        }
        ...
        if (this._data.value > Api.playerVoApi.getPlayerGem()) {
            ...
            var itemName = LanguageManager.getlocal("acCourierPackageName" + Courier2ScrollItem.TADAY + "_" + this.nowCode);
            if (Api.switchVoApi.checkOpenNewYearActiveTwoConfirm() && costNum >= tmpCfg.maxGem) {
                ...
                App.MessageHelper.addEventListener(NetManager.getMessageName(NetRequestConst.REQUEST_ACTIVITY_BUYCOURIERGIFT), this.useCallback, this);
                NetManager.request(NetRequestConst.REQUEST_ACTIVITY_BUYCOURIERGIFT, { "activeId": AcConst.AID_COURIER + "-" + this._code });
            }
        }
    };
    Courier2ScrollItem.prototype.confirmCallbackHandler = function () {
        App.MessageHelper.addEventListener(NetManager.getMessageName(NetRequestConst.REQUEST_ACTIVITY_BUYCOURIERGIFT), this.useCallback, this);
        NetManager.request(NetRequestConst.REQUEST_ACTIVITY_BUYCOURIERGIFT, { "activeId": AcConst.AID_COURIER + "-" + this._code });
    };
    Courier2ScrollItem.prototype.useCallback = function (event) {
        if (event.data.ret) {
    ...
    Courier2ScrollItem.prototype.getSpaceY = function () {
        return 0;
    };
    Courier2ScrollItem.prototype.dispose = function () {
        this._goBtn3 = null;
        this._goBtn2 = null;
        this._goBtn = null;
        ...
        App.MessageHelper.removeEventListener(NetManager.getMessageName(NetRequestConst.REQUEST_ACTIVITY_BUYCOURIERGIFT), this.useCallback, this);
    ...
    Courier2ScrollItem.TADAY = 0;
    return Courier2ScrollItem;
}(ScrollListItem));
__reflect(Courier2ScrollItem.prototype, "Courier2ScrollItem");
//# sourceMappingURL=Courier2ScrollItem.js.map