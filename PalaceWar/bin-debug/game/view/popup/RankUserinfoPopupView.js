...
var RankUserinfoPopupView = (function (_super) {
    ...
    RankUserinfoPopupView.prototype.initView = function () {
        ...
        var title = App.CommonUtil.getTitleData(UIData.title);
        if (title.clothes) {
            var titleCfg = Config.TitleCfg;
            var titleconfig = titleCfg.getTitleCfgById(title.clothes);
        ...
        var posX = 20;
        var tinfo = App.CommonUtil.getTitleData(UIData.title);
        var pic = UIData.pic;
        if (tinfo.clothes != "") {
        ...
        var temX = this.viewBg.x + this.viewBg.width - 230 - GameData.popupviewOffsetX;
        var temY = fanImg.y;
        ...
        var alliStr = LanguageManager.getlocal("allianceCreateViewTitle") + ": ";
        ...
        if (UIData && UIData.crossZone) {
            var serverTxt = ComponentManager.getTextField(LanguageManager.getlocal('crossranserver', [UIData.zid.toString()]), 20, TextFieldConst.COLOR_WARN_YELLOW);
        ...
        if (Number(UIData.ruid) == Api.playerVoApi.getPlayerID() || !UIData.hideVip) {
            if (UIData.vip > 0) {
                ...
                var vipImg = Api.vipVoApi.getVipIcon2(UIData.vip);
        ...
        var officeTF = ComponentManager.getBitmapText(Api.playerVoApi.getPlayerOfficeByLevel(UIData.level), "office_fnt", 0xfff000);
        ...
        var proPosY = playerview_probg.y + 60 - 50;
        ...
        this.refreshProTxt();
        //下面部分改成整体滑动
        var bottomScrollNode = new BaseDisplayObjectContainer();
        var posY = 15;
        if (UIData.bio && Object.keys(UIData.bio).length > 0) {
            var lineb = BaseBitmap.create("public_line3");
            lineb.width = 500;
            lineb.x = tarWidth / 2 - lineb.width / 2;
            lineb.y = posY;
            bottomScrollNode.addChild(lineb);
            var bio_titleTxt = ComponentManager.getTextField(LanguageManager.getlocal("biography_title"), 22, 0x272727);
            bio_titleTxt.x = tarWidth / 2 - bio_titleTxt.width / 2;
            bio_titleTxt.y = lineb.y + lineb.height / 2 - bio_titleTxt.height / 2;
            bottomScrollNode.addChild(bio_titleTxt);
            var probgb = BaseBitmap.create("public_9_bg21");
            probgb.width = 500;
            probgb.height = 190;
            probgb.x = tarWidth / 2 - probgb.width / 2 + bottomBg.x;
            probgb.y = lineb.y + 30;
            bottomScrollNode.addChild(probgb);
            var infoObject_1 = UIData.bio;
            var keys = Object.keys(infoObject_1);
            keys.sort(function (a, b) {
                var cfga = Config.BiographyCfg.getCfgBgId(infoObject_1[a].id);
                var cfgb = Config.BiographyCfg.getCfgBgId(infoObject_1[b].id);
                ...
                if (cfga.sortID > cfgb.sortID) {
                    ...
                    return infoObject_1[b].st - infoObject_1[a].st;
                }
                return -1;
            });
            for (var i = 0; i < keys.length; i += 2) {
                var info1 = infoObject_1[keys[i]];
                var key1 = info1.id;
                var leftContainer = Api.biographyVoApi.getBookInfoContainerByInfo(info1, true, UIData.ruid, UIData.name);
                leftContainer.setPosition(56, posY + 45);
                leftContainer.setScale(0.8);
                bottomScrollNode.addChild(leftContainer);
                if (keys[i + 1]) {
                    var info2 = infoObject_1[keys[i + 1]];
                    var key2 = info2.id;
                    var rightContainer = Api.biographyVoApi.getBookInfoContainerByInfo(info2, true, UIData.ruid, UIData.name);
        ...
        var titleinfo = this.sortTitleInfo(UIData.titleinfo);
        var startX = probg1.x + 10;
        var startY = probg1.y + 5 + 10;
        var addH = 0;
        var titleNum = 0;
        ...
        for (var key in titleinfo) {
            if (titleinfo[key]["isHas"] >= 1) {
                ...
                titleNum++;
                var officerImg = App.CommonUtil.getTitlePic(titleinfo[key].id, titleinfo[key]["isHas"]); //BaseLoadBitmap.create("user_title_" + titleinfo[key].id +"_3");
                var deltaV = 0.8;
        ...
        var bgHeight = startY - probg1.y + 45;
        if (bgHeight > probg1.height) {
            probg1.height = bgHeight;
        }
        bottomScrollNode.height = probg1.y + probg1.height + 10;
        var rectb = new egret.Rectangle(0, 0, this.viewBg.width, bottomBg.height - 40);
        var bottomScrollView = ComponentManager.getScrollView(bottomScrollNode, rectb);
        ...
        if (UIData.promoteType) {
            var api = Api.promoteVoApi;
            if (api.isKing()) {
                var xrenBtn = ComponentManager.getButton(ButtonConst.BTN_NORMAL_YELLOW, 'PromotePlayersPopViewcancel', this.cancelPosConfirm, this);
            ...
            var DesctText = ComponentManager.getTextField(LanguageManager.getlocal('PromotePlayersPopViewText', [LanguageManager.getlocal("promoteType" + UIData.promoteType), LanguageManager.getlocal("promoteType" + UIData.promoteType + "eff")]), 20);
            ...
            var dinnerBtn = ComponentManager.getButton(ButtonConst.BTN_SMALL_YELLOW, "dinner_shotJoin", this.dinerBtnHandler, this);
            ...
            var btnImg = ButtonConst.BTN_SMALL_RED;
            var btnTxt = "";
            if (friendflag == 1) {
    ...
    RankUserinfoPopupView.prototype.cancelPosConfirm = function () {
        var view = this;
        var cfg = Config.PromoteCfg;
    ...
    RankUserinfoPopupView.prototype.cancel = function () {
        var view = this;
        var UIData = view.param.data;
    ...
    RankUserinfoPopupView.prototype.cancelCallback = function (evt) {
        var view = this;
    ...
    /**称号排序 */
    RankUserinfoPopupView.prototype.sortTitleInfo = function (titleinfo) {
        // let sorttitleinfo = {};
        var titleCfgList = [];
        for (var key in titleinfo) {
            var titleItemcfg = Config.TitleCfg.getTitleCfgById(key);
            titleItemcfg["isHas"] = titleinfo[key]; //isHas 不属于配置里面的 暂且加一个字段
            titleCfgList.push(titleItemcfg);
        }
        titleCfgList.sort(function (a, b) { return a.sortId - b.sortId; });
...
//# sourceMappingURL=RankUserinfoPopupView.js.map