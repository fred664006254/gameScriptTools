...
var AllianceWarJoinBattleInfoScrollItem = (function (_super) {
    ...
    AllianceWarJoinBattleInfoScrollItem.prototype.initItem = function (index, data) {
        ...
        var tipTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarJoinBattleInfoAdditionPo" + data.po, [String(addInfo.level), addInfo.addition]), TextFieldConst.FONTSIZE_ACTIVITY_COMMON, 0x3e9c00);
        ...
        for (var i = 0; i < data.poList.length; i++) {
            ...
            this.addChild(line);
            var playerNameTxt = ComponentManager.getTextField(data.poList[i].name, TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BROWN);
            playerNameTxt.setPosition(bg.x + 45, line.y + line.height + 10);
            this.addChild(playerNameTxt);
            if (data.poList[i].stra) {
                var straTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarPlanType" + data.poList[i].stra), TextFieldConst.FONTSIZE_CONTENT_COMMON);
            ...
            var servantNameTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarJoinBattleInfoServantName", [servantNamrStr]), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BROWN);
            servantNameTxt.setPosition(playerNameTxt.x, playerNameTxt.y + playerNameTxt.height + 10);
            this.addChild(servantNameTxt);
            var fightTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarJoinBattleInfoServantFight"), TextFieldConst.FONTSIZE_CONTENT_COMMON, TextFieldConst.COLOR_BROWN);
            ...
            if (titleAdd) {
                var titelTxt = ComponentManager.getTextField(LanguageManager.getlocal("allianceWarTitleAdd", [String(titleAdd)]), TextFieldConst.FONTSIZE_CONTENT_SMALL, TextFieldConst.COLOR_WARN_GREEN2);
...
//# sourceMappingURL=AllianceWarJoinBattleInfoScrollItem.js.map