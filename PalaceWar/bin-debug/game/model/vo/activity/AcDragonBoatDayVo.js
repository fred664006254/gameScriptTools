...
var AcDragonBoatDayVo = (function (_super) {
    ...
    AcDragonBoatDayVo.prototype.getCurJindu = function () {
        ...
        for (var i in this.cfg.teamReward) {
            if (curRice < this.cfg.teamReward[i].needMeter) {
                ...
                // 	break;
                // }
                if (!this.isGetJinduAward(Number(i) + 1)) {
                    curJindu = (Number(i) + 1);
                    break;
                }
            }
    ...
    AcDragonBoatDayVo.prototype.getCurPeriod = function () {
        var curJindu = 1;
        var curRice = this.getTotalRiceNum();
        for (var i in this.cfg.teamReward) {
            if (curRice <= this.cfg.teamReward[i].needMeter) {
        ...
        if (this.getTotalRiceNum() >= this.getEndMeter()) {
            curJindu = Object.keys(this.cfg.teamReward).length;
        }
        return curJindu;
    };
    //获取累积充值数目
    ...
    AcDragonBoatDayVo.prototype.getCountDown = function () {
        var num = 0;
        if (this.isInActivity()) {
            num = this.et - 1 * 86400 - GameData.serverTime;
        }
        else {
            num = 0;
        }
...
//# sourceMappingURL=AcDragonBoatDayVo.js.map