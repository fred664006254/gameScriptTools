...
var Socket = (function (_super) {
    ...
    Socket.prototype.connect = function (host, port) {
        ...
        if (NetManager.checkHttps() || (PlatformManager.checkIsWxSp() && this._host.indexOf("192.168.") < 0) || App.TestUtil.getTestPlat() == "wx") {
            ...
            var url = "wss://" + this._host + ":" + String(Number(this._port) + addValue);
            var proxy = this.getProxy(this._host, Number(this._port) + addValue);
            if (proxy) {
                url = "wss://" + proxy;
            }
            ...
            var proxy = this.getProxy(this._host, this._port);
            if (proxy) {
    ...
    Socket.prototype.getProxy = function (host, port) {
        var url = "";
        var proxy = window['gameproxyline'];
        if (proxy) {
            var tmpHost = host.split(".")[0];
