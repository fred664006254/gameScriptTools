...
var TabBarGroup = (function (_super) {
    ...
    TabBarGroup.prototype.init = function (buttonName, textArr, callback, handler, param, aligh, buttonWidth, showPic, showWidth, showHeight) {
        ...
        if (aligh == TabBarGroup.ALIGN_VERTICAL) {
            ...
            if (buttonName == ButtonConst.BTN2_TAB || buttonName == ButtonConst.BTN_BIG_TAB2 || buttonName == ButtonConst.BTN2_SMALL_TAB) {
                rect.setTo(0, 0, GameConfig.stageWidth, 66);
            }
            this.scrollView = ComponentManager.getScrollView(this.scrollContiner, rect);
    ...
    TabBarGroup.prototype.addZshi = function () {
        if (this._buttonName == ButtonConst.BTN_BIG_TAB2) {
            var zshi = BaseBitmap.create("commonview_tabbarzshi");
            App.DisplayUtil.setLayoutPosition(LayoutConst.lefttop, zshi, this, [-zshi.width + 8, 10], true);
            this.addChildAt(zshi, 2);
            var zshi2 = BaseBitmap.create("commonview_tabbarzshi");
            zshi2.scaleX = -1;
            App.DisplayUtil.setLayoutPosition(LayoutConst.righttop, zshi2, this, [-zshi.width - 30, 10], true);
            this.addChildAt(zshi2, 3);
        }
        // let tab = this.getTabBar(0);
        // let zshi = BaseBitmap.create(`commonview_tabbarzshi`);
        // App.DisplayUtil.setLayoutPosition(LayoutConst.lefttop, zshi, tab, [-zshi.width+8,-3], true);
        // tab.addChild(zshi);
        // let tab2 = this.getTabBar(this._tbArr.length - 1);
        // let zshi2 = BaseBitmap.create(`commonview_tabbarzshi`);
        // App.DisplayUtil.setLayoutPosition(LayoutConst.righttop, zshi2, tab2, [-zshi.width-30,-3], true);
        // tab2.addChild(zshi2);
    };
    ...
    TabBarGroup.prototype.addRedPoint = function (index) {
        ...
        for (var i = 0; i < l; i++) {
            if (i == index && this._tbArr[i]) {
                App.CommonUtil.addIconToBDOC(this._tbArr[i]);
                var red = this._tbArr[i].getChildByName("reddot");
                this.setRedPos(i, this._tbArr[i]._buttonImage.width - red.width, 0);
            }
        }
    };
    TabBarGroup.prototype.setRedPos = function (index, posx, posy) {
        if (this._tbArr && this._tbArr[index]) {
