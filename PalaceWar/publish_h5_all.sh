...
if [[ $childFile == '' ]];then
    ...
    if [ "$platname" = "iosshenhe"]
    then
        /usr/local/bin/node $filepath/obfuscated.js $filepath/bin-release/web/gt_$platname/js/main.min.js
    fi
...
replaceResource()
{
    ...
    if [[ $childFile == '' ]];then
        ...
        if [ -f "$filepath2/$zhengshiname/gt_$platname/index.php" ]; then
            ...
            then
                sudo mkdir -p -m 777 /Library/WebServer/Documents/publish
                sudo cp -rf $filepath/template/index.php /Library/WebServer/Documents/publish/
            else
                cp -rf $filepath/template/index.php /Library/WebServer/Documents/publish/
            fi

            if [ "$localpath" = "local" ]
            then
                curl -d "publishplat=$platname&local=$localpath" 127.0.0.1/publish/index.php > $filepath2/$zhengshiname/gt_$platname/index.html
            else
                curl -d "publishplat=$platname" 127.0.0.1/publish/index.php > $filepath2/$zhengshiname/gt_$platname/index.html
            fi
